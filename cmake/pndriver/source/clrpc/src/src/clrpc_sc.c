	...
	)
{
	...
	len   = (LSA_INT32)(scall->up_call->ndr_len - scall->send.frag_curr_fragnum * CLRPC_MUST_RECV_BODY_SIZE);
	...
	CLRPC_MEMCPY (body, scall->up_call->ndr_data + scall->send.frag_curr_fragnum * CLRPC_MUST_RECV_BODY_SIZE, (LSA_UINT)len);
...
# pragma CLRPC_MESSAGE("VV: drop this ugly up_call...")
		...
		if (scall->up_call->alloc_len > 0)
		{
			CLRPC_ALLOC_UPPER_MEM (CLRPC_CAST_UPPER_MEM_PTR_PTR (&scall->up_call->ndr_data)
				, clrpc_null_user_id
				, scall->up_call->alloc_len, scall->server->channel->sysptr
				);
	...
	)
{
	...
	if (CLRPC_RQB_GET_RESPONSE (scall->upper) == CLRPC_ERR_SERVER_BUSY)
	{
		...
		clrpc_sc_send_error (scall, CLRPC_PT_REJECT, CLRPC_NCA_SERVER_TOO_BUSY);
		return;
	}

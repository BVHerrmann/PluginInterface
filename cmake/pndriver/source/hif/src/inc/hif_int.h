#ifndef HIF_INT_H               /* ----- reinclude-protection ----- */
#define HIF_INT_H
...
#define HIF_FCT_PTR_NULL             (0)

#define HIF_IS_NULL(ptr)			 (LSA_HOST_PTR_ARE_EQUAL((ptr), LSA_NULL))
#define HIF_IS_NOT_NULL(ptr)		 (! LSA_HOST_PTR_ARE_EQUAL((ptr), LSA_NULL))
#define HIF_FCT_IS_NOT_NULL(fct_ptr) (fct_ptr != HIF_FCT_PTR_NULL)
#define HIF_ARE_EQUAL(ptr1,ptr2)	 (LSA_HOST_PTR_ARE_EQUAL((ptr1), (ptr2)))

/* checks whether an unsigned32 - referenced by ptr - is not null */
#define HIF_IS_NOT_NULL_U32_REF_BY_PTR(ptr) (!(0 == *((LSA_UINT32*)HIF_CAST_TO_VOID_PTR(ptr))))
...
#define HIF_BUFFER_ADMIN_LIST_ENTRIES       100          // max entries in hif buffer administration
...
typedef struct hif_pipe_tag            const * HIF_PIPE_CONST_PTR_TYPE;
...
typedef struct hif_shm_header_tag
{
	...
	struct
	{
		...
		/**
		 * When the HIF Partner (B) could not add new RQBs to your HIF (A) receive Queue, B sets A.bRingWasFull in SHM. 
		 */
		LSA_UINT8 		bRingWasFull;
		/**
		 * When HIF A sees, that its A.bRingWasFull is set in SHM, A sets B.bRingWasFullAck in SHM of B. 
		 * When HIF B sees, that its B.bRingWasFullAck is set, B can clear the A.bRingWasFull Flag in SHM of A.
		 * When HIF A sees, that its A.bRingWasFull is cleared again, it clears the B.bRingWasFullAck Flag in SHM of B.
		 */
		LSA_UINT8 		bRingWasFullAck;
	} tm_control;
...
typedef HIF_PIPE_SHM_RECEIVE_RING_TYPE   const* HIF_PIPE_SHM_RECEIVE_RING_CONST_PTR_TYPE;
...
typedef struct hif_pipe_tag
{
	...
	union 
    {
		...
		struct
        {
			...
			EDD_HANDLE_LOWER_TYPE           lower_orig_comp_edd_ch_handle;  //original handle of the target EDD component - only set on lower side
            #endif
		} lower;
	} d;
...
typedef struct hif_ch_handle_tag
{
	LSA_BOOL                bUsed;
	LSA_BOOL                bValid;
	LSA_HANDLE_TYPE         hHifCompCh;                     //new component handle send to the user (set by HIF: this handle is used as index in a HIF array)
...
/*------------------------------------------------------------------------------
// hif_buffer_admin_list
//----------------------------------------------------------------------------*/
typedef struct hif_buffer_admin_list
{
    LSA_VOID*               pBuffer;
    LSA_UINT16              compId;
    LSA_UINT16              memType;
    HIF_HD_ID_TYPE          hd_id;
    volatile LSA_UINT32*    pReadyForFree;
    LSA_UINT32              expectedMagicValue;

}HIF_BUFFER_AMDIN_LIST;

/*------------------------------------------------------------------------------
// hif_buffer_admin
//----------------------------------------------------------------------------*/
typedef struct hif_buffer_admin
{
    HIF_BUFFER_AMDIN_LIST buffer_list[HIF_BUFFER_ADMIN_LIST_ENTRIES];
    LSA_UINT32 buffer_list_next_entry;
    LSA_BOOL   bIsEmpty;

}HIF_BUFFER_ADMINISTRATION;
...
typedef struct hif_data_tag
{
    ...
    struct
    {
        LSA_BOOL         bUseInternalPoll;  // set to active if at least one HIF instance runs in polling mode
	    LSA_UINT16       internalPollCnt;   // internal counter 
        HIF_RQB_PTR_TYPE pInternalPoll;     // pointer to the trigger RQB
    } internal;
    ...
    HIF_BUFFER_ADMINISTRATION BufferAdministration;
...
#endif
#if (HIF_CFG_USE_EDD == 1)
EDD_HANDLE_LOWER_TYPE   hif_get_open_channel_lower_handle_edd(HIF_RQB_PTR_TYPE const pRQB);
#endif
...
/*-hif_dev.c------------------------------------------------------------------*/
LSA_VOID   hif_start_internal_poll();
...
LSA_VOID   hif_free_local_internal_rqb(HIF_RQB_PTR_TYPE const pRQB);
...
LSA_VOID   hif_add_to_rqb_queue_as_first(HIF_RQB_PTR_TYPE* rqb_queue, HIF_RQB_PTR_TYPE rqb, LSA_UINT16 PipeIndex);
LSA_UINT16 hif_add_to_rqb_queue(HIF_RQB_PTR_TYPE* rqb_queue, HIF_RQB_PTR_TYPE rqb, LSA_UINT16 PipeIndex);
LSA_VOID   hif_get_from_rqb_queue(HIF_RQB_PTR_TYPE* rqb_queue, HIF_RQB_PTR_TYPE* rqb, LSA_UINT16 *pPipeIndex);
...
LSA_BOOL   hif_shm_ring_buffer_send_enqueue(HIF_PIPE_SHM_SEND_RING_PTR_TYPE pSndRing, LSA_UINT32 uRqbOffset, LSA_UINT32 uRqb, LSA_UINT16 pipe_id);
...
/*-hif_ser_dns.c--------------------------------------------------------------*/
LSA_UINT16  hif_dns_serialize_upper (HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_INT hShmPool, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest, LSA_UINT32* pSerRqbLen);
LSA_UINT16  hif_dns_serialize_lower (HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_INT hShmPool, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest, LSA_UINT32* pSerRqbLen);
LSA_UINT16  hif_dns_deserialize_lower (HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest);
LSA_UINT16  hif_dns_deserialize_upper (HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest);
#endif
...
/*-hif_ser_eps.c--------------------------------------------------------------*/
LSA_UINT16  hif_eps_serialize_upper (HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_INT hShmPool, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest, LSA_UINT32* pSerRqbLen);
LSA_UINT16  hif_eps_serialize_lower (HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_INT hShmPool, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest, LSA_UINT32* pSerRqbLen);
LSA_UINT16  hif_eps_deserialize_lower (HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest);
LSA_UINT16  hif_eps_deserialize_upper (HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest);
#endif

#if ( (HIF_CFG_COMPILE_SERIALIZATION == 1) && (HIF_CFG_USE_HSA == 1) )
/*-hif_ser_hsa.c--------------------------------------------------------------*/
LSA_UINT16  hif_hsa_serialize_upper(HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_INT hShmPool, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest, LSA_UINT32* pSerRqbLen);
LSA_UINT16  hif_hsa_serialize_lower(HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_INT hShmPool, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest, LSA_UINT32* pSerRqbLen);
LSA_UINT16  hif_hsa_deserialize_lower(HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest);
LSA_UINT16  hif_hsa_deserialize_upper(HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest);
#endif

#if ( (HIF_CFG_COMPILE_SERIALIZATION == 1) && (HIF_CFG_USE_SOCKAPP == 1) )
/*-hif_ser_sockapp.c----------------------------------------------------------*/
LSA_UINT16  hif_sockapp_serialize_upper(HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_INT hShmPool, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest, LSA_UINT32* pSerRqbLen);
LSA_UINT16  hif_sockapp_serialize_lower(HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_INT hShmPool, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest, LSA_UINT32* pSerRqbLen);
LSA_UINT16  hif_sockapp_deserialize_lower(HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest);
LSA_UINT16  hif_sockapp_deserialize_upper(HIF_SYS_HANDLE hSysDev, LSA_VOID* pHifPipeHandle, LSA_VOID* pRqbSrc, LSA_VOID** ppRqbDest);
#endif
...
#define HIF_SER_COPY_HTSHM_64(dest, src)\
{\
	HIF_MEMCPY(*(dest),&(src),8);\
	*(dest) = *(dest)+2;\
}
#endif

/****************************************************************************
*  Name:        HIF_SER_COPY_HTSHM_ENUM()
*  Purpose:     - copies enum from host to shared-mem and takes care of swapping
*			    - increases help pointer + enum size
*  Input:       address of help pointer to destination, source parameter, enum type
*  Return:      -
****************************************************************************/
#define HIF_SER_COPY_HTSHM_ENUM(ppdest, src, enum_type)\
{\
    **(ppdest) = 0;\
    switch (sizeof(enum_type))\
    {\
        case 1: HIF_SER_COPY_HTSHM_8(ppdest,  src); break;\
        case 2: HIF_SER_COPY_HTSHM_16(ppdest, src); break;\
        case 4: HIF_SER_COPY_HTSHM_32(ppdest, src); break;\
        default: \
            HIF_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "HIF_SER_COPY_HTSHM_ENUM() - sizeof(enum_type) not valid (0x%x)", sizeof(enum_type)); \
            HIF_FATAL(0);\
    }\
}
...
/****************************************************************************
*  Name:        HIF_SER_COPY_SHMTH_ENUM()
*  Purpose:     - copies enum from shared-mem to host and takes care of swapping
*			    - increases help pointer + enum size
*  Input:       pointer to destination, address of help pointer to source, enum type
*  Return:      -
****************************************************************************/
#define HIF_SER_COPY_SHMTH_ENUM(pdest, ppsrc, enum_type)\
{\
    switch (sizeof(enum_type))\
    {\
        case 1: HIF_SER_COPY_SHMTH_8(pdest,  ppsrc); break;\
        case 2: HIF_SER_COPY_SHMTH_16(pdest, ppsrc); break;\
        case 4: HIF_SER_COPY_SHMTH_32(pdest, ppsrc); break;\
        default: \
            HIF_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "HIF_SER_COPY_SHMTH_ENUM() - sizeof(enum_type) not valid (0x%x)", sizeof(enum_type)); \
            HIF_FATAL(0);\
    }\
}
...
*  Return:      opcode
****************************************************************************/
#ifndef HIF_DESER_GET_COMP_ID_FROM_SHM

...
/**
 * \brief Returns a bit mask with the configuration of this HIF instance
 *
 * @param [in] pHifInst - current HIF instance
 * @return The calculated bitmask with the configuration
 */
...
/**
 * \brief maps LSA Return values to HIF return values
 *
 * @param [in] lsaResponseVal - LSA return value
...
/**
 * \brief Allocates an internal RQB and sets the OPC to Opcode
 *
 * @param [out] ppRQB   - pointer to the allocated RQB
 * @param [in] Opcode   - This function sets the OPC of the new RQB to this value
 * @return
 */
LSA_RESULT hif_alloc_local_internal_rqb(HIF_RQB_PTR_TYPE* ppRQB, LSA_OPCODE_TYPE Opcode)
{
    LSA_UINT16 const RqbSize = sizeof(HIF_RQB_TYPE);

    HIF_PROGRAM_TRACE_02(0, LSA_TRACE_LEVEL_CHAT, "hif_alloc_local_internal_rqb(+) -  ppRQB(0x%x), Opcode(0x%x)", ppRQB, Opcode);

    HIF_ASSERT(HIF_IS_NOT_NULL(ppRQB));

    HIF_ALLOC_LOCAL_MEM((LSA_VOID**)ppRQB, RqbSize, LSA_COMP_ID_HIF, HIF_MEM_TYPE_RQB);
    ...
    HIF_RQB_SET_COMP_ID(*ppRQB, LSA_COMP_ID_HIF);

    return LSA_RET_OK;
}

/**
 * @brief Frees an internal RQB
 * @param [in] pRQB - pointer to the RQB that is freed.
 */
LSA_VOID hif_free_local_internal_rqb(HIF_RQB_PTR_TYPE const pRQB)
{
    ...
    HIF_FREE_LOCAL_MEM(&retVal, pRQB, LSA_COMP_ID_HIF, HIF_MEM_TYPE_RQB);

    HIF_ASSERT(retVal == LSA_RET_OK);
}

/**
 * \brief Allocates a channel handle from the internal management structure
 *
 *
 * @param [in] pPipe            - current pipe
 * @param [out] ppHifChHandle   - channel handle
 * @return
 */
...
/**
 * \brief Releases a channel handle allocated by hif_alloc_ch_handle
 *
 * @param [in] pHifChHandle - handle to release
 * @return
 */
...
/**
 * \brief Allocates a RQB tag structure and sets the pipe within this tag.
 *
 * The RQB tag is stored in the original RQB user_id.
 *
 * @param [out] ppHifRbTag  - allocated RQB tag
 * @param [in] pPipe        - current pipe
...
LSA_VOID hif_alloc_rb_tag(HIF_RB_TAG_PTR_TYPE* ppHifRbTag, HIF_PIPE_PTR_TYPE pPipe, HIF_RQB_PTR_TYPE pRQB)
{
    ...
    HIF_ALLOC_LOCAL_MEM((LSA_VOID**) ppHifRbTag, sizeof(HIF_RB_TAG_TYPE), LSA_COMP_ID_HIF, HIF_MEM_TYPE_OTHER);

    if (HIF_IS_NULL(*ppHifRbTag)) /* Out of resources */
    {
        HIF_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "hif_alloc_rb_tag(-) ppHifRbTag(0x%x) is not valid", ppHifRbTag);
        HIF_FATAL(0);
    }
...
/**
 * \brief Frees a RQB tag that was allocated by hif_alloc_rb_tag.
 *
 * @param [in] pHifRbTag - RQB tag to be released.
 * @return
 */
LSA_VOID hif_free_rb_tag(HIF_RB_TAG_PTR_TYPE pHifRbTag)
{
    ...
    HIF_FREE_LOCAL_MEM(&retVal, pHifRbTag, LSA_COMP_ID_HIF, HIF_MEM_TYPE_OTHER);
    HIF_ASSERT(retVal == LSA_RET_OK);
}

/**
 * \brief Allocates a system RQB tag and sets the pipe within this tag.
 *
 * @param [out] ppHifSysRbTag   - pointer to the allocated system RQB tag.
 * @param [in] pPipe            - current pipe
 * @param [in] pRQB             - not used
 * @return
 */
LSA_VOID hif_alloc_sys_rb_tag(HIF_SYS_RB_TAG_PTR_TYPE* ppHifSysRbTag, HIF_PIPE_PTR_TYPE pPipe, HIF_RQB_PTR_TYPE pRQB)
{
    ...
    HIF_ALLOC_LOCAL_MEM((LSA_VOID**) ppHifSysRbTag, sizeof(HIF_SYS_RB_TAG_TYPE), LSA_COMP_ID_HIF, HIF_MEM_TYPE_OTHER);

    if (HIF_IS_NULL(*ppHifSysRbTag))
    {
        HIF_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "hif_alloc_sys_rb_tag(-) ppHifSysRbTag(0x%x) is not valid", ppHifSysRbTag);
        HIF_FATAL(0);
    }
...
/**
 * \brief Frees a RQB tag that was allocated by hif_alloc_sys_rb_tag.
 *
 * @param [in] pHifSysRbTag - RQB tag to be released.
 * @return
 */
LSA_VOID hif_free_sys_rb_tag(HIF_SYS_RB_TAG_PTR_TYPE pHifSysRbTag)
{
    ...
    HIF_FREE_LOCAL_MEM(&retVal, pHifSysRbTag, LSA_COMP_ID_HIF, HIF_MEM_TYPE_OTHER);
    HIF_ASSERT(retVal == LSA_RET_OK);
}

/**
 * \brief Allocates a new HIF instance from the internal management struture
 *
 * The allocated instance is preset with correct values. The values are calculated from the opcode, see function block.
 * - Sets the state of the instance to HIF_INST_STATE_OPENING.
 * - Sets the state of the admin pipe to HIF_PIPE_STATE_CLOSED.
 * - Sets REQUEST_DONE to HIF_XX_REQUEST_UPPER_DONE where XX = LD / HD
 *
 * @param [out] ppHifInst - the pointer to the HIF instance is stored here
 * @param [in] opcode     - one of these values: HIF_OPC_LD_UPPER_OPEN, HIF_OPC_LD_LOWER_OPEN, HIF_OPC_HD_UPPER_OPEN, HIF_OPC_HD_LOWER_OPEN
 * @return
 */
LSA_VOID hif_alloc_instance(HIF_INST_PTR_TYPE* ppHifInst, LSA_OPCODE_TYPE opcode)
{
    ...
    /* Estimate resource pool */
    if ((opcode == HIF_OPC_LD_UPPER_OPEN) || (opcode == HIF_OPC_LD_LOWER_OPEN))
    {
        /* LD Device */
        loop_start = 0;
...
/**
 * \brief Releases an instance by setting the state to HIF_INST_STATE_FREE.
 *
 * Note: Does not set the values in this instance to 0. The initialization is done in hif_alloc_instance.
 *
 *
 *
 * @param pHifInst
...
/**
 * \brief Returns the HIF instance by a HIF handle hH.
 *
 *
 * @param [out] ppHifInst - handle to the HIF instance
 * @param [in] hH         - HIF handle used as index.
 * @return
 */
...
/**
 * \brief Checks if any HIF instance is opened.
 *
 * The check is done by checking the state.
 *
 * @return LSA_TRUE - At least one HIF instance is opened.
 * @return LSA_FALSE - all HIF instances have the state HIF_INST_STATE_FREE
 */
...
/**
 * \brief Checks if the HIF instance is a upper instance or not.
 *
 * @param [in] pHifInst - handle to the HIF instance
 * @return LSA_TRUE     - This is a HIF Upper instance
 * @return LSA_FALSE    - This is a HIF Lower instance
 */
...
/**
 * \brief Checks if this instance is a LD or HD instance
 *
 * @param [in] pHifInst - handle to the HIF instance
 * @return LSA_TRUE     - This is a HIF LD instance
 * @return LSA_FALSE    - This is a HIF HD instance
 */
...
/**
 * \brief Internal checker function
 *
 * Checks HIF RQB Queues
 *
 * @param [in] rqb_queue   - first element of the local send queue
 * @param [in] rqb         - pointer to the RQB, stored in the host’s local memory
 * @return
 */
...
/**
 * \brief Adds the RQB to the queue as the head element.
 *
 * @param [inout] rqb_queue - first element of the local send queue
 * @param [in] rqb          - pointer to the RQB, stored in the host’s local memory
 * @param [in] PipeIndex    - The PipeIndex is encoded into comp_id.
 */
LSA_VOID hif_add_to_rqb_queue_as_first( HIF_RQB_PTR_TYPE  *rqb_queue,
                                        HIF_RQB_PTR_TYPE  rqb,
                                        LSA_UINT16        PipeIndex)
{
    ...
    if (ret_val)
    {
        HIF_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "hif_add_to_rqb_queue_as_first() - return value of hif_test_rqb_queue (0x%x) not valid", ret_val);
        HIF_FATAL(0);
    }
    ...
    // Byte   0     |   1
    //      comp id | PipeIndex
    HIF_RQB_SET_COMP_ID(rqb, (LSA_UINT16)(HIF_RQB_GET_COMP_ID(rqb) | (PipeIndex<<8)));
 ...
 * @param [in] rqb              - pointer to the RQB, stored in the host’s local memory
 * @param [in] PipeIndex        - The PipeIndex is encoded into comp_id.
 * @return 1 - first element in the queue
 * @return 0 - else
 */
                                 ...
                                 LSA_UINT16        PipeIndex )
{
    ...
    if (ret_val)
    {
        HIF_PROGRAM_TRACE_01(0, LSA_TRACE_LEVEL_FATAL, "hif_add_to_rqb_queue() - return value of hif_test_rqb_queue (0x%x) not valid", ret_val);
        HIF_FATAL(0);
    }
 ...
 * @param [inout] rqb_queue - first element of the local send queue
 * @param [in] rqb          - pointer to the RQB, stored in the host’s local memory
 * @param [out] pPipeIndex  - The PipeIndex is decoded from the comp_id.
 */
LSA_VOID hif_get_from_rqb_queue( HIF_RQB_PTR_TYPE  *rqb_queue,
                                 HIF_RQB_PTR_TYPE  *rqb,
 ...
 * \brief Returns the first RQB and removes it from the queue
 *
 * @param [inout] rqb_queue - first element of the queue
 * @param [in] rqb          - pointer to the RQB, stored in the host’s local memory.
 * @return 1 - first element in the queue
 * @return 0 - else
 */
 ...
 *
 * @param [in] rqbShm
 * @return rqb_id (original upper rqb-pointer)
 */

...
/* 11.11.14		Th 	  removed FirstSend from _EDDS_CSRT_PROVIDER_TYPE        */
#endif
/*****************************************************************************/
...
typedef struct _EDDS_CSRT_STAT_TYPE
{
    ...
    /* new benchmark and statistic counters are type LSA_UINT64 */

    LSA_UINT64		ProvCycleOutDiscards;	/* number of cycles, where not all provider could be send */
} EDDS_CSRT_STAT_TYPE;
...
#define EDDS_CSRT_PROVIDER_CR_LOCKED        1
#define EDDS_CSRT_PROVIDER_CR_UNLOCKED      0

#define EDDS_CSRT_PROVIDER_CR_USED          1
#define EDDS_CSRT_PROVIDER_CR_UNUSED        0
...
PNIO_PACKED_ATTRIBUTE_PRE struct _EDDS_CSRT_PROVIDER_CR_TYPE
{
    ...
    LSA_UINT8 IsLocked;              // locked by user
                                    // no special locking mechanism needed; calls to lock/unlock a provider
                                    // have to be from one context

    LSA_UINT8 CrUsed;                // is this Cr used or not
    LSA_UINT8 IsSystemRedundant;    // EDD_PROVADD_TYPE_DEFAULT for no system redundancy provider
                                    // EDD_PROVADD_TYPE_SYSRED for sytem redundancy provider

    LSA_UINT8 DataStatus;           // DataStatus Byte used for the system redundancy
                                    // this element is only needed localy in EDDS-IO-Buffer
...
#define EDDS_CSRT_CONSUMER_CR_LOCKED        1
#define EDDS_CSRT_CONSUMER_CR_UNLOCKED      0

#define EDDS_CSRT_CONSUMER_CR_USED          1
#define EDDS_CSRT_CONSUMER_CR_UNUSED        0
...
typedef struct _EDDS_CSRT_CONSUMER_FRAME_TYPE EDDS_LOCAL_MEM_ATTR const *   EDDS_CSRT_CONSUMER_FRAME_TYPE_PTR_TO_CONST;
...
typedef struct _EDDS_CSRT_CONSUMER_TYPE
{
    ...
    LSA_BOOL                    IsSystemRedundant; /* LSA_FALSE if the consumer is not a system redundant one. */
                                                   /* LSA_TRUE if the consumer is a system redundant one. */ 
    /* freelist                  */
} EDDS_CSRT_CONSUMER_TYPE;

typedef struct _EDDS_CSRT_CONSUMER_TYPE EDDS_LOCAL_MEM_ATTR *       EDDS_CSRT_CONSUMER_TYPE_PTR;
typedef struct _EDDS_CSRT_CONSUMER_TYPE EDDS_LOCAL_MEM_ATTR const * EDDS_CSRT_CONSUMER_TYPE_PTR_TO_CONST;
...
//default for no system redundant Primary und No Redundancy
#define EDDS_CSRT_CONS_STAT_DEFAULT         ( EDD_CSRT_DSTAT_BIT_STATE         | \
EDD_CSRT_DSTAT_BIT_STOP_RUN      | \
EDD_CSRT_DSTAT_BIT_STATION_FAILURE )

//default for system redundant Backup und No Redundancy
#define EDDS_CSRT_SRD_CONS_STAT_DEFAULT         ( EDD_CSRT_DSTAT_BIT_STOP_RUN  | \
EDD_CSRT_DSTAT_BIT_STATION_FAILURE )
...
{
    EDDS_SRT_LINK_BASE_TYPE     IndicationRequestQueue;      /* indication request queue */
    EDDS_SRT_LINK_BASE_TYPE     IndicationConfirmationQueue; /* indication confirmation queue */
    EDDS_INTERNAL_REQUEST_TYPE  ConsIndTriggerRQB;           /* internal consumer indication trigger RQB */
    LSA_BOOL                    IndicationTriggerPending;    /* indication trigger to RQB context pending */
...
typedef struct _EDDS_DDB_COMP_SRT_TYPE
{
    ...
    /* Configvalues -------------*/
    /* --------------------------*/
    EDDS_DPB_CSRT_TYPE                  Cfg;            /* from SRT-Init             */
    ...
    /* Internal CRT Modulo Factor for scheduler : unit in ms   */
    LSA_UINT16 schedulerModuloFactorIO;
...
typedef struct _EDDS_DDB_COMP_SRT_TYPE    EDDS_LOCAL_MEM_ATTR const *   EDDS_LOCAL_DDB_COMP_SRT_PTR_TO_CONST_TYPE;

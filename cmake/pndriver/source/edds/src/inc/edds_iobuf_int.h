...
typedef struct _EDDS_IOBUFFER_TYPE
{
    ...
    EDDS_CSRT_PROVIDER_CR_TYPE_PTR   pProvBufferTable;        // address in shared memory
    EDDS_CSRT_CONSUMER_CR_TYPE_PTR   pConsBufferTable;        // address in shared memory

    EDDS_CSRT_SRD_ARSET_TYPE_PTR     pPrimaryARIDTable;       // address in shared memory
...
#define EDDS_IOBUFFER_MEM_U16_PTR_IS_VALID(ptr)    ( (EDD_COMMON_MEM_U16_PTR_TYPE) 0 != (EDD_COMMON_MEM_U16_PTR_TYPE) (ptr) )
...
#define EDDS_IOBUFFER_VALIDATE(statement, err_code) {                                         \
    if(!statement)                                                                            \
    {                                                                                         \
        EDDS_IOBuffer_FatalError ( err_code, EDDS_MODULE_ID, __LINE__);                       \
    }                                                                                         \
}

#define EDDS_IOBUFFER_VALIDATE_PROV_IS_SYSTEM_REDUNDANT(IsSystemRedundant) {                                \
    if( ( EDD_PROVADD_TYPE_DEFAULT  != IsSystemRedundant ) &&                                 \
        ( EDD_PROVADD_TYPE_SYSRED   != IsSystemRedundant )                                    \
      )                                                                                                     \
    {                                                                                                       \
        EDDS_IOBuffer_FatalError( EDDS_IOBUF_FATAL_ERR_PROV_ALLOC_IS_SYSTEM_REDUNDANT, EDDS_MODULE_ID, __LINE__); \
    }                                                                                                  \
}

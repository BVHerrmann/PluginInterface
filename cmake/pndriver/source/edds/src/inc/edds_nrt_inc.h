...
#define EDDS_ARP_FILTER_INVALID_HDB_TABLE_INDEX          0xFFFFFFFF
#define EDDS_DCP_FILTER_INVALID_HDB_TABLE_INDEX          0xFFFFFFFF
...
#define EDDS_NRT_TX_PRIO_3_HSYNC_LOW    3  /* prio 3: used for HSYNC low      */
#define EDDS_NRT_TX_PRIO_4_HSYNC_HIGH   4  /* prio 4: used for HSYNC high     */
#define EDDS_NRT_TX_PRIO_5              5  /* prio 5: used for ORG -Prio      */

#define EDDS_NRT_TX_PRIO_CNT (EDDS_NRT_TX_PRIO_5+1)
...
#define EDDS_NRT_FILTER_IP_VRRP_IDX            18
#define EDDS_NRT_FILTER_IP_OTHER_IDX           19
#define EDDS_NRT_FILTER_RARP_IDX               20
#define EDDS_NRT_FILTER_XRT_DCP_HELLO_IDX      21
#define EDDS_NRT_FILTER_XRT_UDP_DCP_HELLO_IDX  22
#define EDDS_NRT_FILTER_LEN_STDBY_IDX          23
#define EDDS_NRT_FILTER_LEN_HSR_IDX            24
#define EDDS_NRT_FILTER_LEN_SINEC_FWL_IDX      25
#define EDDS_NRT_FILTER_HSYNC_IDX              26

#define EDDS_NRT_MAX_FILTER_IDX                26      /* we have currently 26 filters     */
...
#define EDDS_IS_CARP_MAC(SrcMAC)   ( ( (SrcMAC[0] == 0x00) && (SrcMAC[1] == 0x00) &&     \
                                       (SrcMAC[2] == 0x5E) && (SrcMAC[3] == 0x00) &&     \
                                       (SrcMAC[4] == 0x01) ) ? 1 : 0 )

#define EDDS_IS_HSR_MAC(SrcMAC)    ( ( (SrcMAC[0] == 0x00) && (SrcMAC[1] == 0x80) &&     \
                                       (SrcMAC[2] == 0x63) && (SrcMAC[3] == 0x07) &&     \
                                       (SrcMAC[4] == 0x10) && (SrcMAC[5] == 0x00) ) ? 1 : 0 )
...
typedef struct _EDDS_HDB_COMP_NRT_TYPE
{
    ...
    EDDS_NRT_LINK_BASE_TYPE  RxCancel;       /* Tx to be canceled queue               */
...
typedef struct _EDDS_NRT_RX_DSCR_TYPE    EDDS_LOCAL_MEM_ATTR const *   EDDS_NRT_DSCR_TYPE_PTR_TO_CONST;
...
typedef struct _EDDS_NRT_DCP_FILTER_MGM_TYPE
{
	...
	LSA_UINT32                  HDBIndex;       /* index of HDB in HDB table within DDB */
...
#define EDDS_MAX_ARP_FILTER                2 

typedef struct _EDDS_ARP_FRAME_FILTER_TYPE
{
    LSA_UINT8                   FilterCnt;                            /* Count of active Filters */
    LSA_UINT32                  HDBIndexTable[EDDS_MAX_ARP_FILTER];   /* table of HDBIndex */
    EDD_IP_ADR_TYPE             IPAddr[EDDS_MAX_ARP_FILTER];          /* Active IP adresses */
    EDD_IP_ADR_TYPE             IpAddrForLookup[EDDS_MAX_ARP_FILTER]; /* Sorted IP adresses for lookup */ 
...
typedef struct _EDDS_NRT_TX_MGM_TYPE
{
    ...
    LSA_UINT32                  TxByteCntMax;
    LSA_UINT32                  TxByteCntCurrent;
...
typedef struct _EDDS_DDB_COMP_NRT_TYPE
{
    ...
    LSA_UINT32                     TxPacketsQuotaMS; /* maximum of NRT Send packets per millisecond. */
    LSA_UINT32                     TxPacketsCycleQuota; /* calculated maximum Tx packets per cycle */
    LSA_UINT32                     actTxPacketsCycle;  /* tx packets send in actual cycle */
    
    EDDS_NRT_TX_MGM_TYPE           Tx[EDDS_NRT_TX_PRIO_CNT]; /* NRT Tx management for mult.  priorities. */
    ...
    LSA_UINT32 RxBufferCntARPIP[EDDS_DPB_PARAMETERSET_COUNT];
    LSA_UINT32 RxBufferCntASRTAlarm[EDDS_DPB_PARAMETERSET_COUNT];
    LSA_UINT32 RxBufferCntASRTOther[EDDS_DPB_PARAMETERSET_COUNT];
    LSA_UINT32 RxBufferCntOther[EDDS_DPB_PARAMETERSET_COUNT];
    
    LSA_BOOL                       RxFilterUDP_Unicast;
    LSA_BOOL                       RxFilterUDP_Broadcast;
    
    LSA_UINT8                      CARPMacCnt;

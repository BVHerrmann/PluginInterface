...
/* OBSD_TRACE_PATCH_START - patch no. (obsd_kernel_kern_tc_act_module) */
#define LTRC_ACT_MODUL_ID 4004 /* OBSD_ITGR added for LSA traces */
/* OBSD_TRACE_PATCH_END - patch no. (obsd_kernel_kern_tc_act_module) */
...
static u_int obsd_kernel_get_timecount(struct timecounter *tc)
{
	OBSD_UNUSED_ARG(tc);
    return cticks;
}

static struct timecounter obsd_kernel_timecounter = {
	obsd_kernel_get_timecount, 0, 0xffffffff, HAL_CLOCK_RATE_HZ, "obsd_kernel_tc", 0
};
...
struct timecounter *timecounter = &obsd_kernel_timecounter;
static struct timecounter *timecounters = &obsd_kernel_timecounter;
...
tc_init(struct timecounter *tc)
{
	...
	u = (u_int)(tc->tc_frequency / tc->tc_counter_mask); /* gerlach/gh2289n: added explicit type cast to avoid compiler warning C4244 */
	/* XXX: We need some margin here, 10% is a guess */
	u *= 11;
	u /= 10;
	if (tc->tc_quality >= 0) {
		if (u > (u_int)hz) { /* gerlach/gh2289n: added explicit type cast to avoid compiler warning C4018: '>' : signed/unsigned mismatch */
			tc->tc_quality = -2000;
/* OBSD_TRACE_PATCH_START - patch no. (obsd_kernel_kern_tc_001) */
			TCIP_OBSD_TRACE_01(0, OBSD_TRACE_LEVEL_ERROR, "Timecounter: Frequency %lu Hz", (unsigned long)tc->tc_frequency);
			TCIP_OBSD_TRACE_01(0, OBSD_TRACE_LEVEL_ERROR, "Insufficient Hz, needs at least %u", u);
/* OBSD_TRACE_PATCH_END - patch no. (obsd_kernel_kern_tc_001) */
...
/* OBSD_TRACE_PATCH_START - patch no. (obsd_kernel_kern_tc_002) */
		TCIP_OBSD_TRACE_04(0, OBSD_TRACE_LEVEL_CHAT, "Time stepped from %ld.%09ld to %ld.%09ld",
		    (long)ts2.tv_sec, ts2.tv_nsec,
		    (long)ts->tv_sec, ts->tv_nsec);
/* OBSD_TRACE_PATCH_END - patch no. (obsd_kernel_kern_tc_002) */
...
sysctl_tc_choice(void *oldp, size_t *oldlenp, void *newp, size_t newlen)
{
	...
	for (tc = timecounters; tc != NULL; tc = tc->tc_next)
		maxlen += sizeof(buf);
	if((choices = malloc(maxlen, M_TEMP, M_NOWAIT)) == NULL)
		return ENOMEM;
	*choices = '\0';
	for (tc = timecounters; tc != NULL; tc = tc->tc_next) {
...
ntp_update_second(int64_t *adjust, time_t *sec)
{
	...
	OBSD_UNUSED_ARG(sec); /* gerlach/gh2289n: avoid compiler warning C4100: unreferenced formal parameter */

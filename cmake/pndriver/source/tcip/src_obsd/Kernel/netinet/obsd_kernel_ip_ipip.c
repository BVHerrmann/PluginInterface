...
ipip_input(struct mbuf *m, int iphlen, struct ifnet *gifp, int proto)
{
	...
	u_int8_t itos;
#endif
	int isr = 0; /* gerlach/gh2289n: added init value to avoid compiler warning C4701: potentially uninitialized local variable used */
	unsigned int hlen; /* gerlach/gh2289n: changed type from int to unsigned int to avoid compiler warning C4018 */
	u_int8_t otos;
	u_int8_t v;
#if NBPFILTER > 0 /* sado -- Greenhills compiler warning 550 - set, but never used */
	sa_family_t af;
#endif

	OBSD_UNUSED_ARG(gifp); /* gerlach/gh2289n: avoid compiler warning C4100: unreferenced formal parameter */

	ipipstat.ipips_ipackets++;

	m_copydata(m, 0, 1, (caddr_t)(&v)); /* gerlach/gh2289n: added explicit type cast to avoid compiler warning C4133 */
    ...
    int dummy2)
{
	...
	OBSD_UNUSED_ARG(dummy); 
	OBSD_UNUSED_ARG(dummy2); /* gerlach/gh2289n: avoid compiler warning C4100: unreferenced formal parameter */

	/* XXX Deal with empty TDB source/destination addresses. */

	m_copydata(m, 0, 1, (caddr_t)(&tp));  /* gerlach/gh2289n: added explicit type cast to avoid compiler warning C4133 */
	...
	case AF_INET:
		...
		CAL_SET_HIGH_NIBBLE_OF_BYTE(ipo->ip_v_ip_hl, IPVERSION);   /* high nibble = version    */
		CAL_SET_LOW_NIBBLE_OF_BYTE( ipo->ip_v_ip_hl, 5);           /* low nibble  = header len */
		ipo->ip_len = htons(m->m_pkthdr.len);
		ipo->ip_ttl = (u_int8_t)ip_defttl; /* gerlach/gh2289n: added explicit type cast to avoid compiler warning C4244 */
		ipo->ip_sum = 0;
		 ...
		 * endianness.
		 */
		ipo->ip_id = ip_randomid();		/* mh2290: LINT 666   Expression with side effects passed to repeated parameter Integer of macro */

...
{
	size_t		 n=0, count, end, ptr, start;  /* gh2289n: added init value for n */
	ssize_t		 res;
	...
	for (; (n = data[offset]) != 0; ) {  /* gh2289n: added comparison to 0 to avoid compiler warning */
		if ((n & 0xc0) == 0xc0) {
			if (offset + 2 > len)
				return (-1);
		...
		if (dname_check_label((char *)data + offset + 1, n) == -1)
			return (-1);
...
unpack_dname(struct asr_unpack *p, char *dst, size_t max)
{
	...
	e = dname_expand((unsigned char *)p->buf, p->len, p->offset, &p->offset, dst, max);
	if (e == -1) {
...
asr_sockaddr_from_str(struct sockaddr *sa, int family, const char *str)
{
	...
	case PF_INET6:
		...
		if (IN6_IS_ADDR_LINKLOCAL(&in6a) ||
			...
			if (((sin6->sin6_scope_id = if_nametoindex(cp))) != 0) /* gh2289n: added comparison to 0 to  to avoid compiler warning */
				return (0);

		sin6->sin6_scope_id = (u_int32_t)strtonum(cp, 0, UINT32_MAX, &errstr);
		if (errstr)
			return (-1);
		return (0);

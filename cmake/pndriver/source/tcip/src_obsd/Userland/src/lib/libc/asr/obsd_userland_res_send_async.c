...
#include <sys/obsd_kernel_poll.h> /* OBSD_ITGR -- no extra file for one include */
#include <obsd_userland_resolv.h> /* for res_random */
...
sockaddr_connect(const struct sockaddr *sa, int socktype)
{
	...
	if ((sock = socket(sa->sa_family, socktype, 0)) == -1)
	 ...
	 obsd_pnio_set_ip_portrange_high(sock);
	...
	int class, int type)
{
	...
	memset(&h, 0, sizeof h);
	h.id = (uint16_t)res_randomid();
	if (as->as_ctx->ac_options & RES_RECURSE)
		h.flags |= RD_MASK;
	h.qdcount = 1;

	asr_pack_init(&p, (char *)as->as.dns.obuf, as->as.dns.obufsize);
	asr_pack_header(&p, &h);
	asr_pack_query(&p, (uint16_t)type, (uint16_t)class, dname);
	if (p.err) {
...
validate_packet(struct asr_query *as)
{
	...
	asr_unpack_init(&p, (char *)as->as.dns.ibuf, as->as.dns.ibuflen);

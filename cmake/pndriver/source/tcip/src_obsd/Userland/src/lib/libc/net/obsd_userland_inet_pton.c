	...
	*(tp = tmp) = 0;
	while ((ch = *src++) != '\0') {
		...
		if ((pch = strchr(digits, ch)) != NULL) {
			u_int new = (u_int)(*tp * 10 + (pch - digits));  /* gerlach/gh2289n: added explicit type cast to avoid compiler warning C4244 */
			...
			*tp = (u_char)new; /* gerlach/gh2289n: added explicit type cast to avoid compiler warning C4244 */
...
inet_pton6(const char *src, u_char *dst)
{
	...
	if (colonp != NULL) {
		 ...
		 */
		const int n = (int)(tp - colonp);  /* gerlach/gh2289n: added explicit type cast to avoid compiler warning C4244 */
		int i;

...
#include <eps_tasks.h>         /* eps_tasks_sleep()                 */
...
static LSA_VOID   eps_noshmdrv_uninstall              (LSA_VOID);
static LSA_UINT16 eps_noshmdrv_app_ready              (EPS_SHM_HW_PTR_TYPE pHwInstIn, LSA_VOID* pHifHandle);
static LSA_UINT16 eps_noshmdrv_app_shutdown           (EPS_SHM_HW_PTR_TYPE pHwInstIn);
static LSA_UINT16 eps_noshmdrv_sig_fatal              (EPS_SHM_HW_CONST_PTR_TYPE pHwInstIn, EPS_SHM_FATAL_PAR_TYPE sFatal);

/* - Global Data ---------------------------------------------------------------------------------- */

static EPS_NOSHMDRV_STORE_TYPE g_EpsNoShmDrv;
static EPS_NOSHMDRV_STORE_PTR_TYPE g_pEpsNoShmDrv;
...
{
    LSA_UINT16 retVal;
    retVal = eps_free_critical_section(g_pEpsNoShmDrv->hEnterExit);
...
{
    LSA_UINT16 retVal;
    retVal = eps_alloc_critical_section(&g_pEpsNoShmDrv->hEnterExit, LSA_FALSE);
    EPS_ASSERT( LSA_RET_OK == retVal );
}
 ...
 * @return LSA_VOID
 */
static LSA_VOID eps_noshmdrv_open_hif_driver(EPS_SHM_HW_CONST_PTR_TYPE pHwInstOut)
{
	LSA_UINT16 retVal;
 ...
 * @return LSA_VOID
 */
static LSA_VOID eps_noshmdrv_close_hif_driver(EPS_SHM_HW_CONST_PTR_TYPE pHwInstOut)
{
	LSA_UINT16 retVal;
 ...
 * @return LSA_VOID
 */
static LSA_VOID eps_noshmdrv_trace_buffer_full_cbf(LSA_VOID_PTR_TYPE hTrace, LSA_UINT32 hBuffer, LSA_UINT8* pBuffer, LSA_UINT32 uLength)
{
 ...
 */
static LSA_UINT16 eps_noshmdrv_update_device_list(LSA_VOID)
{
    //Do nothing
    return EPS_SHM_RET_OK;
}
...
LSA_UINT16 eps_noshmdrv_open (LSA_VOID_PTR_TYPE hSys, EPS_SHM_OPEN_OPTION_CONST_PTR_TYPE pOption, EPS_SHM_HW_PTR_PTR_TYPE ppHwInstOut)
{
	...
	if(pOption != LSA_NULL)
	{
        ...
        EPS_SYSTEM_TRACE_02(0, LSA_TRACE_LEVEL_NOTE_HIGH, "eps_noshmdrv_open() pOption->uHdNr (%d) pOption->bEnableWatchdog (%d)", pOption->uHdNr, pOption->bEnableWatchdog);
	}
...
static LSA_UINT16 eps_noshmdrv_sig_fatal( EPS_SHM_HW_CONST_PTR_TYPE pHwInstIn, EPS_SHM_FATAL_PAR_TYPE sFatal )
{
	...
	if (g_pEpsData->bBlockingPermitted == LSA_TRUE)
	{
        ...
        switch (sFatal.exit_code_org)
        {
            ...
            {
			    printf("!!!EPS FATAL!!! => File(%s) Function(%s) Line(%u) Msg(%s)", sFatal.pFile, sFatal.pFunc, sFatal.uLine, sFatal.pMsg);
            }
        }

...
#if !defined( PSI_CFG_EDDI_CFG_PHY_BLINK_EXTERNAL_NEC )  // must be set for EB200 PnBoards (see pnboards_psi_cfg_plf_eps.h)
#error ("config error PSI_CFG_EDDI_CFG_PHY_BLINK_EXTERNAL_NEC must be set for EB200-PNBoards")
#endif
...
// Partitioning of the DEV and NRT memory for ERTEC200, all values in percent
#define CP_MEM_ERTEC200_PERCENT_DEV                 0
#define CP_MEM_ERTEC200_PERCENT_NRT_TX              25
#define CP_MEM_ERTEC200_PERCENT_NRT_RX              70
#define CP_MEM_ERTEC200_PERCENT_NRT_DMACW_DEFAULT   5

#define CP_MEM_ERTEC200_PERCENT_TOTAL (CP_MEM_ERTEC200_PERCENT_DEV + CP_MEM_ERTEC200_PERCENT_NRT_TX + CP_MEM_ERTEC200_PERCENT_NRT_RX + CP_MEM_ERTEC200_PERCENT_NRT_DMACW_DEFAULT)

#if (CP_MEM_ERTEC200_PERCENT_TOTAL != 100)
...
static EPS_ERTEC200_HW_STORE_TYPE eps_ertec200_hw_store[PSI_CFG_MAX_IF_CNT+1];
...
static LSA_UINT32 eps_get_LED_mask( LSA_UINT16 gpio_nr )
{
	...
	{
		EPS_ASSERT( gpio_nr < 64 ); // in range ?

		result = ( gpio_nr < 32 ) ? (((LSA_UINT32) 1) << gpio_nr) : ( ((LSA_UINT32) 1) << (gpio_nr - 32) );
	}
...
LSA_VOID eps_ertec200_FillDevNrtMemParams( LSA_VOID * const pBoardInfoVoid )
{
	EPS_BOARD_INFO_TYPE	* const pboardInfo           = (EPS_BOARD_INFO_TYPE *)pBoardInfoVoid;
    LSA_UINT32            const dev_nrt_mem_size_all = pboardInfo->eddi.sdram.size;

    // setup DEV and NRT memory pools

    pboardInfo->dev_mem.base_ptr    = pboardInfo->eddi.sdram.base_ptr;
    pboardInfo->dev_mem.phy_addr    = pboardInfo->eddi.sdram.phy_addr;
    pboardInfo->dev_mem.size        = EPS_CALC_MEM_SIZE(dev_nrt_mem_size_all, CP_MEM_ERTEC200_PERCENT_DEV);
    ...
    pboardInfo->nrt_tx_mem.size     = EPS_CALC_MEM_SIZE(dev_nrt_mem_size_all, CP_MEM_ERTEC200_PERCENT_NRT_TX);
    ...
    pboardInfo->nrt_rx_mem.size     = EPS_CALC_MEM_SIZE(dev_nrt_mem_size_all, CP_MEM_ERTEC200_PERCENT_NRT_RX);

	EPS_ASSERT(pboardInfo->dev_mem.base_ptr != LSA_NULL);

    pboardInfo->eddi.fragmentation_supported              = LSA_FALSE;

    pboardInfo->eddi.nrt_mem_dmacw_default.base_ptr       = pboardInfo->nrt_rx_mem.base_ptr + pboardInfo->nrt_rx_mem.size;
    pboardInfo->eddi.nrt_mem_dmacw_default.phy_addr       = pboardInfo->nrt_rx_mem.phy_addr + pboardInfo->nrt_rx_mem.size;
    pboardInfo->eddi.nrt_mem_dmacw_default.size           = EPS_CALC_MEM_SIZE(dev_nrt_mem_size_all, CP_MEM_ERTEC200_PERCENT_NRT_DMACW_DEFAULT);
    ...
    pboardInfo->eddi.user_mem_fcts_tx_low_frag.Alloc      = eps_cp_mem_eddi_nrt_fatal_alloc;
    pboardInfo->eddi.user_mem_fcts_tx_low_frag.Free       = eps_cp_mem_eddi_nrt_fatal_free;
    ...
    pboardInfo->eddi.user_mem_fcts_tx_mid_frag.Alloc      = eps_cp_mem_eddi_nrt_fatal_alloc;
    pboardInfo->eddi.user_mem_fcts_tx_mid_frag.Free       = eps_cp_mem_eddi_nrt_fatal_free;
    ...
    pboardInfo->eddi.user_mem_fcts_tx_high_frag.Alloc     = eps_cp_mem_eddi_nrt_fatal_alloc;
    pboardInfo->eddi.user_mem_fcts_tx_high_frag.Free      = eps_cp_mem_eddi_nrt_fatal_free;
    ...
    pboardInfo->eddi.user_mem_fcts_tx_mgmtlow_frag.Alloc  = eps_cp_mem_eddi_nrt_fatal_alloc;
    pboardInfo->eddi.user_mem_fcts_tx_mgmtlow_frag.Free   = eps_cp_mem_eddi_nrt_fatal_free;
    ...
    pboardInfo->eddi.user_mem_fcts_tx_mgmthigh_frag.Alloc = eps_cp_mem_eddi_nrt_fatal_alloc;
    pboardInfo->eddi.user_mem_fcts_tx_mgmthigh_frag.Free  = eps_cp_mem_eddi_nrt_fatal_free;
}
...
LSA_VOID eps_pof_set_led_EB200( LSA_UINT16 portID, LSA_BOOL on, LSA_SYS_PTR_TYPE sysPtr )
{
    ...
    // get needed GPIO register
    pGPIO_IOCTRL2 = (LSA_UINT32*)(pGPIO + ERTEC200_x0002520_GPIO2_IOCTRL);
    pGPIO_OUT2 = (LSA_UINT32*)(pGPIO + ERTEC200_x0002524_GPIO2_OUT);

	if ( !eps_ertec200_hw_store[pSys->hd_nr].pof_led_init_done ) // POF LED initialized ?
	{
        hw_port_id = pBoard->port_map[1].hw_port_id;

        if (pBoard->port_media_type_by_HwPortId[hw_port_id] == PSI_MEDIA_TYPE_FIBER) //POF port?
		{
        ...
        hw_port_id = pBoard->port_map[2].hw_port_id;
    ...
    if (pBoard->port_media_type_by_HwPortId[hw_port_id] == PSI_MEDIA_TYPE_FIBER) //POF port?
	{
		switch( pBoard->port_map[portID].hw_port_id ) // GPIO setting is based on HW port ID mapping
		{
		    case 1: // POF1_LED
			{
				if ( on )
...
LSA_VOID eps_i2c_set_scl_low_highz_EB200( LSA_SYS_PTR_TYPE sysPtr, LSA_BOOL Level )
{
	LSA_UINT8  *pGPIO        = LSA_NULL;
	LSA_UINT32 *pGPIO_IOCTRL = LSA_NULL;
...
/*----------------------------------------------------------------------------*/
LSA_VOID eps_i2c_set_sda_low_highz_EB200( LSA_SYS_PTR_TYPE sysPtr, LSA_BOOL level )
{
	LSA_UINT8  *pGPIO        = LSA_NULL;
	LSA_UINT32 *pGPIO_IOCTRL = LSA_NULL;
...
LSA_BOOL eps_i2c_select_EB200( LSA_SYS_PTR_TYPE sysPtr, LSA_UINT16 PortId, LSA_UINT16 I2CMuxSelect )
{
	...
	switch ( PortId ) // User port ID (from 1..N)
	{
	    ...
	    case 2:
		{
			LSA_UINT32 hw_port_id = pBoard->port_map[PortId].hw_port_id;

...
#define EPS_CFG_HIF_DRV_IF_MAX_DEVICES    (PSI_CFG_MAX_IF_CNT*2+2)
...
/* Global definition */
static EPS_HIF_DRV_IF_STORE_TYPE g_EpsHifDrvIfStore;
static EPS_HIF_DRV_IF_STORE_PTR_TYPE g_pEpsHifDrvIfStore = LSA_NULL;
 ...
 */
static LSA_BOOL eps_hif_drv_config_fits_perfect(EPS_HIF_DRV_CFG_CONST_PTR_TYPE pCfg1, EPS_HIF_DRV_CFG_CONST_PTR_TYPE pCfg2)
{
 ...
 */
LSA_VOID eps_hif_drv_if_register(EPS_HIF_DRV_IF_CONST_PTR_TYPE pHifDrvIf, EPS_HIF_DRV_CFG_CONST_PTR_TYPE pConfig)
{
	LSA_UINT32 i;
	...
	EPS_ASSERT(pHifDrvIf->DmaCpy        != LSA_NULL);
    EPS_ASSERT(pHifDrvIf->DmaPendFree   != LSA_NULL);
 ...
 */
static LSA_UINT16 eps_hif_drv_if_open_intern (LSA_UINT16 hd_id, LSA_BOOL bUpper, EPS_HIF_DRV_CFG_CONST_PTR_TYPE pCfg, EPS_HIF_DRV_OPENPAR_CONST_PTR_TYPE pInPars, EPS_HIF_DRV_HW_PTR_PTR_TYPE ppHwInstOut)
{
	LSA_UINT32 i;
    ...
    for(i=0; i<EPS_CFG_HIF_DRV_IF_MAX; i++)
    {
        ...
        if(g_pEpsHifDrvIfStore->sInterface[i].bUsed == LSA_TRUE)
		{
            ...
            // Note: pIf is always != LSA_NULL since g_EpsHifDrvIf is a structure.
 ...
 *
 * @see PSI_HIF_MEMCPY_BUF_DMA - calls this function
 * @param [in] hd_id           - 0 = LD, 1...4 = HD
 * @param [in] bUpper          - 1 = Upper, 0 = Lower
 * @param [in] pDst            - pointer to the destination
 * @param [in] pSrc            - pointer to the data to be copied
...
LSA_VOID eps_hif_drv_if_dma_copy (LSA_UINT16 hd_id, LSA_BOOL bUpper, LSA_VOID* pDst, LSA_VOID* pSrc, LSA_UINT32 uLength)
{
	...
	pIf->DmaCpy(g_pEpsHifDrvIfStore->sHw[DeviceIndex].pHw, pDst, pSrc, uLength);
}
 ...
 *
 * @see PSI_HIF_DMA_TRANSFERS_FINISHED  - calls this function
 * @param [in] hd_id                    - 0 = LD, 1...4 = HD
 * @param [in] bUpper                   - 1 = Upper, 0 = Lower
 * @param [in] pBuffer                  - pointer to the buffer to be freed
 * @param [out] pReadyForFree           - pointer to where dma writes HIF_DMA_FREE_BUF_FLAG, 
...
LSA_BOOL eps_hif_drv_if_dma_pend_free (LSA_UINT16 hd_id, LSA_BOOL bUpper, LSA_VOID* pBuffer, LSA_VOID_PTR_TYPE *ppReadyForFree)
{
    ...
    return pIf->DmaPendFree(g_pEpsHifDrvIfStore->sHw[DeviceIndex].pHw, pBuffer, ppReadyForFree);
}
...
LSA_VOID eps_hif_drv_if_check_reenable_isr(LSA_VOID)
{
	...
	for(i=0; i<EPS_CFG_HIF_DRV_IF_MAX_DEVICES; i++)
	{
		if(g_pEpsHifDrvIfStore->sHw[i].pHw != LSA_NULL)
		{
            ...
            retVal = pIf->ReenableIsr(g_pEpsHifDrvIfStore->sHw[i].pHw);

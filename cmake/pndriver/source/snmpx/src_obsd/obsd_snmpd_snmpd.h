...
#include <obsd_kernel_mbuf.h>  /* gh2289h: pfvar.h and route.h uses struct mbuf* in function      */
                               /*          declarations, could lead to compiler warning           */
                               /*          "'struct mbuf' declared inside parameter list" (GCC?)  */
...
#include "obsd_snmpd_ber.h"
#define    BSD_STACKPORT_DISABLE_SNMPV3 1  /* gerlach/gh2289n: no SNMPv3 so far */
#include "obsd_snmpd_snmp.h"

#ifdef OBSD_SNMP_AGENTX
#include <obsd_userland_imsg.h>
#else
struct imsg { int dummy; };    /* gh2289n: we need a dummy for the struct's imsg *. get compiler  */
                               /*          warnings in VS2010 else                                */
#endif
...
#define READ_BUF_SIZE		1400 /* OBSD_ITGR: SOCK_SNMP_BUFFER_SIZE, use a literal 1400 because it's used in the same way over the whole PN system */
		                     /* (SNMPD can never recide packets larger than 1400 bytes because that is limited by the PN system anywhere        */
...
#ifdef    OBSD_SNMPD_DEFINE_PRIVSEP_PROCESS /* gh2289n: avoid multiple instances of this variable */
enum privsep_procid privsep_process;
#endif /* OBSD_SNMPD_DEFINE_PRIVSEP_PROCESS */
...
struct privsep {
	...
	void			*ps_env;
                            /*                  (which is a single process snmpd variant)         */
	struct privsep_proc	 *p; /* gerlach/gh2289n: need this for calling the right shutdown function*/
	                         /* not really needed for the STACK_TEST_VARIANT because we don't     */
	                         /* shutdown via a synchron function call there, but save the         */
	                         /* if !STACK_TEST_VARIANT here (don't use a '#' before if here in    */
	                         /* the comment to make OBSD_ITGR's script happy ;-)                  */
	struct event_base	*sc_evbase; /* gerlach/gh2289n: need to store eventbase for deinit        */
...
#if       SUPPORT_OHA_EXTERNAL_MIBS     /* need Identifier for externaly maintained MIBs          */
#define OID_EXTERNAL_OHA_MIB    0x8000  /* don't know the MIB tree, only the highest level OID    */
                                        /* for the MIB                                            */
...
#ifdef IPv6_Genua
struct kif_addr *obsd_snmpd_kr_getprefix(uint8_t, struct sockaddr *, uint8_t);
struct kif_addr *obsd_snmpd_kr_getnextprefix(uint8_t, struct sockaddr *,
		     uint8_t);

struct kroute	*obsd_snmpd_kr_getdefaultrouter(struct sockaddr_in *);
struct kroute	*obsd_snmpd_kr_getnextdefaultrouter(struct sockaddr_in *);
int		 obsd_snmpd_cmp_prefix6(struct sockaddr_in6 *,
		     struct sockaddr_in6 *, struct sockaddr_in6 *);

int		 obsd_snmpd_kr_getdefaultrouter6(struct in6_addr *,
		     struct in6_defrouter *);
int		 obsd_snmpd_kr_getnextdefaultrouter6(struct in6_addr *,
		     struct in6_defrouter *);
u_int8_t	 obsd_snmpd_mask2prefixlen(in_addr_t);
u_int8_t	 obsd_snmpd_mask2prefixlen6(struct sockaddr_in6 *);
#endif /*IPv6_Genua*/
...
struct kroute	*kroute_get_by_prefix(in_addr_t, int require_exact_match, int);
...
/* mps.c */
#define OBSD_SNMPD_MPS_ERRCODE_OK            ( 0) /* reserved and widely used in original SNMPD code as literal 0    */
#define OBSD_SNMPD_MPS_ERRCODE_GENERIC_ERROR (-1) /* reserved and widely used in original SNMPD code as literal -1   */
#define OBSD_SNMPD_MPS_ERRCODE_READ_ONLY     (-2)
#define OBSD_SNMPD_MPS_ERRCODE_WRONG_LENGTH  (-3) /* e.g. for values to set that are too long (e.g. DISPLAY_STRINGS) */
#define OBSD_SNMPD_MPS_ERRCODE_BADVALUE		 (-4)
#define OBSD_SNMPD_MPS_ERRCODE_NOSUCHNAME	 (-5)
...
#ifdef IPv6_Genua
int		 obsd_snmpd_mps_decodeinaddr(struct ber_oid *, struct in_addr *, int);
void		 obsd_snmpd_mps_encodeinaddr6(struct ber_oid *,
		     struct in6_addr *, int);
int		 obsd_snmpd_mps_decodeinaddr6(struct ber_oid *,
		     struct in6_addr *, int);
void		 obsd_snmpd_mps_encodeinprefix(struct ber_oid *,
		     struct in_addr *, int, uint8_t);
int		 obsd_snmpd_mps_decodeinprefix(struct ber_oid *,
		     struct in_addr *, int);
void		 obsd_snmpd_mps_encodeinprefix6(struct ber_oid *,
		     struct in6_addr *, int, uint8_t);
int		 obsd_snmpd_mps_decodeinprefix6(struct ber_oid *,
		     struct in6_addr *, int);
#endif /*IPv6_Genua*/
...
#include "obsd_snmpd_snmpdext.h"   /* own external interface and config constants                 */

#define log_info(msg, ...)
#define log_init(debug_level)
#define log_debug(msg, ...)
#define log_warn(msg, ...)
#define log_warnx(msg, ...)
#define log_verbose(v)
#define fatalx(msg) fatal(msg)
#define fatal(msg) exit(1)         /* don't use _exit(1) here to avoid destruction of our proc,     */
                                   /* exit(1) returns only (to the caller of obsd_execv(_ex)        */

...
#include <obsd_userland_err.h>	/* XXX for debug output */
#include <obsd_userland_stdio.h>	/* XXX for debug output */
#include <obsd_userland_string.h> /* gh2289n: from strings.h  */
#include <obsd_userland_unistd.h>
#include <sys/obsd_kernel_stdarg.h> /* gh2289n: missing in original code, but they have var args  */
                                    /*          in ber_printf_elements and ber_scanf_elements     */
...
#define MAX_SEQUENCE_RECURSIONS  (128)   /* count of recursions in BER sequences and type sets (sequence in sequence)*/
...
{
	if (elm == NULL || n == NULL) /* sado -- Task 2402776 */
		return -1;
...
{
	if (elm == NULL || b == NULL) /* sado -- Task 2402776 */
		return -1;
...
{
	if (elm == NULL || s == NULL) /* sado -- Task 2402776 */
		return -1;
...
{
	if (elm == NULL || p == NULL || len == NULL) /* sado -- Task 2402776 */
		return -1;
...
{
	if (elm == NULL || v == NULL || len == NULL) /* sado -- Task 2402776 */
		return -1;
...
ber_oid2ber(struct ber_oid *o, u_int8_t *buf, size_t len)
{
	...
	for (i = 2, j = 0; i <= o->bo_n; v = o->bo_id[i], i++) {
		for (k = 28; k >= 7; k -= 7) {
			if (v >= (u_int)(1 << k)) {
				if (len)
					buf[j] = (u_int8_t)(v >> k | BER_TAG_MORE); /* OBSD_ITGR -- omit warning */
				j++;
			}
		}
		if (len)
...
ber_string2oid(const char *oidstr, struct ber_oid *o)
{
	...
	for (p = sp = str; p != NULL; sp = p) {
		if ((p = strpbrk(p, "._-")) != NULL)
			*p++ = '\0';
		o->bo_id[o->bo_n++] = (u_int32_t) strtonum(sp, 0, UINT_MAX, &errstr); /* OBSD_ITGR -- omit warnings */
...
ber_get_oid(struct ber_element *elm, struct ber_oid *o)
{
	...
	if (elm == NULL || o == NULL) /* sado -- Task 2402776 */
		return -1;
...
#define _MAX_SEQ		 128
	...
	while (*fmt) {
		...
		case 'd':
			...
			*d = (int)l; /* sado - omit warning */
			ret++;
			break;
		case 'e':
			e = va_arg(ap, struct ber_element **);
			if (e != NULL) /* sado -- Task 2402776 */
			{
				*e = ber;
			}
			else
		...
		case 't':
			...
			if (d != NULL && t != NULL) /* sado -- Task 2402776 */
			{
				*d = ber->be_class;
				*t = ber->be_type;
			}
...
ber_dump_header(struct ber *ber, struct ber_element *root)
{
	...
	/* length encoding */
	if (root->be_type <= BER_TYPE_SINGLE_MAX) {
		id = (u_char)(root->be_type | (root->be_class << BER_CLASS_SHIFT)); /* OBSD_ITGR -- omit warning */
		...
		for (t = 0, type = root->be_type; type > 0; type >>= 7)
			buf[t++] = (u_char)(type & ~BER_TAG_MORE); /* OBSD_ITGR -- omit warning */
	...
	if (root->be_len < BER_TAG_MORE) {
		/* short form */
		ber_putc(ber, (u_char) root->be_len); /* OBSD_ITGR -- omit warning */
	} else {
...
get_id(struct ber *b, unsigned long *tag, int *class, int *cstruct)
{
	...
	*cstruct = (u & BER_TYPE_CONSTRUCTED) == BER_TYPE_CONSTRUCTED;
	...
	if (*class != 0) /* sado -- Task 2402776; long form only if class != universal */
	{
		do {
			if (ber_getc(b, &u) == -1)
				return -1;
...
ber_read_element(struct ber *ber, struct ber_element *elm)
{
...
#ifdef BSD_STACKPORT
	/* sado -- RQ 2710524; limit recursion level of BER-structure */
	ber->seq_recursion_level++;
	if (ber->seq_recursion_level > MAX_SEQUENCE_RECURSIONS)
	{
		errno = ECANCELED;
		return -1;
	}
	...
	if (elm == NULL || ber == NULL) /* sado -- Task 2402776 */
		return -1;
	...
	if (ber->fd == -1 && len > ber->br_rend - ber->br_rptr) {
		errno = ECANCELED; /* sado -- Task 2402776; this check omits reading or writing out of the msg-boundary */
		return -1;
	}
	...
	elm->be_class = (u_int8_t)class; /* OBSD_ITGR -- omit warning */
	...
	case BER_TYPE_OBJECT:
		...
		elm->be_free = 1;
		elm->be_len = len;
		if (ber_read(ber, elm->be_val, len) == -1) return -1; /* sado -- Task 2402776 */
	...
	case BER_TYPE_SEQUENCE:
	case BER_TYPE_SET:
		if (class == 0 && cstruct != 1) return -1; /* sado -- Task 2402776; ASN-1 Universal Types, sequence and set only with constructed flag */
		if (elm->be_sub == NULL) {
		...
		while (len > 0) {
			...
			if (r == -1)
				return -1;
			len -= r;
			if (len < 0) /* sado -- Task 2402776; length fields do not match -- discard msg */
			{
				return -1;
			}
			else if (len > 0) /* sado -- Task 2402776; next field is NULL if len == 0 */
			{
				if (next->be_next == NULL) {
					if ((next->be_next = ber_get_element(0)) == NULL) return -1;
				}
				next = next->be_next;
			}
			/* else len == 0 --> break while */
		}
		break;
	default:		/* mh2290: LINT 744 switch statement has no default */
		return -1;  /* sado -- Task 2402776; only these types above are allowed for SNMP, SNMP messages (A0-A7) are treated like sequence-type */
	}

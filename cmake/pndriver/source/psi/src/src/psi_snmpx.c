...
/*                                                                           */
/*  channel-detail settings for SNMPX                                        */
/*                                                                           */
/*****************************************************************************/

#define LTRC_ACT_MODUL_ID   116
#define PSI_MODULE_ID       116 /* PSI_MODULE_ID_PSI_SNMPX */
	...
	LSA_SYS_PATH_TYPE       sys_path)
{
    ...
    PSI_LD_TRACE_06( 0, LSA_TRACE_LEVEL_NOTE, "psi_snmpx_get_path_info(sys_path=%#x): pnio_if_nr(%u) hd_nr(%u) hd_runs_on_level_ld(%u) path(%#x/%u)", 
    ...
    switch (path)
	{
	    case PSI_PATH_GLO_APP_SNMPX:  // SNMP User
		{
            ...
            #endif
            {
			    mbx_id_rsp = PSI_MBX_ID_USER; // SNMPX user
            }
        ...
        case PSI_PATH_GLO_SYS_SNMPX_SOCK:
		{
			...
			pDetail->Params.Sock.NumOfRecv = 0; /* SNMP Agent not supported */ 
		...
		{
			pDetail->PathType             = SNMPX_PATH_TYPE_OHA;
	    ...
	    default:
		{
			...
			*sys_ptr_ptr    = LSA_NULL;
            ...
            PSI_LD_TRACE_05( 0, LSA_TRACE_LEVEL_ERROR, "psi_snmpx_get_path_info(sys_path=%#x): invalid path(%#x/%u), hd_nr(%u) result(0x%x)",
	...
	LSA_VOID_PTR_TYPE   detail_ptr)
{
    ...
    PSI_LD_TRACE_00( 0, LSA_TRACE_LEVEL_NOTE, "psi_snmpx_release_path_info()" );
...
/*----------------------------------------------------------------------------*/
LSA_VOID psi_snmpx_open_channel(
    LSA_SYS_PATH_TYPE   sys_path)
{
	...
	PSI_SYSTEM_TRACE_04( trace_idx, LSA_TRACE_LEVEL_NOTE, "psi_snmpx_open_channel(): Open SNMPX channel, pnio_if_nr(%u) hd_nr(%u) sys_path(%#x) path(%#x)", 
...
/*----------------------------------------------------------------------------*/
LSA_VOID psi_snmpx_close_channel(
    LSA_SYS_PATH_TYPE   sys_path, 
    LSA_HANDLE_TYPE     handle)
{
	...
	PSI_SYSTEM_TRACE_05( trace_idx, LSA_TRACE_LEVEL_NOTE, "psi_snmpx_close_channel(): Close SNMPX channel, pnio_if_nr(%u) hd_nr(%u) sys_path(%#x) path(%#x) handle(%u)", 
...
/*----------------------------------------------------------------------------*/
LSA_VOID psi_snmpx_channel_done(
    LSA_VOID_PTR_TYPE   rqb_ptr)
{
	LSA_UINT16                     rc;
	LSA_UINT32                     trace_idx;
	LSA_OPCODE_TYPE                opc;
	SNMPX_UPPER_RQB_PTR_TYPE const pRQB = (SNMPX_UPPER_RQB_PTR_TYPE)rqb_ptr;
    ...
    /* Handle response for SNMPX Opcode */
	switch (opc)
	{
	    case SNMPX_OPC_OPEN_CHANNEL:
		{
			...
			PSI_SYSTEM_TRACE_04( trace_idx, LSA_TRACE_LEVEL_NOTE, "psi_snmpx_channel_done(): Open SNMPX channel done, rsp(%u/%#x) sys_path(%#x) handle(%u)",
        ...
        case SNMPX_OPC_CLOSE_CHANNEL:
		{
			...
			PSI_SYSTEM_TRACE_03( trace_idx, LSA_TRACE_LEVEL_NOTE, "psi_snmpx_channel_done(): Close SNMPX channel done, rsp(%u/%#x) sys_path(%#x)",
				SNMPX_RQB_GET_RESPONSE(pRQB), 
				SNMPX_RQB_GET_RESPONSE(pRQB), 
				sys_path );
        ...
        default:
		{
			PSI_SYSTEM_TRACE_02( 0, LSA_TRACE_LEVEL_FATAL, "psi_snmpx_channel_done(): invalid opcode(%u/%#x)", opc, opc );
			PSI_FATAL(0);
		}
	}
}

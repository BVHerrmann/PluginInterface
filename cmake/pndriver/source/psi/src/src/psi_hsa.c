...
/*                                                                           */
/*  channel-detail settings for HSA                                          */
/*                                                                           */
/*****************************************************************************/

#define LTRC_ACT_MODUL_ID   119
#define PSI_MODULE_ID       119 /* PSI_MODULE_ID_PSI_HSA */
    ...
    LSA_SYS_PATH_TYPE   const sys_path )
{
    ...
    PSI_HD_TRACE_06(0, LSA_TRACE_LEVEL_NOTE, "psi_hsa_get_path_info(sys_path=%#x): pnio_if_nr(%u) hd_nr(%u) hd_runs_on_level_ld(%u) path(%#x/%u)",
        sys_path, pDetailsIF->pnio_if_nr, hd_nr, pDetailsIF->hd_runs_on_level_ld, path, path);

    PSI_ALLOC_LOCAL_MEM( ((LSA_VOID_PTR_TYPE*)&pDetail), user_id, sizeof(*pDetail), 0, LSA_COMP_ID_HSA, PSI_MTYPE_LOCAL_MEM );
    PSI_ASSERT(pDetail != LSA_NULL);

    PSI_ALLOC_LOCAL_MEM( ((LSA_VOID_PTR_TYPE*)&pSys), user_id, sizeof(*pSys), 0, LSA_COMP_ID_HSA, PSI_MTYPE_LOCAL_MEM );
    ...
    switch (path)
    {
        ...
        {
            pDetail->PathType = HSA_PATH_TYPE_EDD;

            PSI_ASSERT(hd_nr && (hd_nr <= PSI_CFG_MAX_IF_CNT));
			pDetail->edd.id		        = hd_nr;
			pDetail->edd.bIsHsaAppl		= (LSA_UINT8)pDetailsIF->hsa.bHsyncAppl;
			pDetail->edd.eddType		= pDetailsIF->edd_comp_id;

            #if (PSI_CFG_USE_HIF_HD == 1) // Using HIF HD?
            if (PSI_HD_RUNS_ON_LEVEL_LD_NO == pDetailsIF->hd_runs_on_level_ld)
            {
        ...
        {
            pDetail->PathType = HSA_PATH_TYPE_USER;

            #if (PSI_CFG_USE_HIF_LD) // Using HIF LD?
            switch (psi_get_ld_runs_on())
            {
                case PSI_LD_RUNS_ON_ADVANCED:
                case PSI_LD_RUNS_ON_BASIC:
                {
        ...
        {		
            ...
            PSI_FREE_LOCAL_MEM( &rc, pDetail, 0, LSA_COMP_ID_HSA, PSI_MTYPE_LOCAL_MEM );
            PSI_ASSERT(rc == LSA_RET_OK);

            PSI_FREE_LOCAL_MEM( &rc, pSys, 0, LSA_COMP_ID_HSA, PSI_MTYPE_LOCAL_MEM );
            PSI_ASSERT(rc == LSA_RET_OK);
            ...
            *sys_ptr_ptr    = LSA_NULL;
            ...
            PSI_HD_TRACE_05( 0, LSA_TRACE_LEVEL_ERROR, "psi_hsa_get_path_info(sys_path=%#x): invalid path(%#x/%u), hd_nr(%u) result(0x%x)",
    ...
    LSA_VOID_PTR_TYPE const detail_ptr )
{
    ...
    PSI_ASSERT(pSys->comp_id == LSA_COMP_ID_HSA);

    PSI_HD_TRACE_00(0, LSA_TRACE_LEVEL_NOTE, "psi_hsa_release_path_info()");

    PSI_FREE_LOCAL_MEM( &rc, detail_ptr, 0, LSA_COMP_ID_HSA, PSI_MTYPE_LOCAL_MEM );
    PSI_ASSERT(rc == LSA_RET_OK);

    PSI_FREE_LOCAL_MEM( &rc, sys_ptr, 0, LSA_COMP_ID_HSA, PSI_MTYPE_LOCAL_MEM );
    PSI_ASSERT(rc == LSA_RET_OK);
...
/*----------------------------------------------------------------------------*/
LSA_VOID psi_hsa_open_channel(
    LSA_SYS_PATH_TYPE const sys_path )
{
    LSA_UINT32             trace_idx;
    HSA_UPPER_RQB_PTR_TYPE pRQB = LSA_NULL;
	...
	HSA_ALLOC_LOCAL_MEM((LSA_VOID**)&pRQB, sizeof(*pRQB));
    PSI_ASSERT(pRQB != LSA_NULL);

    /* note: using low byte of path as upper-handle (path without PNIO IF nr ! */
    pRQB->args.channel.handle                       = PSI_INVALID_HANDLE; // No HSA_INVALID_HANDLE
    pRQB->args.channel.handle_upper                 = (LSA_UINT8)path;
    pRQB->args.channel.sys_path                     = sys_path;
    pRQB->args.channel.hsa_request_upper_done_ptr   = (HSA_UPPER_CALLBACK_FCT_PTR_TYPE)psi_hsa_channel_done;

    PSI_RQB_SET_HANDLE(pRQB, PSI_INVALID_HANDLE);
    PSI_RQB_SET_OPCODE(pRQB, HSA_OPC_OPEN_CHANNEL);
    PSI_RQB_SET_COMP_ID(pRQB, LSA_COMP_ID_HSA);

    trace_idx = psi_get_trace_idx_for_sys_path(sys_path);

    PSI_SYSTEM_TRACE_04(trace_idx, LSA_TRACE_LEVEL_NOTE, "psi_hsa_open_channel(): Open HSA channel, pnio_if_nr(%u) hd_nr(%u) sys_path(%#x) path(%#x)", 
        pnio_if_nr, hd_nr, pRQB->args.channel.sys_path, path);

    /* HSA is on LD side --> open can be done direct */
    psi_request_start(PSI_MBX_ID_HSA, (PSI_REQUEST_FCT)hsa_open_channel, pRQB);
}

/*----------------------------------------------------------------------------*/
LSA_VOID psi_hsa_close_channel(
    LSA_SYS_PATH_TYPE const sys_path,
    LSA_HANDLE_TYPE   const handle )
{
    LSA_UINT32             trace_idx;
    HSA_UPPER_RQB_PTR_TYPE pRQB;
    ...
    PSI_RQB_SET_OPCODE(pRQB, HSA_OPC_CLOSE_CHANNEL);
    PSI_RQB_SET_COMP_ID(pRQB, LSA_COMP_ID_HSA);
    ...
    PSI_SYSTEM_TRACE_05(trace_idx, LSA_TRACE_LEVEL_NOTE, "psi_hsa_close_channel(): Close hsa channel, pnio_if_nr(%u) hd_nr(%u) sys_path(%#x) path(%#x) handle(%u)", 
        pnio_if_nr, hd_nr, sys_path, path, handle);

    /* HSA is on LD side --> close can be done direct */
    psi_request_start(PSI_MBX_ID_HSA, (PSI_REQUEST_FCT)hsa_close_channel, pRQB);
}

/*----------------------------------------------------------------------------*/
LSA_VOID psi_hsa_channel_done(
    LSA_VOID_PTR_TYPE const rqb_ptr )
{
    LSA_UINT16                   rc;
    LSA_UINT32                   trace_idx;
    LSA_OPCODE_TYPE              opc;
    HSA_UPPER_RQB_PTR_TYPE const pRQB     = (HSA_UPPER_RQB_PTR_TYPE)rqb_ptr;
    LSA_SYS_PATH_TYPE            sys_path = 0;
    LSA_HANDLE_TYPE              handle   = 0;

    PSI_ASSERT(pRQB != LSA_NULL);
    PSI_ASSERT(HSA_RQB_GET_RESPONSE(pRQB) == HSA_OK);

    opc = HSA_RQB_GET_OPCODE(pRQB);

    /* Handle response for HSA Opcode */
    switch (opc)
    {
        case HSA_OPC_OPEN_CHANNEL:
        {
            ...
            PSI_SYSTEM_TRACE_04(trace_idx, LSA_TRACE_LEVEL_NOTE, "psi_hsa_channel_done(): Open HSA channel done, rsp(%u/%#x) sys_path(%#x) handle(%u)",
                HSA_RQB_GET_RESPONSE(pRQB),
                HSA_RQB_GET_RESPONSE(pRQB),
                pRQB->args.channel.sys_path,
                pRQB->args.channel.handle );

            HSA_FREE_LOCAL_MEM(&rc, pRQB);
            PSI_ASSERT(rc == HSA_OK);
        ...
        case HSA_OPC_CLOSE_CHANNEL:
        {
            // Unregister handle for this PNIO IF (stored in USER-ID)
            sys_path    = LSA_RQB_GET_USER_ID_UVAR16(pRQB);
            trace_idx   = psi_get_trace_idx_for_sys_path(sys_path);

            PSI_SYSTEM_TRACE_03(trace_idx, LSA_TRACE_LEVEL_NOTE, "psi_hsa_channel_done(): Close HSA channel done, rsp(%u/%#x) sys_path(%#x)",
                HSA_RQB_GET_RESPONSE(pRQB),
                HSA_RQB_GET_RESPONSE(pRQB),
                sys_path );
        ...
        default:
        {
            PSI_SYSTEM_TRACE_02(0, LSA_TRACE_LEVEL_FATAL, "psi_hsa_channel_done(): invalid opcode(%u/%#x)", opc, opc);
            PSI_FATAL(0);
        }
	}
}

#endif //((PSI_CFG_USE_HSA == 1) && (PSI_CFG_USE_LD_COMP == 1))

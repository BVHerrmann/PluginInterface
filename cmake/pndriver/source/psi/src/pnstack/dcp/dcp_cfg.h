...
*/
#define PSI_DCP_HELLO_RESOURCES		 2  // max. number of concurrent server HELLO requests /* to prevent a resource error on Stop/Start sequence */ 
...
*/
#define DCP_MAX_LOW_CHANNELS        (PSI_CFG_MAX_IF_CNT) /* Lower Interfaces */

/* Maximum number of client channels on one lower channel (61)
 */
#define DCP_MAX_CLIENT_CHANNELS     1 /* One DCP Client Channel / Interface */
...
*/
#define DCP_MAX_CHANNELS  		((DCP_MAX_CLIENT_CHANNELS + 2 /* Hello + Server Channel */) * DCP_MAX_LOW_CHANNELS)
 ...
 *===========================================================================*/
#ifndef DCP_FILE_SYSTEM_EXTENSION
#define DCP_FILE_SYSTEM_EXTENSION(module_id_) /* Currently not supported */
#endif
/*---------------------------------------------------------------------------*/
...
#define DCP_PUT_INFO_16(info_id_, para1_, para2_, para3_, para4_, para5_, \
 ...
 /*- alloc mem macros----------------------------------------------------------*/
#define DCP_ALLOC_UPPER_RQB(mem_ptr_ptr, user_id, length, sys_ptr)  PSI_ALLOC_LOCAL_MEM((mem_ptr_ptr), (user_id), (length), (sys_ptr), LSA_COMP_ID_DCP, PSI_MTYPE_UPPER_RQB)
#define DCP_ALLOC_UPPER_RQB_LOCAL(mem_ptr_ptr, length)              PSI_ALLOC_LOCAL_MEM((mem_ptr_ptr), 0, (length), 0, LSA_COMP_ID_DCP, PSI_MTYPE_UPPER_RQB)
#define DCP_ALLOC_LOWER_RQB(mem_ptr_ptr, user_id, length, sys_ptr)  PSI_ALLOC_LOCAL_MEM(&(mem_ptr_ptr)->VoidPtr, (user_id), (length), (sys_ptr), LSA_COMP_ID_DCP, PSI_MTYPE_LOWER_RQB_EDD)
#define DCP_ALLOC_LOWER_MEM(mem_ptr_ptr, user_id, length, sys_ptr)  PSI_ALLOC_LOCAL_MEM(&(mem_ptr_ptr)->VoidPtr, (user_id), (length), (sys_ptr), LSA_COMP_ID_DCP, PSI_MTYPE_LOWER_MEM_EDD)
#define DCP_ALLOC_LOCAL_MEM(mem_ptr_ptr, length)                    PSI_ALLOC_LOCAL_MEM((mem_ptr_ptr), 0, (length), 0, LSA_COMP_ID_DCP, PSI_MTYPE_LOCAL_MEM)

#define DCP_FREE_UPPER_RQB(ret_val_ptr, upper_rqb_ptr, sys_ptr)     PSI_FREE_LOCAL_MEM((ret_val_ptr), (upper_rqb_ptr), (sys_ptr), LSA_COMP_ID_DCP, PSI_MTYPE_UPPER_RQB)
#define DCP_FREE_UPPER_RQB_LOCAL(ret_val_ptr, upper_rqb_ptr)        PSI_FREE_LOCAL_MEM((ret_val_ptr), (upper_rqb_ptr), 0, LSA_COMP_ID_DCP, PSI_MTYPE_UPPER_RQB)
#define DCP_FREE_LOWER_RQB(ret_val_ptr, lower_rqb_ptr, sys_ptr)     PSI_FREE_LOCAL_MEM((ret_val_ptr), (lower_rqb_ptr).VoidPtr, 0, LSA_COMP_ID_DCP, PSI_MTYPE_LOWER_RQB_EDD)
#define DCP_FREE_LOWER_MEM(ret_val_ptr, lower_mem_ptr, sys_ptr)     PSI_FREE_LOCAL_MEM((ret_val_ptr), (lower_mem_ptr).VoidPtr, (sys_ptr), LSA_COMP_ID_DCP, PSI_MTYPE_LOWER_MEM_EDD)
#define DCP_FREE_LOCAL_MEM(ret_val_ptr, local_mem_ptr)              PSI_FREE_LOCAL_MEM((ret_val_ptr), (local_mem_ptr), 0, LSA_COMP_ID_DCP, PSI_MTYPE_LOCAL_MEM)

...
#define LTRC_ACT_MODUL_ID   1206
#define PSI_MODULE_ID       1206 /* PSI_MODULE_ID_CM_CFG_NARE */
...
LSA_VOID CM_NARE_OPEN_CHANNEL_LOWER(
...
LSA_VOID CM_NARE_CLOSE_CHANNEL_LOWER(
	CM_NARE_LOWER_RQB_PTR_TYPE lower_rqb_ptr,
	LSA_SYS_PTR_TYPE           sys_ptr )
{
	PSI_SYS_PTR_TYPE pSys = (PSI_SYS_PTR_TYPE)sys_ptr;
...
LSA_VOID CM_NARE_REQUEST_LOWER(
	CM_NARE_LOWER_RQB_PTR_TYPE lower_rqb_ptr,
	LSA_SYS_PTR_TYPE           sys_ptr )
{
	PSI_SYS_PTR_TYPE pSys = (PSI_SYS_PTR_TYPE)sys_ptr;
	...
	PSI_SYSTEM_TRACE_03( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT,
		"CM_NARE_REQUEST_LOWER() rqb(0x%08x) h(%d) opc(%d)",
		lower_rqb_ptr,
...
LSA_VOID CM_NARE_ALLOC_LOWER_RQB(
	...
	LSA_SYS_PTR_TYPE             sys_ptr )
{
    ...
    {
		PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_FATAL, "CM_NARE_ALLOC_LOWER_RQB(): invalid length(%u) != sizeof(CM_NARE_LOWER_RQB_TYPE)", length );
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_NARE_ALLOC_LOWER_RQB(): ptr(0x%08x)", *lower_rqb_ptr_ptr );
}
#else
#error "by design a function!"
#endif
...
LSA_VOID CM_NARE_FREE_LOWER_RQB(
	...
	LSA_SYS_PTR_TYPE             sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_NARE_FREE_LOWER_RQB(): ptr(0x%08x)", lower_rqb_ptr );
...
LSA_VOID CM_NARE_ALLOC_LOWER_MEM(
	...
	LSA_SYS_PTR_TYPE             sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_NARE_ALLOC_LOWER_MEM(): ptr(0x%08x)", *lower_mem_ptr_ptr );
}
#else
#error "by design a function!"
#endif
...
LSA_VOID CM_NARE_FREE_LOWER_MEM(
	LSA_UINT16                 * ret_val_ptr,
	...
	LSA_SYS_PTR_TYPE             sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_NARE_FREE_LOWER_MEM(): ptr(0x%08x)", lower_mem_ptr );

...
#define PSI_MODULE_ID       1202 /* PSI_MODULE_ID_CM_CFG_EDD */
...
LSA_VOID CM_EDD_OPEN_CHANNEL_LOWER(
	...
	LSA_SYS_PTR_TYPE          sys_ptr )
{
	...
	mbx_id = psi_edd_get_mailbox_id( lower_rqb_ptr, LSA_COMP_ID_CM, sys_ptr );
...
LSA_VOID CM_EDD_CLOSE_CHANNEL_LOWER(
	CM_EDD_LOWER_RQB_PTR_TYPE  lower_rqb_ptr,
	LSA_SYS_PTR_TYPE           sys_ptr )
{
	LSA_UINT16      mbx_id;
	...
	LSA_SYS_PTR_TYPE           sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_04( pSys->trace_idx,LSA_TRACE_LEVEL_CHAT,
		"CM_EDD_REQUEST_LOWER(): rqb(0x%08x) h(%d) opc(%u), srv(%#x)",
		lower_rqb_ptr,
...
LSA_VOID CM_EDD_ALLOC_LOWER_RQB(
	...
	LSA_SYS_PTR_TYPE            sys_ptr )
{
    ...
    {
		PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_FATAL, "CM_EDD_ALLOC_LOWER_RQB(): invalid length(%u) != sizeof(CM_EDD_LOWER_RQB_TYPE)", length );
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_EDD_ALLOC_LOWER_RQB(): ptr(0x%08x)", *lower_rqb_ptr_ptr );
}
#else
#error "by design a function!"
#endif
...
LSA_VOID CM_EDD_FREE_LOWER_RQB(
	...
	LSA_SYS_PTR_TYPE            sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_EDD_FREE_LOWER_RQB(): ptr(0x%08x)", lower_rqb_ptr );
...
LSA_VOID CM_EDD_ALLOC_LOWER_MEM(
	...
	LSA_SYS_PTR_TYPE            sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_EDD_ALLOC_LOWER_MEM(): ptr(0x%08x)", *lower_mem_ptr_ptr );
}
#else
#error "by design a function!"
#endif
...
LSA_VOID CM_EDD_FREE_LOWER_MEM(
	LSA_UINT16                * ret_val_ptr,
	...
	LSA_SYS_PTR_TYPE            sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_EDD_FREE_LOWER_MEM(): ptr(0x%08x)", lower_mem_ptr );

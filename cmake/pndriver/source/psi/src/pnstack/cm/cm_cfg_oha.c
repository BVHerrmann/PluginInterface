...
#define LTRC_ACT_MODUL_ID   1207
#define PSI_MODULE_ID       1207 /* PSI_MODULE_ID_CM_CFG_OHA */
...
LSA_VOID CM_OHA_OPEN_CHANNEL_LOWER(
	...
	LSA_SYS_PTR_TYPE          sys_ptr )
{
	...
	psi_request_start( PSI_MBX_ID_OHA, (PSI_REQUEST_FCT)oha_open_channel, lower_rqb_ptr );
    #else
    #error "No possibility to open channel to oha."
    #endif
}
#else
...
LSA_VOID CM_OHA_CLOSE_CHANNEL_LOWER(
	CM_OHA_LOWER_RQB_PTR_TYPE lower_rqb_ptr,
	LSA_SYS_PTR_TYPE          sys_ptr )
{
	PSI_SYS_PTR_TYPE pSys = (PSI_SYS_PTR_TYPE)sys_ptr;
	...
	psi_request_start( PSI_MBX_ID_OHA, (PSI_REQUEST_FCT)oha_close_channel, lower_rqb_ptr );
    #else
    #error "No posibility to close channel to oha!"
    #endif
}
#else
#error "by design a function!"
	...
	LSA_SYS_PTR_TYPE          sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_03( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT,
		"CM_OHA_REQUEST_LOWER(): rqb(0x%08x) h(%d) opc(%d)",
		lower_rqb_ptr,
	...
	psi_request_start( PSI_MBX_ID_OHA, (PSI_REQUEST_FCT)oha_request, lower_rqb_ptr );
    #else
    #error "No possibility to post messages to oha!"
    #endif
}
#else
#error "by design a function!"
...
LSA_VOID CM_OHA_ALLOC_LOWER_RQB(
	...
	LSA_SYS_PTR_TYPE            sys_ptr )
{
    ...
    {
		PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_FATAL, "CM_OHA_ALLOC_LOWER_RQB(): invalid length(%u) != sizeof(CM_OHA_LOWER_RQB_TYPE)", length );
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_OHA_ALLOC_LOWER_RQB(): ptr(0x%08x)", *lower_rqb_ptr_ptr );
}
#else
#error "by design a function!"
#endif
...
LSA_VOID CM_OHA_FREE_LOWER_RQB(
	...
	LSA_SYS_PTR_TYPE            sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_OHA_FREE_LOWER_RQB(): ptr(0x%08x)", lower_rqb_ptr );
...
LSA_VOID CM_OHA_ALLOC_LOWER_MEM(
	...
	LSA_SYS_PTR_TYPE            sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_OHA_ALLOC_LOWER_MEM(): ptr(0x%08x)", *lower_mem_ptr_ptr );
}
#else
#error "by design a function!"
#endif
...
LSA_VOID CM_OHA_FREE_LOWER_MEM(
	LSA_UINT16                * ret_val_ptr,
	...
	LSA_SYS_PTR_TYPE            sys_ptr )
{
	...
	PSI_SYSTEM_TRACE_01( pSys->trace_idx, LSA_TRACE_LEVEL_CHAT, "CM_OHA_FREE_LOWER_MEM(): ptr(0x%08x)", lower_mem_ptr );

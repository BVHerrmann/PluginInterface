    ...
    LSA_BOOL            *   pCompare)
{
    ...
    *pCompare = LSA_TRUE;
	...
	if (CountOfInterfaces > 1)
    {
        ...
        {
            RetVal = OHA_ERR_RESOURCE;
			OHA_FUNCTION_TRACE_01(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_CHAT, "OUT: OHA_CompareTopoInterface_Gateway(RetVal: 0x%X)", RetVal);
            return(RetVal);
        }
					...
					if (NICId != pHDBEdd->Params.pDetail->Params.Edd.NICId) /* not for this If */
					{
							...
							(!OHA_MEMCMP(GatewayPtr, (OHA_COMMON_MEM_U8_PTR_TYPE)OHA_ZERO_IP, OHA_IP_ADDR_SIZE)))
						{
							...
							if(!OHA_MEMCMP(GatewayPtr, IpAddrPtr, OHA_IP_ADDR_SIZE))
							{
								...
								{
									OHA_FUNCTION_TRACE_02(pHDBUser->TraceIdx, LSA_TRACE_LEVEL_NOTE, "CompareTopoInterface: Different Gateways (ThisNICId: 0x%X, NICId: 0x%X)", NICId, pHDBEdd->Params.pDetail->Params.Edd.NICId);
									*pCompare = LSA_FALSE;
									break;
								}
							}
				...
				if ( pHDBEdd->State == OHA_HANDLE_STATE_OPEN )
				{
					...
					if (NICId != pHDBEdd->Params.pDetail->Params.Edd.NICId) /* not for this If */
					{
							...
							(!OHA_MEMCMP(IpAddrPtr, (OHA_COMMON_MEM_U8_PTR_TYPE)OHA_ZERO_IP, OHA_IP_ADDR_SIZE)))
						{
							...
							/* TFS 2520391, TFS 3080364: CDIR mode (overlapping subnets allowed) */
							/* TFS 3080364: using the rule set of IP forwarding on a longest-match basis (CDIR mode) */

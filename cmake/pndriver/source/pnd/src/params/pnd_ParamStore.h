...
public:
    ...
    virtual LSA_UINT32 GetParamISO( PND_HANDLE_PTR_TYPE cpHandle, PND_PARAM_ISO_HD_IF_PTR_TYPE p_params ) = 0;
...
private:
    ...
    CParamStore(const CParamStore&); // No copy constructor
...
public:
	...
	LSA_UINT32 GetParamStartup( PND_PARAM_STARTUP_PTR_TYPE p_params ) const;
	...
	LSA_VOID SetParamIrteBase(LSA_UINT16 hdNr, EPS_HD_IO_OUTPUT_PTR_TYPE p_output);
	...
	// cleanup methodes
	LSA_VOID DeleteArcb( PND_HANDLE_PTR_TYPE cpHandle, CM_UPPER_CL_DEVICE_ADD_PTR_TYPE p_Add ) const;
	LSA_VOID DeletePrmRecordsPD( PrmRecordVector* pRecList ) const;
	LSA_VOID DeleteDeviceParam( SubslotVector* pSubList ) const;
	LSA_VOID UpdateRema(const PNIO_UINT8* data, PNIO_UINT32 dataLength, PNIO_UINT32 index, PNIO_UINT8** remaData, PNIO_UINT32* bufferLength);
	LSA_VOID SetRemaData(PNIO_UINT32 index, PNIO_UINT8* data);
	
    LSA_VOID_PTR_TYPE GetParamMultipleSNMPRecord();
    LSA_UINT32 IsSystemDescriptionValid(PNIO_SYSTEM_DESCR* pSysDescr) const;
    LSA_UINT32 IsIfDescriptionValid(PNIO_SYSTEM_DESCR* pSysDescr) const;
    LSA_UINT32 GetSysDescr(LSA_UINT8* pSysDescr);
    LSA_UINT32 GetIfDescr(LSA_UINT8* pIfDescr, LSA_UINT16 pnio_if_nr);
    LSA_UINT32 GetPortDescr(PSI_HD_IF_OHA_INPUT_TYPE* pInput, LSA_UINT16 pnio_if_nr, LSA_UINT16 port_nr);
private:
	LSA_VOID prepareParamStartup( LSA_UINT32 nrOfEntries, LSA_VOID* pLocationData, const PNIO_SYSTEM_DESCR* sysDescr );

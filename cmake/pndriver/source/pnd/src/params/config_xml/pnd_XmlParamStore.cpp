...
static PARS_VOID (get_PARS_rema_record)(PARS_PND_REMA_RECORD_TYPE *pars_record, PND_REMA_RECORD_TYPE pnd_record)
{
	CValueHelper val;
...
/*lint -e{1746} parameter 'data' could be made const reference*/
...
/*----------------------------------------------------------------------------*/
LSA_UINT32 CXmlParamStore::GetParamISO(PND_HANDLE_PTR_TYPE cpHandle, PND_PARAM_ISO_HD_IF_PTR_TYPE p_params)
{
    // Read the PNIO IOC settings for fixed IF-Nr=1
    LSA_UINT32 result = PNIO_ERR_PRM_INVALIDARG;
    ...
    if ((cpHandle != 0) && (p_params != 0)) // Args OK ?
    {
            ...
            (m_pPnioStore[hdNr] != 0))
        {
            ...
            *p_params = m_pPnioStore[hdNr]->GetXMLInputISO(cpHandle->if_nr);
            result = PNIO_OK;
        }
    }
...
CPnioXMLStore::CPnioXMLStore()
{
    ...
    for (LSA_UINT16 i = 0; i<(PSI_CFG_MAX_HD_IF_CNT+1); i++)
    {
        ...
        m_InternalXMLData[i].iso.iso_hd.isIsoConfiguration = PNIO_FALSE;
    }
}
    ...
    for ( LSA_UINT16 ifNr = 1; ifNr <= m_nrOfIF; ifNr++ )
    {
            ...
            ( !m_InternalXMLData[ifNr].iod_list.empty() ))
        {
            ...
            for ( LSA_UINT16 i = 0; i < iod_count; i++ )
            {
                ...
                // IRDataUUID
                if(m_InternalXMLData[ifNr].iod_list.at(i)->ir_info_data.ir_data_uuid != LSA_NULL)
                {
                    xml_free(m_InternalXMLData[ifNr].iod_list.at(i)->ir_info_data.ir_data_uuid);
                }
                // DFP IOCR Entries
...
/*----------------------------------------------------------------------------*/
LSA_VOID CPnioXMLStore::initializeXMLParser() const
{
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CPnioXMLStore::IsPdevConfigCentral( LSA_UINT16 ifNr ) const
{
    LSA_BOOL result;
...
/*----------------------------------------------------------------------------*/
PND_PARAM_PD_HD_IF_TYPE CPnioXMLStore::GetXMLInputPD( LSA_UINT16 ifNr ) const
{
    XML_ASSERT((ifNr > 0 ) && ( ifNr <= m_nrOfIF));
...
/*----------------------------------------------------------------------------*/
PND_PARAM_IOC_HD_IF_TYPE CPnioXMLStore::GetXMLInputIOC( LSA_UINT16 ifNr ) const
{
    XML_ASSERT((ifNr > 0 ) && ( ifNr <= m_nrOfIF));
...
/*----------------------------------------------------------------------------*/
PND_PARAM_OHA_HD_IF_TYPE CPnioXMLStore::GetXMLInputOHA( LSA_UINT16 ifNr ) const
{
    XML_ASSERT((ifNr > 0 ) && ( ifNr <= m_nrOfIF));
...
/*----------------------------------------------------------------------------*/
PND_PARAM_ISO_HD_IF_TYPE CPnioXMLStore::GetXMLInputISO(LSA_UINT16 ifNr) const
{
    XML_ASSERT((ifNr > 0) && (ifNr <= m_nrOfIF));

    PND_PARAM_ISO_HD_IF_TYPE result = m_InternalXMLData[ifNr].iso.iso_hd;

    return (result);
}

/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'cpHandle' could be declared as pointing to const*/
LSA_UINT32 CPnioXMLStore::GetDeviceParam( PND_HANDLE_PTR_TYPE cpHandle, LSA_UINT16 devNr, PND_PARAM_DEVICE_PTR_TYPE pDev, SubslotVector* pSubList  )
{
	...
	pDev->isIrt        =  m_InternalXMLData[ifNr].iod_list.at(devNr)->ir_info_data.ir_data_uuid != LSA_NULL;
    ...
    for ( LSA_UINT16 i = 0; i < api_count; i++)
    {
        ...
        for ( LSA_UINT16 j = 0; j < slot_count; j++)
        {
            ...
            for ( LSA_UINT16 k = 0; k < subslot_count; k++ )
            {
                ...
                pSubmod->isIsoSubslot     = m_InternalXMLData[ifNr].iod_list.at(devNr)->exp_submod_data.at(i)->arcb_slot_list.at(j)->arcb_submodule_list.at(k)->isIsoSubmodule;
...
LSA_VOID CPnioXMLStore::GetArcbAddCL( PND_HANDLE_PTR_TYPE cpHandle, LSA_UINT16 devNr, LSA_UINT32 hwType, CM_UPPER_CL_DEVICE_ADD_PTR_TYPE pAdd )
{
    ...
    pAdd->device_mode               = m_InternalXMLData[ifNr].iod_list.at(devNr)->pniod_properties.planned_mode;
    pAdd->device_properties         = m_InternalXMLData[ifNr].iod_list.at(devNr)->pniod_properties.write_multiple_is_supported ? CM_CL_DEVICE_PROPERTIES_MULTIPLE_WRITE : 0;
    pAdd->run_mode                  = 0;
    ...
    cm_arcb_set_nare( // Prepare the CM ARCB NARE part
        ...
        xml_htonl(m_InternalXMLData[ifNr].iod_list.at(devNr)->subnet_mask),
        xml_htonl(m_InternalXMLData[ifNr].iod_list.at(devNr)->gateway_ip),
        LSA_TRUE);
	...
	if ( maxRecSize != 0 ) { // RQ 3214515 - cm_arcb_set_rpc handles the case where maxRecSize is 0
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 CPnioXMLStore::getNrOfIF() const
{
    return 1; // currently only 1 interface per HD is supported
}

/*----------------------------------------------------------------------------*/
xml_doc CPnioXMLStore::createXMLDoc(LSA_CHAR* pXMLBuffer) const
{
    return xml_stdint_parse_from_buffer(pXMLBuffer);
}

/*----------------------------------------------------------------------------*/
LSA_VOID CPnioXMLStore::freeXMLDoc(xml_doc XMLDoc) const
{
    xml_stdint_free_doc(XMLDoc);
}

/*----------------------------------------------------------------------------*/
xml_element CPnioXMLStore::getRootXMLElement(xml_doc XMLDoc) const
{
   ...
   result = getPDNetworkParams(ifNr);
    ...
    if (result == PNIO_OK)
    {
        for (i = 1; i <= m_InternalXMLData[ifNr].ioc.ioc_hd.device_count; i++)
        {
            ...
            result = getDeviceNetworkParams(i, &pDevice->ip_address, &pDevice->subnet_mask, &pDevice->gateway_ip, &pDevice->name_of_station, &pDevice->name_of_station_length,
            ...
            result = getIRInfoData(i, &pDevice->ir_info_data);
            if (result != PNIO_OK)
            {
                return result;
            }

            result = getSubmodulePropertiesData(i, &pDevice->submodule_properties);
            if (result != PNIO_OK)
            {
                return result;
            }
        ...
        if (element != LSA_NULL)
        {
            ...
            if (records_conf_element != LSA_NULL) 
            {
                ...
                while (element1 != LSA_NULL)
                {
					...
					XML_ASSERT(string_length < sizeof(key_string));
                    ...
                    if((key_value != 0x10000)                  // if not SEND_CLOCK
                        ...
                        &&(key_value != 0x230FF)                // if not IsoDataRecord
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 CPnioXMLStore::getISOParams(LSA_UINT16 ifNr)
{
    LSA_UINT16 result;
    xml_element element; 
    xml_element records_conf_element;
    ...
    if (element != LSA_NULL)
    {
        ...
        if (element != LSA_NULL)
        {
            ...
            if (element != LSA_NULL)
            {
                ...
                if (records_conf_element != LSA_NULL)
                {
                    // IOCIsochronousModeData
                    element = xml_stdint_get_element_by_attribute(records_conf_element, "Key", "143615");
                    if (element != LSA_NULL)
                    {
                        PND_XML_RECORD_TYPE record;
                        result = getRecord(element, &record);
                        if (result == PNIO_OK)
                        {
                            m_InternalXMLData[ifNr].iso.iso_hd.CACF = getUINT16FromRecord(record.record_data, XML_BLOCK_HEADER);
                            m_InternalXMLData[ifNr].iso.iso_hd.Tm_ns = getUINT32FromRecord(record.record_data, XML_BLOCK_HEADER + 2);
                            m_InternalXMLData[ifNr].iso.iso_hd.Te_ns = getUINT32FromRecord(record.record_data, XML_BLOCK_HEADER + 6);
                            m_InternalXMLData[ifNr].iso.iso_hd.ReductionRatio = 1; // Default: 1
                            m_InternalXMLData[ifNr].iso.iso_hd.Tk.SeparateTracks.Tki_ns = 20000; // Default: 20 us
                            m_InternalXMLData[ifNr].iso.iso_hd.Tk.SeparateTracks.Tko_ns = 20000; // Default: 20 us
                            m_InternalXMLData[ifNr].iso.iso_hd.isIsoConfiguration = PNIO_TRUE;
                            xml_free(record.record_data);
                        }
                        else
                        {
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 CPnioXMLStore::getDeviceNetworkParams(LSA_UINT16 devNr, LSA_UINT32* ip_address, LSA_UINT32* subnet_mask, LSA_UINT32* gateway_ip, LSA_UINT8** name_of_station,
                ...
                if (network_element != LSA_NULL) 
                {
                    ...
                    if (element != LSA_NULL) 
                    {
                        ...
                        if(result == PNIO_OK)
                        {
                            ...
                            *subnet_mask = getUINT32FromRecord(record.record_data, XML_BLOCK_HEADER+4);   // SubnetMask
                            *gateway_ip = getUINT32FromRecord(record.record_data, XML_BLOCK_HEADER+8);   // GatewayIP
                            xml_free(record.record_data);
                        }
                        else
                        {
                    ...
                    {
                        PND_XML_RECORD_TYPE record;
                        LSA_UINT16 block_length, pointer, countAlias;

                        result = getRecord(element, &record);
                        if(result == PNIO_OK)
                        {
                            pointer = XML_BLOCK_HEADER;
							countAlias = getUINT16FromRecord(record.record_data, pointer);
                            pointer = pointer + 2;
                            for (LSA_UINT16 i = 0; i < countAlias; i++)
                            {
...
LSA_UINT16 CPnioXMLStore::getDeactivatedConfig(LSA_UINT16 devNr, LSA_UINT8* is_active)
{
	...
	/*lint -e{550} symbol 'content_length' not accessed*/
    LSA_UINT32      content_length;
            ...
            for(LSA_UINT16 k = 0; k < nr_of_subslots; k++)   // Subslots
            {
                ...
                if(element != LSA_NULL)
                {
                    ...
                    while (element != LSA_NULL)
                    {
						...
						XML_ASSERT(string_length < sizeof(element_content));
                    ...
                    if (records_conf_element != LSA_NULL) 
                    {
                        ...
                        while (element != LSA_NULL)
                        {
                            ...
                            if (result == PNIO_OK)
                            {
                                ...
                                if (pArcbRecord->record.record_index == 0x8030) // IsochronousModeData
                                {
                                    exp_submod_data->at(i)->arcb_slot_list.at(j)->arcb_submodule_list.at(k)->isIsoSubmodule = PNIO_TRUE;
                                }
                                else
                                {
                                    exp_submod_data->at(i)->arcb_slot_list.at(j)->arcb_submodule_list.at(k)->isIsoSubmodule = PNIO_FALSE;
                                }
                            }
                            else
                            {
    ...
    if(device_element != LSA_NULL)
    {
        ...
        if (element != LSA_NULL) 
        {
            ...
            if (element != LSA_NULL) 
            {
                ...
                {
                    PND_XML_RECORD_TYPE record;
                    LSA_UINT16 numberOfLSCF, pointer, block_length;
                    result = getRecord(element, &record);
                    if(result == PNIO_OK)
                    {
                        pointer = XML_BLOCK_HEADER;
						numberOfLSCF = getUINT16FromRecord(record.record_data, pointer);
                        pointer = pointer + 2;
                        for (LSA_UINT16 i = 0; i < numberOfLSCF; i++)
                        {
    ...
    if(element != LSA_NULL)
    {
        ...
        if (element != LSA_NULL) 
        {
            ...
            if (element != LSA_NULL) 
            {
                ...
                {
                    PND_XML_RECORD_TYPE record;
                    LSA_UINT16 pointer, block_length, numberOfRecords;
                    ...
                    {
                        pointer = XML_BLOCK_HEADER;
						numberOfRecords = getUINT16FromRecord(record.record_data, pointer);
                        pointer = pointer + 2;
                        for (LSA_UINT16 i = 0; i < numberOfRecords; i++)
                        {
...
/*----------------------------------------------------------------------------*/
LSA_UINT16  CPnioXMLStore::getIRInfoData(LSA_UINT16 devNr, PND_XML_IRINFO_DATA_TYPE* ir_info_data)
{
    LSA_UINT16      result;
    xml_element     element;

    ir_info_data->ir_data_uuid = LSA_NULL;
    ...
    if (element != LSA_NULL)
    {
        ...
        if (element != LSA_NULL)
        {
            ...
            {
                // IRInfoData
                element = xml_stdint_get_element_by_attribute(element, "Key", "12550");
                if (element != LSA_NULL)
                {
                    PND_XML_RECORD_TYPE record;
                    LSA_UINT16 pointer, numberOfDFPIOCRs;

                    result = getRecord(element, &record);

                    if (result == PNIO_OK)
                    {
                        ir_info_data->ir_data_uuid = (CLRPC_UUID_PTR_TYPE)xml_malloc(sizeof(CLRPC_UUID_TYPE));

                        pointer = XML_BLOCK_HEADER;
                        
                        // read IRDataUUID
                        ir_info_data->ir_data_uuid->time_low = getUINT32FromRecord(record.record_data, pointer);
                        ir_info_data->ir_data_uuid->time_mid = getUINT16FromRecord(record.record_data, pointer+4);
                        ir_info_data->ir_data_uuid->time_hi_and_version = getUINT16FromRecord(record.record_data, pointer+6);
                        ir_info_data->ir_data_uuid->clock_seq_hi_and_reserved = getUINT8FromRecord(record.record_data, pointer+8);
                        ir_info_data->ir_data_uuid->clock_seq_low = getUINT8FromRecord(record.record_data, pointer+9);
                        ir_info_data->ir_data_uuid->node[0] = getUINT8FromRecord(record.record_data, pointer+10);
                        ir_info_data->ir_data_uuid->node[1] = getUINT8FromRecord(record.record_data, pointer+11);
                        ir_info_data->ir_data_uuid->node[2] = getUINT8FromRecord(record.record_data, pointer+12);
                        ir_info_data->ir_data_uuid->node[3] = getUINT8FromRecord(record.record_data, pointer+13);
                        ir_info_data->ir_data_uuid->node[4] = getUINT8FromRecord(record.record_data, pointer+14);
                        ir_info_data->ir_data_uuid->node[5] = getUINT8FromRecord(record.record_data, pointer+15);
                        pointer = pointer + 16;

						numberOfDFPIOCRs = getUINT16FromRecord(record.record_data, pointer);
                        pointer = pointer + 2;

                        for (LSA_UINT16 i = 0; i < numberOfDFPIOCRs; i++)
                        {
                            PND_XML_DFP_IOCR_PTR_TYPE iocr = (PND_XML_DFP_IOCR_PTR_TYPE)xml_malloc(sizeof(PND_XML_DFP_IOCR_TYPE));

                            iocr->iocr_ref = getUINT16FromRecord(record.record_data, pointer);
                            iocr->subframe_offset = getUINT16FromRecord(record.record_data, pointer+2);
                            iocr->subframe_data = getUINT32FromRecord(record.record_data, pointer+4);
                            pointer += 8;
                            ir_info_data->dfp_iocr_list.push_back(iocr);
                        }
                        xml_free(record.record_data);
                    }
                }
...
/*----------------------------------------------------------------------------*/
LSA_UINT16  CPnioXMLStore::getSubmodulePropertiesData(LSA_UINT16 devNr, PND_XML_SUBMODULE_PROPERTIES_TYPE *submodule_properties)
{
    LSA_UINT16      result;
    xml_element     element;
    ...
    if (element != LSA_NULL)
    {
        ...
        // find SUBMODULE_PROPERTIES_DATA
        element = xml_locate_element_by_child_content(element, "AID", "13", 1);
        if (element != LSA_NULL)
        {
            element = xml_stdint_get_element_by_name(element, "Value");
            if (element != LSA_NULL)
            {
                // SUBMODULE_PROPERTIES_DATA
                element = xml_stdint_get_element_by_attribute(element, "Key", "12560");
                if (element != LSA_NULL)
                {
                    PND_XML_RECORD_TYPE record;
                    ...
                    if (result == PNIO_OK)
                    {
                        submodule_properties->submodule_properties_data_len = static_cast <LSA_UINT16> (record.record_data_length);

                        submodule_properties->submodule_properties_data = (LSA_UINT8*)xml_malloc(submodule_properties->submodule_properties_data_len);
                        xml_memcpy(submodule_properties->submodule_properties_data, record.record_data, submodule_properties->submodule_properties_data_len);
                ...
                {
                    submodule_properties->submodule_properties_data_len = 0;
                    result = PNIO_OK;
                }
            }
        }
...
/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'pOutput' could be declared as pointing to const */
...
LSA_VOID* CPnioXMLStore::createArcbExpectedConfig( PND_HANDLE_PTR_TYPE cpHandle, LSA_UINT16 devNr, LSA_VOID* psiSysPtr )
{
    ...
    pARCB = cm_arcb_new_io_ar( // create AR ARCB setting
        ...
        m_InternalXMLData[ifNr].iod_list.at(devNr)->ir_info_data.ir_data_uuid,
        (PNIO_UINT8)m_InternalXMLData[ifNr].ioc.ioc_hd.sr_ar_nr); // last param added for V6.0...02.00.65 integration
    ...
    if (m_InternalXMLData[ifNr].iod_list.at(devNr)->submodule_properties.submodule_properties_data_len != 0)
    {
        CM_UPPER_MEM_U8_PTR_TYPE pMEM = cm_arcb_append_submodule_properties(
            psiSysPtr,
            pARCB,
            m_InternalXMLData[ifNr].iod_list.at(devNr)->submodule_properties.submodule_properties_data_len,
            m_InternalXMLData[ifNr].iod_list.at(devNr)->submodule_properties.submodule_properties_data,
            LSA_TRUE);

        m_InternalXMLData[ifNr].iod_list.at(devNr)->submodule_properties.submodule_properties_data_len = 0;
        xml_free( m_InternalXMLData[ifNr].iod_list.at(devNr)->submodule_properties.submodule_properties_data );
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 CPnioXMLStore::getDatalenIOCR( LSA_UINT16 iocrDatalen, LSA_UINT32 iocrProp ) const
{
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 CPnioXMLStore::getRecord(xml_element element, PND_XML_RECORD_PTR_TYPE record) const
{
    LSA_UINT16  result;
...
/*----------------------------------------------------------------------------*/
LSA_UINT32 CPnioXMLStore::getValueFromRecord(const LSA_UINT8* record_data, LSA_UINT16 start, LSA_UINT16 length) const
{
    LSA_UINT16  i;
    LSA_UINT32  v;
...
/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'record_data' could be declared as pointing const*/
LSA_UINT8 CPnioXMLStore::getUINT8FromRecord(LSA_UINT8* record_data, LSA_UINT16 start) const
{
    LSA_UINT32 value;
    value = 0;
...
/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'record_data' could be declared as pointing to const*/
LSA_UINT16 CPnioXMLStore::getUINT16FromRecord(LSA_UINT8* record_data, LSA_UINT16 start) const
{
    LSA_UINT32 value;
    value = 0;
...
/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'record_data' could be declared as pointing to const */
LSA_UINT32 CPnioXMLStore::getUINT32FromRecord(LSA_UINT8* record_data, LSA_UINT16 start) const
{
    LSA_UINT32 value;
    value = 0;

...
/*----------------------------------------------------------------------------*/
LSA_UINT32 CParamStore::GetParamStartup( PND_PARAM_STARTUP_PTR_TYPE p_params ) const
{
...
/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'p_output' could be declared as pointing to const*/
LSA_UINT32 CParamStore::SetParamPsiHD( LSA_UINT16 hdNr, PSI_HD_OUTPUT_PTR_TYPE p_output )
{
	    ...
	    ( m_created     ))    // created ?
	{
		...
		PND_ASSERT(PSI_CFG_MAX_HD_CNT >= hdNr );
		result = m_pParamHandlerXML->UpdatePnioIF( hdNr, 1, &m_hdOutput[hdNr].pnio_if);
	}
...
/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'p_output' could be declared as pointing to const*/
LSA_VOID CParamStore::SetParamIrteBase(LSA_UINT16 hdNr, EPS_HD_IO_OUTPUT_PTR_TYPE p_output)
{
	PND_ASSERT(hdNr < PSI_CFG_MAX_HD_CNT);

	if ((p_output != 0) &&  // Param ok ?
		(m_created))    // created ?
	{
		m_startup.ioAddress[hdNr].irte_base.pBase = p_output->irte_base.pBase;
	}
}
...
LSA_UINT32 CParamStore::GetParamOHA( PND_HANDLE_PTR_TYPE cpHandle, PND_PARAM_OHA_HD_IF_PTR_TYPE p_params )
{
    ...
    //add info given by user at run-time
    PND_ASSERT(sizeof(p_params->station_type) == sizeof(m_systemDescription.IM_DeviceType));

    pnd_memcpy(p_params->station_type, m_systemDescription.IM_DeviceType, sizeof(p_params->station_type));
...
LSA_UINT32 CParamStore::GetParamISO(PND_HANDLE_PTR_TYPE cpHandle, PND_PARAM_ISO_HD_IF_PTR_TYPE p_params)
{
    LSA_UINT32 result;
    ...
    // delegate to implementation class
    result = m_pParamHandlerXML->GetParamISO(cpHandle, p_params);
	p_params->pBase = m_startup.ioAddress[cpHandle->hd_nr-1].irte_base.pBase;
...
/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'cpHandle' could be declared as pointint to const */
...
LSA_VOID CParamStore::DeleteArcb( PND_HANDLE_PTR_TYPE cpHandle, CM_UPPER_CL_DEVICE_ADD_PTR_TYPE pAdd ) const
{
	if ( pAdd != 0 )
	{
		CValueHelper valObj;
        ...
        CM_UPPER_ARCB_ADD_PTR_TYPE  arcb = (CM_UPPER_ARCB_ADD_PTR_TYPE)pAdd->arcb;
        cm_arcb_free(&locSys, arcb, LSA_TRUE, LSA_TRUE, LSA_TRUE, LSA_TRUE, LSA_TRUE);
		cm_arcb_free_nare( &locSys, &pAdd->dev_addr, LSA_TRUE, LSA_TRUE );

	}
}

/*----------------------------------------------------------------------------*/
LSA_VOID CParamStore::DeletePrmRecordsPD( PrmRecordVector* pRecList ) const
{
	if (( pRecList != 0 ) &&
		( !pRecList->empty() ))
	{
		// cleanup the created PRM records
		LSA_UINT recordCount = pRecList->size();

		for ( LSA_UINT lvi = 0; lvi < recordCount; lvi++ )
		{
...
/*----------------------------------------------------------------------------*/
LSA_VOID CParamStore::DeleteDeviceParam( SubslotVector* pSubList ) const
{
	if (( pSubList != 0 ) &&
	    ( !pSubList->empty() ))
	{
		// cleanup the created PRM records
		LSA_UINT subslotCount = pSubList->size();

		for ( LSA_UINT lvi = 0; lvi < subslotCount; lvi++ )
		{
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'pLocationData' could be declared as pointing const*/
LSA_VOID CParamStore::prepareParamStartup( LSA_UINT32 nrOfEntries, LSA_VOID* pLocationData, const PNIO_SYSTEM_DESCR* pSysDescr )
{
    ...
    m_imInput.hw_revision           = m_systemDescription.IM_HwRevision;
    m_imInput.profile_id            = m_systemDescription.IM_ProfileId;
    m_imInput.profile_specific_type = m_systemDescription.IM_ProfileSpecificType;
    m_imInput.revision_counter      = m_systemDescription.IM_RevisionCounter;
    m_imInput.supported             = m_systemDescription.IM_Supported;
    m_imInput.vendor_id_high        = 0;
    m_imInput.vendor_id_low         = 0x2a;
    m_imInput.version               = m_systemDescription.IM_Version;

	PND_ASSERT(sizeof(m_imInput.order_id) == sizeof(m_systemDescription.IM_OrderId));
	PND_ASSERT(sizeof(m_imInput.device_type) == sizeof(m_systemDescription.IM_DeviceType));
    PND_ASSERT(sizeof(m_imInput.serial_nr) <= sizeof(m_systemDescription.ProductSerialNr));

	pnd_memcpy(m_imInput.order_id, m_systemDescription.IM_OrderId, sizeof(m_systemDescription.IM_OrderId));
	pnd_memcpy(m_imInput.device_type, m_systemDescription.IM_DeviceType, sizeof(m_systemDescription.IM_DeviceType));
    pnd_memcpy(m_imInput.serial_nr, m_systemDescription.ProductSerialNr, sizeof(m_imInput.serial_nr));

    m_imInput.sw_version.revision_prefix        = m_systemDescription.IM_SwVersion.revision_prefix;
    m_imInput.sw_version.functional_enhancement = m_systemDescription.IM_SwVersion.functional_enhancement;
    m_imInput.sw_version.bug_fix                = m_systemDescription.IM_SwVersion.bug_fix;
    m_imInput.sw_version.internal_change        = m_systemDescription.IM_SwVersion.internal_change;
	...
	{		
        ...
        #if (EPS_PLF == EPS_PLF_SOC_MIPS || EPS_PLF == EPS_PLF_LINUX_SOC1 || defined(PND_CFG_BASIC_VARIANT))        
		m_startup.pnd_handle[idx].edd_comp_id = LSA_COMP_ID_EDDI;
        ...
        #else
        // HD location setting
        pHD->hd_location.hd_selection = PSI_HD_SELECT_EMBEDDED;
        #endif  

        // HD common settings
        pHD->hd_id                    = idx +1;
        pHD->hd_runs_on_level_ld      = PSI_HD_RUNS_ON_LEVEL_LD_YES;
		pHD->nr_of_ports			  = 2;
        pHD->edd_type                 = LSA_COMP_ID_EDDI;
        pHD->asic_type                = PSI_ASIC_TYPE_SOC1;
        pHD->rev_nr                   = 0;
        pHD->allow_none_pnio_mautypes = LSA_TRUE;
        pHD->send_clock_factor        = pnioParam.send_clock;       
        pHD->fill_active              = LSA_TRUE;
		pHD->io_configured            = LSA_TRUE;
            
        pHD->irte.use_setting = LSA_TRUE;
        pHD->irte.irt_supported = LSA_TRUE;
        pHD->irte.nr_irt_forwarder = 128;
        pHD->irte.buffer_capacity_use_case = PSI_IRTE_USE_CASE_DEFAULT;
        pHD->irte.support_irt_flex = LSA_TRUE;
        pHD->irte.disable_max_port_delay_check = LSA_FALSE;   
        
        pHD->stdmac.use_setting     = LSA_FALSE;
        pHD->stdmac.gigabit_support = LSA_FALSE;    
            
        // HW port settings
        pHD->hw_port[0].user_nr           = 1;
        pHD->hw_port[0].mrp_ringport_type = PSI_NO_RING_PORT;
        pHD->hw_port[1].user_nr = 2;
        pHD->hw_port[1].mrp_ringport_type = PSI_NO_RING_PORT;
        pHD->hw_port[2].user_nr = 3;
        pHD->hw_port[2].mrp_ringport_type = PSI_NO_RING_PORT;

        // PNIO IF settings common
        pHD->pnio_if.edd_if_id = m_startup.pnd_handle[idx].pnio_if_nr;

        // PNIO IF settings IOC
        pHD->pnio_if.ioc.use_setting           = LSA_TRUE;
        pHD->pnio_if.ioc.nr_of_rt_devices      = PSI_MAX_CL_DEVICES;
        pHD->pnio_if.ioc.nr_of_qv_provider     = 1;
        pHD->pnio_if.ioc.nr_of_qv_consumer     = 16;
        pHD->pnio_if.ioc.nr_of_irt_devices     = PSI_MAX_CL_IRT_DEVICES;
        pHD->pnio_if.ioc.logical_addr_space    = 8192;
        pHD->pnio_if.ioc.nr_of_submod          = 2048;
        pHD->pnio_if.ioc.max_iocr_data_size    = 1440;
        pHD->pnio_if.ioc.max_mcr_data_size     = 256;
        pHD->pnio_if.ioc.max_record_length     = PSI_MAX_CL_RECORD_SIZE;
        pHD->pnio_if.ioc.max_alarm_data_length = pnioParam.max_alarm_data_length;

        // PNIO IF settings OHA
        pHD->pnio_if.oha.use_setting       = LSA_TRUE;
        pHD->pnio_if.oha.vendor_id         = pnioParam.vendor_id;
        pHD->pnio_if.oha.device_id         = pnioParam.device_id;
        pHD->pnio_if.oha.check_ip_enabled  = pnioParam.check_ip_enabled;
        pHD->pnio_if.oha.rema_station_format = OHA_REMA_STATION_FORMAT_ADVANCED;
        result = GetSysDescr(pHD->pnio_if.oha.sys_description);
        PND_ASSERT( result == PNIO_OK );
        result = GetIfDescr(pHD->pnio_if.oha.if_description, m_startup.pnd_handle[idx].pnio_if_nr);
        PND_ASSERT( result == PNIO_OK );
        result = GetPortDescr(&pHD->pnio_if.oha, m_startup.pnd_handle[idx].pnio_if_nr, pHD->nr_of_ports); 
        PND_ASSERT( result == PNIO_OK );

        // PNIO IF settings IP
        pHD->pnio_if.ip.use_setting = LSA_TRUE;
        pHD->pnio_if.ip.nr_of_send  = 32;
        pHD->pnio_if.ip.nr_of_arp   = 32;
        pHD->pnio_if.ip.nr_of_icmp  = 32;
        pHD->pnio_if.ip.nr_of_udp   = 100;
        pHD->pnio_if.ip.nr_of_tcp   = 100;
        pHD->pnio_if.ip.multicast_support_on = LSA_FALSE;

        // PNIO IF settings MRP
        pHD->pnio_if.mrp.use_setting             = LSA_FALSE;
        pHD->pnio_if.mrp.max_instances           = 0;
        pHD->pnio_if.mrp.supported_role          = 0;
        pHD->pnio_if.mrp.default_role_instance0  = (PSI_MRP_ROLE_TYPE)EDD_MRP_ROLE_NONE;
        pHD->pnio_if.mrp.supported_multiple_role = 0;

        // PNIO IF settings NRT
        pHD->pnio_if.nrt.RxFilterUDP_Broadcast = LSA_TRUE;
        pHD->pnio_if.nrt.RxFilterUDP_Unicast   = LSA_TRUE;
		...
		switch ( pLoc[idx].CpSelection )
		{
			case PNIO_CP_SELECT_WITH_PCI_LOCATION: // PCI location
			{
                ...
                pHD->pnio_if.ioc.use_setting = LSA_TRUE;
                pHD->pnio_if.ioc.nr_of_rt_devices = PSI_MAX_CL_DEVICES;
                pHD->pnio_if.ioc.nr_of_qv_provider = 0;
                pHD->pnio_if.ioc.nr_of_qv_consumer = 0;
                pHD->pnio_if.ioc.nr_of_irt_devices = PSI_MAX_CL_IRT_DEVICES;
                pHD->pnio_if.ioc.logical_addr_space = 8192;
                pHD->pnio_if.ioc.nr_of_submod = 2048;
                pHD->pnio_if.ioc.max_iocr_data_size = 1440;
...
/*----------------------------------------------------------------------------*/
LSA_VOID CParamStore::UpdateRema(const PNIO_UINT8* data, PNIO_UINT32 dataLength, PNIO_UINT32 index, PNIO_UINT8** remaData, PNIO_UINT32* bufferLength)
{
	CValueHelper val;
	...
	m_pParamHandlerXML->UpdateRema(m_remaData, remaData, bufferLength);
}
/*lint -e{1762} Member function could be made const */
/*lint -e{1536} Exposing low access member 'CParamStore::m_remaData' */
...
/*lint -e{818} Pointer parameter 'pSysDescr' could be declared as pointing to const */
LSA_UINT32 CParamStore::IsSystemDescriptionValid(PNIO_SYSTEM_DESCR* pSysDescr) const
{
    const CValueHelper val;
    if(pSysDescr == LSA_NULL)
    {
        return PNIO_ERR_PRM_POINTER;
    }

    const LSA_UINT32 delimiterLength = pnd_strlen(SYSTEM_DESCRIPTION_DELIMITER);
    const LSA_UINT32 dotLength = pnd_strlen(SYSTEM_DESCRIPTION_DOT_FOR_FW_VERSION);
    const LSA_UINT32 sysDescrLength = pnd_strlen((LSA_CHAR*)pSysDescr->Vendor) + delimiterLength
                                ...
                                +pnd_strlen(SYSTEM_DESCRIPTION_HW_VERSION_PREFIX) + val.GetDigitCount(pSysDescr->IM_HwRevision) + delimiterLength
                                +pnd_strlen(SYSTEM_DESCRIPTION_FW_VERSION_PREFIX) + 1 
                                ...
                                +pnd_strlen(SYSTEM_DESCRIPTION_PRODUCT_SERIAL_NUMBER_PREFIX) + pnd_strlen((LSA_CHAR*)pSysDescr->ProductSerialNr);
...
/*lint -e{818} Pointer parameter 'pSysDescr' could be declared as pointing to const*/
LSA_UINT32 CParamStore::IsIfDescriptionValid(PNIO_SYSTEM_DESCR* pSysDescr) const
{
    if(pSysDescr == LSA_NULL)
    {
        return PNIO_ERR_PRM_POINTER;
    ...
    LSA_UINT32 ifDescrLength = pnd_strlen((LSA_CHAR*)pSysDescr->Vendor) + pnd_strlen(SYSTEM_DESCRIPTION_DELIMITER)
...
LSA_UINT32 CParamStore::GetSysDescr(LSA_UINT8* pSysDescr)
{
    ...
    if(result == PNIO_OK)
    {
        pnd_sprintf(pSysDescr, "%s%s%s%s%s%s%s%s%s%d%s%s%c%d%s%d%s%d%s%s", 
            ...
            m_systemDescription.ProductSerialNr);
    }

    return result;
}

LSA_UINT32 CParamStore::GetIfDescr(LSA_UINT8* pIfDescr, LSA_UINT16 pnio_if_nr)
{
    LSA_UINT32 result;
    LSA_UINT8 extension[40];
    LSA_UINT8 if_desc[PSI_CFG_OHA_MAX_DESCRIPTION_LEN];

    result = IsIfDescriptionValid(&m_systemDescription);

    if(result == PNIO_OK)
    {
        pnd_sprintf(if_desc, "%s%s%s", m_systemDescription.Vendor, SYSTEM_DESCRIPTION_DELIMITER, m_systemDescription.ProductFamily);
        pnd_sprintf(extension, "%sX%d", ", internal, ", pnio_if_nr); 
   
        if((pnd_strlen((LSA_CHAR*)if_desc) + pnd_strlen((LSA_CHAR*)extension)) < PSI_CFG_OHA_MAX_DESCRIPTION_LEN)
        {
            pnd_sprintf(pIfDescr, "%s%s", if_desc, extension);
        }
    }
...
LSA_UINT32 CParamStore::GetPortDescr(PSI_HD_IF_OHA_INPUT_TYPE* pInput, LSA_UINT16 pnio_if_nr, LSA_UINT16 port_nr)
{
    LSA_UINT32 result;
    LSA_UINT8 extension[40];
    LSA_UINT8 port_desc[PSI_CFG_OHA_MAX_DESCRIPTION_LEN];

    result = IsIfDescriptionValid(&m_systemDescription);

    if(result == PNIO_OK)
    {
        pnd_sprintf(port_desc, "%s%s%s", m_systemDescription.Vendor, SYSTEM_DESCRIPTION_DELIMITER, m_systemDescription.ProductFamily);

        for(int port_id = 1; port_id <= port_nr; port_id++)
        {
            pnd_sprintf(extension, "%sX%d P%d", ", Ethernet Port, ",pnio_if_nr, port_id);

            if((pnd_strlen((LSA_CHAR*)port_desc) + pnd_strlen((LSA_CHAR*)extension)) < PSI_CFG_OHA_MAX_DESCRIPTION_LEN)
            {
                pnd_sprintf(pInput->port_description[port_id-1].description, "%s%s", port_desc, extension);
            }
        }
    }

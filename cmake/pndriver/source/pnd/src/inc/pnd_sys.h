...
LSA_UINT    pnd_strlen(const LSA_CHAR *str);
LSA_INT    pnd_strncmp(const LSA_CHAR *s1, const LSA_CHAR *s2, LSA_UINT n);
...
LSA_INT32 pnd_core_thread_proc(LSA_VOID_PTR_TYPE argument);

/*  applsync     ----------------------------------------------------------*/
LSA_VOID pnd_eddi_alloc_applsync_lock(LSA_VOID);
LSA_VOID pnd_eddi_free_applsync_lock(LSA_VOID);

/*  pnd_eddi_iocc ---------------------------------------------------------*/
LSA_VOID pnd_eddi_alloc_iocc_ch1_lock(LSA_VOID);
LSA_VOID pnd_eddi_alloc_iocc_ch2_lock(LSA_VOID);
LSA_VOID pnd_eddi_free_iocc_ch1_lock(LSA_VOID);
LSA_VOID pnd_eddi_free_iocc_ch2_lock(LSA_VOID);
LSA_BOOL is_pnd_eddi_iocc_ch1_lock_allocated(LSA_VOID);
LSA_BOOL is_pnd_eddi_iocc_ch2_lock_allocated(LSA_VOID);



/* locks       ----------------------------------------------------------*/

#define pnd_alloc_critical_section_prio_protected	eps_alloc_critical_section_prio_protected
...
#define pnd_timer_free     eps_timer_free
#define pnd_timer_start    eps_timer_start
#define pnd_timer_stop     eps_timer_stop

/*---------- fatal --------------------*/

LSA_BOOL PND_FATAL_ERROR_OCCURED( LSA_VOID );

#define PND_FATAL( txt_ )  EPS_FATAL( txt_ )

#ifdef PND_DEBUG
#define PND_ASSERT(cond_) { \
	if (!(cond_)) { \
		EPS_FATAL ("assert: " #cond_); \
	} \
}
#else
#define PND_ASSERT(cond_) { \

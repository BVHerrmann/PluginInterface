...
// CIOCEventHandler
/////////////////////////////////////////////////////////////////////////////
/*lint -e{818} Pointer parameter 'pHandle' could be declared as pointing to const*/
...
static PNIO_ALARM_TYPE PND_ConvertAlarmTypeToHostLibType(LSA_UINT16 AlarmLeType)
{
  PNIO_ALARM_TYPE ret;
  switch(AlarmLeType)
  {
...
LSA_VOID CIOCEventHandler::ArAlarmIndication( CM_UPPER_RQB_PTR_TYPE pRQB )
{
    ...
    CM_UPPER_ALARM_PTR_TYPE pAlarm = pRQB->args.pd.alarm;
    PND_ASSERT( pAlarm != 0 );
    ...
    if (!p_Rqb)
    {
		...
		pAlarm->cm_pnio_err = CM_PNIO_ERR_NONE;
    ...
    else
    {
        ...
        if(pAlarm) {
            ...
            pndAlarm->alarm.ModDiffBlock.ModDiffBlockLength     = pAlarm->u.cl.mod_diff_block_length;
            pndAlarm->alarm.ModDiffBlock.pModDiffBlock          = pAlarm->u.cl.mod_diff_block;
...
LSA_VOID CIOCEventHandler::ArAbortIndication( CM_UPPER_RQB_PTR_TYPE pRQB )
{
    ...
    CM_UPPER_EVENT_PTR_TYPE pEvent = pRQB->args.pd.event;
    PND_ASSERT( pEvent != 0 );
...
LSA_VOID CIOCEventHandler::ArInDataIndication( CM_UPPER_RQB_PTR_TYPE pRQB )
{
    ...
    else
    {
		...
		p_Rqb->args.iob_alarm.alarm.ModDiffBlock.ModDiffBlockLength = pEvent->u.cl.in_data.mod_diff_block_length;
		p_Rqb->args.iob_alarm.alarm.ModDiffBlock.pModDiffBlock = pEvent->u.cl.in_data.mod_diff_block;  
...
LSA_VOID CIOCEventHandler::ArPrmBeginIndication( CM_UPPER_RQB_PTR_TYPE pRQB )
{
    ...
    else
    {
        PND_IOC_TRACE_07( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
            ...
            pEvent->ar_nr,
            pEvent->session_key,
            pEvent->u.cl.prm_begin.api,
            pEvent->u.cl.prm_begin.slot_nr, 
            pEvent->u.cl.prm_begin.subslot_nr, 
            pRQB );
    }
...
LSA_VOID CIOCEventHandler::ArPrmInDataIndication( CM_UPPER_RQB_PTR_TYPE pRQB )
{
    ...
    else
    {
        PND_IOC_TRACE_07( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
            ...
            pEvent->ar_nr,
            pEvent->session_key,
            pEvent->u.cl.prm_indata.api,
            pEvent->u.cl.prm_indata.slot_nr, 
            pEvent->u.cl.prm_indata.subslot_nr,
            pRQB );
...
LSA_VOID CIOCEventHandler::StartDeviceListening( LSA_UINT16 nrOfEvents, LSA_UINT16 nrOfAlarms )
{
    ...
    for ( int lvi = 0; lvi < nrOfEvents; lvi++ )
    {
        ...
        pRQB->args.pd.event->device_nr = (LSA_UINT16)m_pDevice->GetNumber();
    ...
    for ( int lvi = 0; lvi < nrOfAlarms; lvi++ )
    {
        ...
        pRQB->args.pd.alarm->device_nr      = (LSA_UINT16)m_pDevice->GetNumber();
        pRQB->args.pd.alarm->ar_nr          = 0; // don't care
        pRQB->args.pd.alarm->alarm_priority = 0; // don't care
...
/*----------------------------------------------------------------------------*/
LSA_VOID CIOCEventHandler::traceAbortReason( CM_UPPER_CL_AR_ABORT_PTR_TYPE pAbort ) const
{
    PND_ASSERT( pAbort != 0 );
    ...
    case CM_ERROR_CAUSE_MP_CONFIG_INCOMPLETE:
        ...
        PND_IOC_TRACE_06( 0, LSA_TRACE_LEVEL_NOTE, "EC%u: MasterProject, configuration incomplete (e.g. neighbour does not exist because of device-remove); reason(%u), ErrCode(%02x), ErrDecode(%02x), ErrCode1(%02x), ErrCode2(%02x)",
    ...
    case CM_ERROR_CAUSE_OUT_OF_CR_RESOURCES:
        ...
        PND_IOC_TRACE_06( 0, LSA_TRACE_LEVEL_NOTE, "EC%u: Connect, Out of CR resources; reason(%u), ErrCode(%02x), ErrDecode(%02x), ErrCode1(%02x), ErrCode2(%02x)",
    ...
    case CM_ERROR_CAUSE_PDEV_OWNERSHIP:
        ...
        PND_IOC_TRACE_06( 0, LSA_TRACE_LEVEL_NOTE, "EC%u: Connect, Conflicting parameter assignment to the PN interface of the device; reason(%u), ErrCode(%02x), ErrDecode(%02x), ErrCode1(%02x), ErrCode2(%02x)",
    ...
    case CM_ERROR_CAUSE_SYSTEM_REDUNDANCY:
        ...
        PND_IOC_TRACE_06( 0, LSA_TRACE_LEVEL_NOTE, "EC%u: Connect, Conflicting states or parameters in a system redundancy configuration; reason(%u), ErrCode(%02x), ErrDecode(%02x), ErrCode1(%02x), ErrCode2(%02x)",
    ...
    default:
        ...
        PND_IOC_TRACE_06(0, LSA_TRACE_LEVEL_WARN, "EC%u: CM_ERROR_CAUSE is not defined; reason(%u), ErrCode(%02x), ErrDecode(%02x), ErrCode1(%02x), ErrCode2(%02x)",

...
CIOCDevice::CIOCDevice( LSA_UINT16 devNr, PND_HANDLE_PTR_TYPE pHandle, CUserNode *pParent ) : CUserNode( pHandle, pParent )
{
    ...
    m_responseRQB    = 0;
    m_nrOfAlarmRes   = 0;
	m_DiagDeviceState.ErrorCause = 0;
...
/*---------------------------------------------------------------------------*/
LSA_UINT CIOCDevice::GetSubslots( UserNodeVector &list )
{
...
LSA_VOID CIOCDevice::ResetSubslots( LSA_UINT16 state )
{
    ...
    LSA_UINT subslotCount = getNodeList( m_subslotMap, tmpList );
...
/*---------------------------------------------------------------------------*/
LSA_BOOL CIOCDevice::GetDeviceState() const
{
...
/*---------------------------------------------------------------------------*/
LSA_BOOL CIOCDevice::IsIrt() const
{
    return m_isIrt;
}

/*---------------------------------------------------------------------------*/
LSA_VOID CIOCDevice::GetDeviceDiagState(LSA_UINT16 *error_cause, LSA_UINT8 *reason_code, LSA_UINT8 *additional_info) const
{
...
/*---------------------------------------------------------------------------*/
LSA_VOID CIOCDevice::SetDeviceDiagState(LSA_UINT16 error_cause, LSA_UINT8 reason_code, const LSA_UINT8 *additional_info)
{
...
LSA_VOID CIOCDevice::StartUpdateBgzd()
{
    ...
    if ( !m_isShutdown ) // Shutdown in progress
    {
        ...
        LSA_UINT subslotCount = getNodeList( m_subslotMap, objList );

        for ( LSA_UINT sub = 0; sub < subslotCount; sub++ )
        {
...
/*---------------------------------------------------------------------------*/
LSA_UINT16 CIOCDevice::GetVendorID() const
{
    return ( m_vendorID );
}
/*---------------------------------------------------------------------------*/
LSA_UINT16 CIOCDevice::GetDeviceID() const
{
    return ( m_deviceID );
}
/*---------------------------------------------------------------------------*/
LSA_UINT16 CIOCDevice::GetInstanceID() const
{
    return ( m_instance );
}
/*---------------------------------------------------------------------------*/
...
LSA_VOID CIOCDevice::AddDone( LSA_VOID* p_inst, LSA_VOID* p_rb)
{
	...
	else if ( rsp == CM_ERR_PARAM)
	{
		//stop startup with corrupted data error
		pDevice->startupDone(PNIO_ERR_CORRUPTED_DATA);
	}
    else
    {
        // stop startup with error
...
CIOCAr* CIOCDevice::createArFromARCB( LSA_VOID* p_cm, LSA_VOID* p_iodu, UserNodeVector& prov, UserNodeVector& cons )
{
    ...
    pARCB = (CM_UPPER_ARCB_ADD_PTR_TYPE)pADD->arcb;
    PND_ASSERT(pARCB != 0);
...
/*----------------------------------------------------------------------------*/
PND_IO_ADDR_PTR_TYPE CIOCDevice::createAddressList( UserNodeVector &sublist, LSA_BOOL isProvider ) const
{
    PND_IO_ADDR_PTR_TYPE pAddr = 0;

    if ( !sublist.empty() ) // empty subslot list ?
    {
        LSA_UINT32 subslotCount = sublist.size();

        pAddr = (PND_IO_ADDR_PTR_TYPE)pnd_mem_alloc(subslotCount * sizeof(PND_IO_ADDR_TYPE));
        PND_ASSERT( pAddr != 0 );

        // add address settings for each subslot in list
        for ( LSA_UINT16 idx = 0; idx < subslotCount; idx++ )
        {

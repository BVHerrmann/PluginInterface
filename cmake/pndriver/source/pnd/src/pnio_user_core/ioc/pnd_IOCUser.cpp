...
LSA_VOID CIOCUser::Shutdown( LSA_VOID* p_rb )
{
    ...
    {
        m_requestList.clear();
        LSA_UINT deviceCount = getNodeList( m_deviceMap, m_requestList );
...
LSA_VOID CIOCUser::StartUpdateBgzd()
{
    ...
    if ( !m_isShutdown ) // Shutdown in progress
    {
        ...
        LSA_UINT deviceCount = getNodeList( m_deviceMap, devList );

        for ( LSA_UINT idx = 0; idx < deviceCount; idx++ )
        {
...
/*----------------------------------------------------------------------------*/
LSA_VOID CIOCUser::AlarmIndication( LSA_VOID * p_rb ) const
{
   PND_ASSERT(p_rb);
...
/*---------------------------------------------------------------------------*/
PND_IOC_MODE_TYPE CIOCUser::GetMode( LSA_VOID ) const
{
    return m_mode;
}
...
/*----------------------------------------------------------------------------*/
LSA_VOID CIOCUser::AllocateLinkList()
{
    PND_RQB_PTR_TYPE pRQB = (PND_RQB_PTR_TYPE)pnd_mem_alloc(sizeof(PND_RQB_TYPE));
    PND_ASSERT(pRQB);

    PND_IOC_TRACE_01(m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCUser::AllocateLinkList(): Allocate link list, rqb(%08x)",
        pRQB);

    LSA_RQB_SET_OPCODE(pRQB, PND_OPC_LL_ALLOC);

    pRQB->args.ll_alloc.pnd_handle = this->m_pndHandle;
    pRQB->args.ll_alloc.pnio_err = PNIO_OK;

    pnd_pnio_user_request_iobc(pRQB, this, CIOCUser::AllocateLinkListDone);
}

/*----------------------------------------------------------------------------*/
LSA_VOID CIOCUser::AllocateLinkListDone(LSA_VOID* p_inst, LSA_VOID* p_rb)
{
    ...
    PND_ASSERT(pIOC != 0);
    PND_ASSERT(pRQB != 0);
    PND_ASSERT(LSA_RQB_GET_OPCODE(pRQB) == PND_OPC_LL_ALLOC);

    LSA_UINT32 pnioErr = pRQB->args.ll_alloc.pnio_err;

    pIOC->startupDone(pnioErr);

    pnd_mem_free(pRQB);
}

/*----------------------------------------------------------------------------*/
LSA_VOID CIOCUser::FreeLinkList()
{
    PND_RQB_PTR_TYPE pRQB = (PND_RQB_PTR_TYPE)pnd_mem_alloc(sizeof(PND_RQB_TYPE));
    PND_ASSERT(pRQB);

    PND_IOC_TRACE_01(m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCUser::FreeLinkList(): FreeLinkList link list, rqb(%08x)",
        pRQB);

    LSA_RQB_SET_OPCODE(pRQB, PND_OPC_LL_FREE);

    pRQB->args.ll_free.pnd_handle = this->m_pndHandle;
    pRQB->args.ll_free.pnio_err = PNIO_OK;

    pnd_pnio_user_request_iobc(pRQB, this, CIOCUser::FreeLinkListDone);
}

/*----------------------------------------------------------------------------*/
LSA_VOID CIOCUser::FreeLinkListDone(LSA_VOID* p_inst, LSA_VOID* p_rb)
{
    CIOCUser        *pIOC = static_cast<CIOCUser*>(p_inst);
    PND_RQB_PTR_TYPE pRQB = static_cast<PND_RQB_PTR_TYPE>(p_rb);
    ...
    PND_ASSERT(pIOC != 0);
    PND_ASSERT(pRQB != 0);
    PND_ASSERT(LSA_RQB_GET_OPCODE(pRQB) == PND_OPC_LL_FREE);

    if (pIOC->m_isShutdown) // shutdown in progress ?
    {
        LSA_UINT32 pnioErr = pRQB->args.ll_free.pnio_err;

        // signal shutdown complete
        pIOC->shutdownDone(pnioErr);
    }
...
LSA_VOID CIOCUser::CreateCmCl()
{
    ...
    pRQB->args.cl.create->IOXS_Required = LSA_FALSE;  // Every PNDriver variant supports discard IOXS
...
LSA_VOID CIOCUser::StartupDone( LSA_VOID* p_inst, LSA_VOID* p_rb )
{
    ...
    if ( pRQB->args.pnuc_internal.pnio_err != PNIO_OK )  // internal error detected
    {
		...
		//call startupDone with pnio_err
		pIOC->startupDone(pRQB->args.pnuc_internal.pnio_err);
...
LSA_VOID CIOCUser::CreateCmClDone( LSA_VOID* p_inst, LSA_VOID* p_rb)
{
    ...
    LSA_UINT32	hardware_type = pRQB->args.cl.create->hardware_type;
    pnd_cm_free_rqb( pRQB, &pUser->m_pndHandle );
    ...
    if ( rsp == CM_OK )
    {
        ...
        if ( pUser->m_isStartup ) // automatic startup ?
        {
            ...
            pUser->setEddHWType( hardware_type );

            pUser->m_requestList.clear();
            LSA_UINT deviceCount = pUser->getNodeList( pUser->m_deviceMap, pUser->m_requestList );
...
LSA_VOID CIOCUser::SetRunDone( LSA_VOID* p_inst, LSA_VOID* p_rb)
{
   ...
   if ( pUser->m_mode == IOC_OFFLINE ) // new state is OFFLINE
   {
      ...
      PND_IOC_TRACE_02( pUser->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
         ...
         pUser->m_modeRQB->args.iob_set_mode.mode, deviceCount);

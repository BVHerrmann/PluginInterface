...
#define LTRC_ACT_MODUL_ID   3209
#define PND_MODULE_ID       3209
...
CIOCSlot::CIOCSlot( LSA_UINT32 api, LSA_UINT16 slot, LSA_UINT16 subslot_ladd, 
                          PND_HANDLE_PTR_TYPE pHandle, CUserNode* parent) : CUserNode( pHandle, parent )
{
    // Initialize key values
    m_api       = api;
    m_slotNr    = slot;
    m_subslotNr = subslot_ladd;

    // basic subslot_ladd attributes
    m_slotIdent    = 0;
    m_subslotIdent = 0;
...
/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::Create( LSA_VOID* pData )
{
    ...
    PND_IOC_TRACE_07( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::Create(): API/Slot/Subslot(%#x/%#x/%#x) modLADDR(%u) subLADDR(%u) modIdent(%#x) subIdent(%#x)",
        m_api, 
        m_slotNr, 
        m_subslotNr, 
        pSUB->slot_laddr,
    ...
    PND_IOC_TRACE_08( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::Create(): API/Slot/Subslot(%#x/%#x/%#x) subProp(%#x) inpLen(%u) iBaseAddr(%u) outLen(%u) qBaseAddr(%u)",
        m_api, 
        m_slotNr, 
        m_subslotNr, 
...
string CIOCSlot::GetKey()
{
    string retStr = CIOCSlot::GetKey( m_api,m_slotNr,m_subslotNr) ;
    return ( retStr );
}

/*----------------------------------------------------------------------------*/
LSA_UINT CIOCSlot::GetNumber()
{
    return( m_subslotNr );
}

/*----------------------------------------------------------------------------*/
LSA_HANDLE_TYPE CIOCSlot::GetPnioHandle()
{
...
string CIOCSlot::GetKey(const LSA_UINT32 api, const LSA_UINT16 slot, const LSA_UINT16 subSlot )
{
...
/*----------------------------------------------------------------------------*/
LSA_UINT32 CIOCSlot::GetApiNr() const
{
    return( m_api );
}

/*----------------------------------------------------------------------------*/
LSA_UINT16 CIOCSlot::GetSlotNr() const
{
    return( m_slotNr );
}

/*----------------------------------------------------------------------------*/
LSA_UINT16 CIOCSlot::GetSubslotNr()
{
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 CIOCSlot::GetSlotLogAddr() const
{
    return ( m_slotLaddr );
}

/*----------------------------------------------------------------------------*/
LSA_UINT16 CIOCSlot::GetSubslotLogAddr() const
{
    return ( m_subslotLaddr );
}

/*----------------------------------------------------------------------------*/
LSA_UINT32 CIOCSlot::GetSlotIdent() const
{
    return ( m_slotIdent );
}

/*----------------------------------------------------------------------------*/
LSA_UINT32 CIOCSlot::GetSubslotIdent() const
{
    return ( m_subslotIdent );
}

/*----------------------------------------------------------------------------*/
PND_IO_ADDR_TYPE CIOCSlot::GetAddressForInput() const
{
    return ( m_input );
}

/*----------------------------------------------------------------------------*/
PND_IO_ADDR_TYPE CIOCSlot::GetAddressForOutput() const
{
    return ( m_output );
}
...
LSA_VOID CIOCSlot::SetState( LSA_UINT16 state )
{
...
/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::SetInputOffset( LSA_UINT16 offset )
{
    m_input.offset_IO_data = offset;
}

/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::SetInputOffsetIOCS( LSA_UINT16 offset )
{
    m_input.offset_IOCS = offset;
}

/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::SetOutputOffset( LSA_UINT16 offset )
{
    m_output.offset_IO_data = offset;
}

/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::SetOutputOffsetIOCS( LSA_UINT16 offset )
{
...
LSA_VOID CIOCSlot::UpdateFromAlarm( LSA_BOOL hasDiag, LSA_UINT32 maintenanceStatus )
{
...
LSA_VOID CIOCSlot::RecordRead( PND_RQB_PTR_TYPE p_rb )
{
    ...
    PND_IOC_TRACE_09( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::RecordRead(): IOC READ record, dev(%u) ar(%u) sesKey(%u) api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u) RQB(0x%08x)", 
        devNr, 
        arNr, 
        sesKey,
        m_api, 
    ...
    eps_user_request_lower( pRQB, this, CIOCSlot::RecordReadDone );
}

/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::RecordWrite(PND_RQB_PTR_TYPE p_rb)
{
    LSA_HANDLE_TYPE  handle = GetPnioHandle();
    ...
    PND_IOC_TRACE_09( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::RecordWrite(): IOC WRITE record, dev(%u) ar(%u) sesKey(%u) api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u) RQB(0x%08x)", 
        devNr, 
        arNr, 
        sesKey,
        m_api, 
    ...
    pRQB->args.cl.write->rec.data_length  = CM_RECORD_OFFSET+ PNIOI_DREC_MAX_SIZE;
    ...
    eps_user_request_lower( pRQB, this, CIOCSlot::RecordWriteDone );
}

/*---------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::DeviceActivate( PND_RQB_PTR_TYPE p_rb )
{
    PND_ASSERT(p_rb != 0);

    CIOCDevice* ParentDevice = CIOCSlot::getDevice();
...
/*---------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::Activate(PND_RQB_PTR_TYPE p_rb)
{
    PND_DEV_ACTIVATE_PTR_TYPE pActivate = &p_rb->args.iob_dev_act;
    ...
    PND_IOC_TRACE_02( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::Activate(): CM-CL DeviceActive.REQ, nr(%u) pRQB(0x%08x)", 
        ParentDeviceNr, pRQB );
    ...
    eps_user_request_lower( pRQB, this, CIOCSlot::ActivateDone );
}

/*---------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::Passivate(PND_RQB_PTR_TYPE p_rb)
{
    PND_DEV_ACTIVATE_PTR_TYPE pActivate = &p_rb->args.iob_dev_act;
    LSA_HANDLE_TYPE handle = GetPnioHandle();
    CIOCDevice* ParentDevice = CIOCSlot::getDevice();
    ...
    PND_IOC_TRACE_02( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::Passivate(): CM-CL DevicePassivate.REQ, nr(%u) pRQB(0x%08x)", 
        ParentDeviceNr, pRQB );
    ...
    eps_user_request_lower( pRQB, this, CIOCSlot::PassivateDone );
}
...
LSA_VOID CIOCSlot::RecordReadDone( LSA_VOID* p_inst, LSA_VOID* p_rb )
{
    CIOCSlot *pSub          = static_cast<CIOCSlot*>(p_inst);
    ...
    PND_IOC_TRACE_04(pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::RecordReadDone(): IOC Record Read done, rsp(%#x) pnio_stat(%#x) dev(%u) RQB(0x%08x)",
        rsp,
        pnioErr,
    ...
    PND_IOC_TRACE_06( pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::RecordReadDone():    api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u/%x)", 
...
/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::RecordWriteDone( LSA_VOID* p_inst, LSA_VOID* p_rb )
{
    CIOCSlot *pSub          = static_cast<CIOCSlot*>(p_inst);
    CM_UPPER_RQB_PTR_TYPE pRQB = static_cast<CM_UPPER_RQB_PTR_TYPE>(p_rb);
    ...
    PND_IOC_TRACE_05(pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::RecordWriteDone(): IOC Record Write done, rsp(%#x) pnioErr(%#x) dev(%u) sesKey(%u) RQB(0x%08x)",
        rsp,
        pnioErr,
    ...
    PND_IOC_TRACE_05( pSub->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::RecordWriteDone():    api(%#x) slot(%#x) sub(%#x) recIdx(%#x) dataSize(%u)", 
        ...
        // Copy CM PNIO state fields to PND RQB
    pPndRQB->args.rec_write.err.AddValue1 = pRQB->args.cl.read->rec.ret_val_1;
    pPndRQB->args.rec_write.err.AddValue2 = pRQB->args.cl.read->rec.ret_val_2;
...
/*---------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::ActivateDone( LSA_VOID* p_inst, LSA_VOID* p_rb)
{
    CIOCSlot *pSubSlot        = static_cast<CIOCSlot*>(p_inst);
    CM_UPPER_RQB_PTR_TYPE pRQB   = static_cast<CM_UPPER_RQB_PTR_TYPE>(p_rb);
    ...
    PND_IOC_TRACE_04( pSubSlot->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::ActivateDone(): CM-CL DeviceActive.CNF, rsp(%u/%#x) nr(%u) pRQB(0x%08x)",
        rsp, rsp,
...
/*---------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::PassivateDone( LSA_VOID* p_inst, LSA_VOID* p_rb)
{
    CIOCSlot *pSubSlot        = static_cast<CIOCSlot*>(p_inst);
    CM_UPPER_RQB_PTR_TYPE pRQB   = static_cast<CM_UPPER_RQB_PTR_TYPE>(p_rb);
    ...
    PND_IOC_TRACE_04( pSubSlot->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
        "CIOCSlot::PassivateDone(): CM-CL DevicePassivate.CNF, rsp(%u/%#x) nr(%u) pRQB(0x%08x)",
        rsp, rsp,
        pRQB->args.cl.dev_remove->device_nr, 
        pRQB );
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 CIOCSlot::GetDeviceNr()
{
    CIOCDevice* pDevice = dynamic_cast<CIOCDevice*>( GetParent() );
    PND_ASSERT( pDevice != 0 );
...
/*----------------------------------------------------------------------------*/
LSA_VOID CIOCSlot::setIOProperties()
{
    ...
    default:
        {
            PND_FATAL( "CIOCSlot::setIOProperties(): unexpected submod type value" );
        }
        break;
    }

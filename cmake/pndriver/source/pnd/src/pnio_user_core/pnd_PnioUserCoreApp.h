...
private:
    ...
    IUserStoreFactory* m_pUserStoreFactory;
    IPndAdapter* m_pPndAdapter;

public:
    CPnioUserCoreApp(IUserStoreFactory*, IPndAdapter*);
	virtual ~CPnioUserCoreApp();

	// PND request Methodes
	void ControllerOpen( PND_RQB_PTR_TYPE pRQB ) const;
	void ControllerClose( PND_RQB_PTR_TYPE pRQB ) const;
	void AlarmProvide( PND_RQB_PTR_TYPE pRQB ) const;
	void AlarmReProvide( PND_RQB_PTR_TYPE pRQB ) const;
	void SetMode(PND_RQB_PTR_TYPE pRQB) const;
	void DiagnosticRequest( PND_RQB_PTR_TYPE pRQB );
	void SetIpAndNos( PND_RQB_PTR_TYPE p_rb ) const;
	void RemaRead( PND_RQB_PTR_TYPE p_rb ) const;
	void InterfaceOpen( PND_RQB_PTR_TYPE pRQB ) const;
	void InterfaceClose( PND_RQB_PTR_TYPE pRQB ) const;
	void RemaProvide( PND_RQB_PTR_TYPE pRQB ) const;
    void RemaReProvide( PND_RQB_PTR_TYPE pRQB ) const;
    void ParamWrite( PND_RQB_PTR_TYPE pRQB ) const;   
#ifdef PND_CFG_ISO_SUPPORT
    void SyncStateChange( PND_RQB_PTR_TYPE pRQB ) const;
    void StartSynchronization( PND_RQB_PTR_TYPE pRQB ) const;
    void SetRxLocalTransferEndNs( PND_RQB_PTR_TYPE pRQB ) const;
#endif
    ...
    void GetLogAddrList( LSA_VOID* p_rb ) const;
...
private:
	...
	LSA_VOID AddSubmoduleConfigToBuffer( CUserNode *pUser, LSA_UINT8 *pBuffer ) const;
	LSA_VOID AddDiagDeviceDiagnosticToBuffer( CUserNode *pUser, LSA_VOID *pBuffer ) const;
	LSA_VOID AddConfNameAddrInfoToBuffer( PND_RQB_PTR_TYPE pRQB, LSA_VOID *pBuffer) const;

	LSA_VOID UpdateRema( PND_RQB_PTR_TYPE p_rb ) const;
};

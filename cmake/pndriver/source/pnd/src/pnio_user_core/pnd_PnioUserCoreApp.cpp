...
/////////////////////////////////////////////////////////////////////////////
CPnioUserCoreApp::CPnioUserCoreApp(IUserStoreFactory* pUserStoreFactory, IPndAdapter* pPndAdapter) : CUserNode(),
m_pUserStoreFactory(pUserStoreFactory),
...
CPnioUserCoreApp::~CPnioUserCoreApp()
{
    ...
    if(m_pUserStoreFactory != NULL)
    {
        delete m_pUserStoreFactory;
        m_pUserStoreFactory = NULL;
    }
...
/*----------------------------------------------------------------------------*/
void CPnioUserCoreApp::ControllerOpen( PND_RQB_PTR_TYPE pRQB ) const
{
...
/*----------------------------------------------------------------------------*/
void CPnioUserCoreApp::ControllerClose( PND_RQB_PTR_TYPE pRQB ) const
{
...
/*----------------------------------------------------------------------------*/
void CPnioUserCoreApp::AlarmProvide( PND_RQB_PTR_TYPE pRQB ) const
{
...
/*----------------------------------------------------------------------------*/
void CPnioUserCoreApp::AlarmReProvide( PND_RQB_PTR_TYPE pRQB ) const
{
    PND_ALARM_PTR_TYPE pAlarm = &pRQB->args.iob_alarm;
...
LSA_VOID CPnioUserCoreApp::DiagnosticRequest( PND_RQB_PTR_TYPE pRQB )
{
	...
	m_pPndAdapter->pnioUserRequestIobcDone( pRQB );
}
...
LSA_VOID CPnioUserCoreApp::GetDiagConfigSubmoduleList( PND_RQB_PTR_TYPE pRQB )
{
	...
	for(logAddrCount = 0; logAddrCount < PND_LADDR_MAX; logAddrCount++)
	{
		if(m_logAddrList[cpIdx].laddr_type[logAddrCount] == IOCSUBSLOT)
		{
            ...
            pSubslot = static_cast<CIOCSubslot*>(m_logAddrList[cpIdx].laddr_ref[logAddrCount]);
            
			if (pSubslot->isMixedSubmodule()) // InOut Module, 2x entry added
            {
                subslotCount++;
            }
		}
    ...
    if (subslotCount > 0)
    {
        ...
        pBuffer = (PNIO_UINT8 *)m_pPndAdapter->memAlloc(bufferLen);
        PND_ASSERT(pBuffer);
        ...
        for (logAddrCount = 0; logAddrCount < PND_LADDR_MAX; logAddrCount++)
        {
            if (m_logAddrList[cpIdx].laddr_type[logAddrCount] == IOCSUBSLOT)
            {
                ...
                AddSubmoduleConfigToBuffer(pSubslot, pBuffer);

                if (pSubslot->isMixedSubmodule()) // InOut Module, 2x entry added
                {
                    pBuffer += 2 * (LSA_UINT8)sizeof(PNIO_CTRL_DIAG_CONFIG_SUBMODULE);
                }
                else
                {
    ...
    {
        pDiagnosticReq->pDiagDataBuffer = NULL;
        pDiagnosticReq->DiagDataBufferLen = 0;
    }
...
LSA_VOID CPnioUserCoreApp::GetDiagDeviceDiagnostic( PND_RQB_PTR_TYPE pRQB )
{
	...
	pBuffer   = m_pPndAdapter->memAlloc( bufferLen );
	PND_ASSERT( pBuffer );
...
/*----------------------------------------------------------------------------*/
LSA_VOID CPnioUserCoreApp::AddSubmoduleConfigToBuffer(CUserNode *pUser, LSA_UINT8 *pBuffer) const
{
	...
	pConfSubmod->Address.IODataType = outputAddress.length_IO_data != 0 ? PNIO_IO_OUT : PNIO_IO_IN;
	
	pConfSubmod->HwIdentifier = pSubslot->GetSubslotLogAddr();  // LADDR

	if (pConfSubmod->Address.IODataType == PNIO_IO_OUT)
	{
		pConfSubmod->Address.u.Addr = outputAddress.IO_base_addr;
	}
	else  // Input submodule or Dataless submodule
	{
		pConfSubmod->Address.u.Addr = inputAddress.IO_base_addr;
	}
	//pConfSubmod->Address.u.Reserved[5] = 0;

	pConfSubmod->DataLength      = outputAddress.length_IO_data != 0 ? outputAddress.length_IO_data : inputAddress.length_IO_data;
	pConfSubmod->DataType        = pSubslot->getDevice()->IsIrt() == PNIO_TRUE ? PNIO_DATA_IRT : PNIO_DATA_RT;  // PNIO_DATA_RT - PNIO_DATA_IRT
    ...
    if (inputAddress.length_IO_data != 0 && outputAddress.length_IO_data != 0) // InOut
    {
        pConfSubmod = (PNIO_CTRL_DIAG_CONFIG_SUBMODULE *)(pBuffer + sizeof(PNIO_CTRL_DIAG_CONFIG_SUBMODULE));

        pConfSubmod->Address.AddrType = PNIO_ADDR_LOG;                // PNIO_ADDR_LOG - PNIO_ADDR_GEO
        pConfSubmod->Address.IODataType = PNIO_IO_IN;  

        pConfSubmod->HwIdentifier = pSubslot->GetSubslotLogAddr();  // LADDR

        pConfSubmod->Address.u.Addr = inputAddress.IO_base_addr;

        pConfSubmod->DataLength = inputAddress.length_IO_data;
        pConfSubmod->DataType = pSubslot->getDevice()->IsIrt() == PNIO_TRUE ? PNIO_DATA_IRT : PNIO_DATA_RT;// PNIO_DATA_RT - PNIO_DATA_IRT
        pConfSubmod->ComType = PNIO_COM_UNICAST;		 // PNIO_COM_DIRECT_DATA_EX - PNIO_COM_UNICAST 
        pConfSubmod->Api = pSubslot->GetApiNr();
...
/*----------------------------------------------------------------------------*/
LSA_VOID CPnioUserCoreApp::AddDiagDeviceDiagnosticToBuffer( CUserNode *pUser, LSA_VOID *pBuffer ) const
{
...
LSA_VOID CPnioUserCoreApp::AddConfNameAddrInfoToBuffer( PND_RQB_PTR_TYPE pRQB, LSA_VOID *pBuffer ) const
{
...
/*----------------------------------------------------------------------------*/
LSA_VOID CPnioUserCoreApp::SetIpAndNos( PND_RQB_PTR_TYPE p_rb ) const
{
	LSA_UINT16 cpIdx = p_rb->args.set_ip_and_nos.pnd_handle.cp_nr;
	PND_ASSERT( m_userStore[cpIdx] != 0 );

	// Get OHAUser to get the type_of_station
    IUserStore *pStore =  dynamic_cast<IUserStore*>( m_userStore[cpIdx] );
	PND_ASSERT( pStore != 0 );

	IOHAUser* pOHAUser = dynamic_cast<IOHAUser*>( pStore->GetUser( OHA_USER ));
	PND_ASSERT( pOHAUser != 0 );
...
/*----------------------------------------------------------------------------*/
LSA_VOID CPnioUserCoreApp::RemaRead( PND_RQB_PTR_TYPE p_rb ) const
{
	UpdateRema( p_rb );
}

/*----------------------------------------------------------------------------*/
LSA_VOID CPnioUserCoreApp::UpdateRema( PND_RQB_PTR_TYPE p_rb ) const
{
...
/*----------------------------------------------------------------------------*/
LSA_VOID CPnioUserCoreApp::SetMode( PND_RQB_PTR_TYPE p_rb ) const
{
...
/*----------------------------------------------------------------------------*/
void CPnioUserCoreApp::InterfaceOpen( PND_RQB_PTR_TYPE pRQB ) const
{
    PND_IOB_CORE_OPEN_PTR_TYPE pOpen = &pRQB->args.iob_open;
...
/*----------------------------------------------------------------------------*/
void CPnioUserCoreApp::InterfaceClose( PND_RQB_PTR_TYPE pRQB ) const
{
    PND_IOB_CORE_CLOSE_PTR_TYPE pClose = &pRQB->args.iob_close;
...
/*----------------------------------------------------------------------------*/
void CPnioUserCoreApp::RemaProvide( PND_RQB_PTR_TYPE pRQB ) const
{
...
/*----------------------------------------------------------------------------*/
void CPnioUserCoreApp::RemaReProvide( PND_RQB_PTR_TYPE pRQB ) const
{
	PND_REMA_DATA_PTR_TYPE pRema = &pRQB->args.rema_data;
...
/*----------------------------------------------------------------------------*/
void CPnioUserCoreApp::ParamWrite( PND_RQB_PTR_TYPE pRQB ) const
{
...
#ifdef PND_CFG_ISO_SUPPORT
void CPnioUserCoreApp::SyncStateChange( PND_RQB_PTR_TYPE pRQB ) const
{
    PND_SYNC_STATE_CHANGE_TYPE *pSyncState = &pRQB->args.sync_change;
    PND_ASSERT( pSyncState != 0 );
    
    LSA_UINT16 cpIdx = pSyncState->pnd_handle.cp_nr;
    PND_ASSERT(m_userStore[cpIdx] != 0);

    CUserStore *pStore = dynamic_cast<CUserStore*>(m_userStore[cpIdx]);
    PND_ASSERT(pStore != 0);

    CISOUser* pISO = dynamic_cast<CISOUser*>(pStore->GetUser(ISO_USER));

    if(pISO != 0)
    {
        pISO->syncStateChanged( pRQB );   
    }
    else
    {
        PND_PNUC_TRACE_00( 0, LSA_TRACE_LEVEL_NOTE_HIGH,
            "CPnioUserCoreApp::SyncStateChange(): Config XML does not support IsoM");
    }
}

LSA_VOID CPnioUserCoreApp::StartSynchronization( PND_RQB_PTR_TYPE p_rb ) const
{
    PND_SET_MODE_PTR_TYPE pSetMode = &p_rb->args.iob_set_mode;
    ...
    // Get ISO User for PND Handle and set sync state according to the mode
    CISOUser* pISO = dynamic_cast<CISOUser*>( pStore->GetUser( ISO_USER ));

    if(pISO != 0 )
    {
        PNIO_UINT32 syncResult = pISO->setMode(pSetMode->mode);
        if(syncResult != PNIO_OK)
        {
            pSetMode->pnio_err = syncResult;
        }
    }
    else
    {
        PND_PNUC_TRACE_00( 0, LSA_TRACE_LEVEL_NOTE_HIGH,
            "CPnioUserCoreApp::StartSynchronization(): Config XML does not support IsoM");
    }
    
    //return RQB back to PND User Core
    pnd_pnio_user_request_iobc_done( p_rb ); 
}

LSA_VOID CPnioUserCoreApp::SetRxLocalTransferEndNs(PND_RQB_PTR_TYPE p_rb) const
{
    PND_SET_RX_LOCAL_TRANSFER_END_NS_TYPE* pSetRxLocalTransferEndNs = &p_rb->args.set_rx_local_transfer_end_ns;

    LSA_UINT16 cpIdx = pSetRxLocalTransferEndNs->pnd_handle.cp_nr;
    ...
    // Get ISO User for PND Handle and set RxLocalTransferEndNs
    CISOUser* pISO = dynamic_cast<CISOUser*>(pStore->GetUser(ISO_USER));

    if (pISO != 0)
    {
        pISO->setRxLocalTransferEndNs(pSetRxLocalTransferEndNs->rxLocalTransferEndNs);
    }
    else
    {
        PND_PNUC_TRACE_00(0, LSA_TRACE_LEVEL_NOTE_HIGH,
            "CPnioUserCoreApp::SetRxLocalTransferEndNs(): Config XML does not support IsoM");
    }
}
...
LSA_VOID CPnioUserCoreApp::Startup( LSA_VOID* p_rb )
{
         ...
         ( pStart->startup_param.nr_of_cp <= PSI_CFG_MAX_HD_CNT ))
    {
        ...
        for ( LSA_UINT16 idx = 0; idx < pStart->startup_param.nr_of_cp; idx++ )
        {
            ...
            IUserStore* pUserStore = m_pUserStoreFactory->create(&m_pndPnioHandle[idx], &m_logAddrList[idx], this);
        ...
        PND_RQB_PTR_TYPE pRQBLocal = (PND_RQB_PTR_TYPE)m_pPndAdapter->memAlloc(sizeof( PND_RQB_TYPE ));
...
LSA_VOID CPnioUserCoreApp::GetLogAddrList( void* p_rb ) const
{
    PND_RQB_PTR_TYPE pRQB = (PND_RQB_PTR_TYPE)p_rb;
    PND_ASSERT( p_rb != 0 );
...
LSA_VOID CPnioUserCoreApp::StartupDone( void* p_inst, void* p_rb )
{
    ...
    if ( pRQB->args.pnuc_internal.pnio_err != PNIO_OK )  // internal error detected
    {
        ...
        // Delete local RQB
        pApp->m_pPndAdapter->memFree( pRQB );

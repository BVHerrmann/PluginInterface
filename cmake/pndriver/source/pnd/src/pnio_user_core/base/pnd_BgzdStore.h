...
public:
	...
	LSA_VOID Reset();
	LSA_VOID UpdateBgzdInfo( CBgzdInfoStore& bgzdInfo ) const;
	...
	LSA_BOOL             IsNotPlugged() const;
	PND_BGZD_STATE_TYPE  GetBgzdState() const;
	string               GetBgzdSubslot() const;
	string               GetBgzdUser() const;

	// Accessor for summary BGZD
	LSA_BOOL IsOkState() const;
	LSA_BOOL IsDeactivatedState() const;
	LSA_BOOL IsNotExistsState() const;
	LSA_BOOL IsNotAvaliableState() const;
	LSA_BOOL IsFaultyState() const;

private:
	LSA_VOID dumpBgzd() const;
};
...
public:
	...
	LSA_VOID Shutdown();
	LSA_VOID UpdateLED();
	LSA_VOID SetLED( const PND_LED_STORE_TYPE& led, LSA_BOOL state );

	// Class methodes for timer (toggle on/off)
	static LSA_VOID EventTimeout( LSA_TIMER_ID_TYPE timer_id, LSA_USER_ID_TYPE user_id );

private:
	PND_LED_STATE_TYPE getLedStateBF() const;
	PND_LED_STATE_TYPE getLedStateSF() const;
	PND_LED_STATE_TYPE getLedStateMAINT() const;
	LSA_BOOL isLedOn( const PND_LED_STATE_TYPE& ledState ) const;
};

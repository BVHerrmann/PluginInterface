...
/*----------------------------------------------------------------------------*/
string CValueHelper::GetError() const
{
	return ( ( m_lastError + '\n') );
}
...
LSA_BOOL CValueHelper::IsMACValid( string value )
{
	...
	LSA_CHAR *pBuf = new LSA_CHAR[value.length() + 1];
	pnd_strcpy(pBuf, value.c_str());
...
LSA_BOOL CValueHelper::IsUUIDValid( string value )
{
	...
	LSA_CHAR *cstr = new LSA_CHAR[value.length() + 1];
	pnd_strcpy(cstr, value.c_str());
	if (( value.size() != 36 ) ||                                                      // invalid size ?
		( clrpc_uuid_from_string( &tmpUUID, (LSA_UINT8*)cstr) != CLRPC_OK )) // invalid format ?
	{
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::IsNotEmptyMAC( const LSA_UINT8 *pValue )
{
	LSA_BOOL result = LSA_TRUE;
...
LSA_BOOL CValueHelper::String2Int( string value, LSA_INT *pIntVal ) const
{
	LSA_INT convVal;
	LSA_UINT nr;

	LSA_BOOL result = LSA_FALSE;
	LSA_UINT strSize = value.size();
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2UINT32( string value, LSA_UINT32 *pVal ) const
{
	LSA_INT convVal = 0;
	LSA_BOOL result = LSA_FALSE;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2UINT16( string value, LSA_UINT16 *pVal ) const
{
	LSA_INT convVal = 0;
	LSA_BOOL result = LSA_FALSE;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2UINT8( string value, LSA_UINT8 *pVal ) const
{
	LSA_INT convVal = 0;
	LSA_BOOL result = LSA_FALSE;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2BOOL( string value, LSA_BOOL *pVal ) const
{
	LSA_INT convVal = 0;
	LSA_BOOL result = LSA_FALSE;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2Location( string cpID, PSI_PCI_LOCATION_PTR_TYPE pLocId ) const
{
...
LSA_BOOL CValueHelper::String2MAC( string macStr, LSA_UINT8 *pMac )
{
	...
	if ( IsMACValid ( macStr ))  //  // MAC format valid ?
	{
		...
		LSA_CHAR *pBuf = new LSA_CHAR[macStr.length() + 1];
		pnd_strcpy(pBuf, macStr.c_str());
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2IP( string ipStr, LSA_UINT32* pIpVal ) const
{
	LSA_BOOL result = LSA_FALSE;
	...
	if ( !ipStr.empty()) // Format size ?
	{
		...
		LSA_CHAR *pBuf = new LSA_CHAR[ipStr.length() + 1];
		pnd_strcpy(pBuf, ipStr.c_str());
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2UUID( string uuidStr, LSA_VOID* pUUID ) const
{
	LSA_BOOL result = LSA_FALSE;
	...
	if ((!uuidStr.empty())  &&     // Format size ?
		( uuidStr.size() == 36 ))
	{
		LSA_CHAR *cstr = new LSA_CHAR[uuidStr.length() + 1];
		pnd_strcpy(cstr, uuidStr.c_str());

		// try to convert it to a CLRPC UUID
		LSA_UINT16 ret = clrpc_uuid_from_string( (CLRPC_UUID_PTR_TYPE)pUUID, (LSA_UINT8*)cstr );
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2EddCompID( string value, LSA_UINT16* pEddCompID ) const
{
	LSA_BOOL result     = LSA_FALSE;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2PortMediaType( string value, PSI_PORT_MEDIA_TYPE *pMediaType ) const
{
	LSA_BOOL result = LSA_FALSE;
	LSA_INT iniType = -1;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2RingPortType( string value, PSI_RINGPORT_TYPE *pRingPortType ) const
{
	LSA_BOOL result = LSA_FALSE;
	LSA_INT iniType = -1;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2MrpRoleType( string value, PSI_MRP_ROLE_TYPE *pRoleType ) const
{
	LSA_BOOL result = LSA_FALSE;
	LSA_INT iniType = -1;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2LdRunsType( string value, PSI_LD_RUNS_ON_TYPE *pLdRunsType ) const
{
	LSA_BOOL result = LSA_FALSE;
	LSA_INT iniType = -1;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2EddType( string value, LSA_UINT16 *pEddType ) const
{
	LSA_BOOL result = LSA_FALSE;
	LSA_INT iniType = -1;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2AsicType( string value, LSA_UINT16 *pAsicType ) const
{
	LSA_BOOL result = LSA_FALSE;
	LSA_INT iniType = -1;
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CValueHelper::String2ImSoftwareVersion( string value, PSI_IM_SW_VERSION_PTR_TYPE pImID ) const
{
...
string CValueHelper::UINT32ToString( const LSA_UINT32 val ) const
{
	ostringstream resStr;
	resStr << val;
...
/*----------------------------------------------------------------------------*/
string CValueHelper::UINT16ToString( const LSA_UINT16 val ) const
{
	ostringstream resStr;
	resStr << val;
...
/*----------------------------------------------------------------------------*/
string CValueHelper::UINT8ToString( LSA_UINT8 val ) const
{
...
/*----------------------------------------------------------------------------*/
string CValueHelper::INT32ToString( LSA_INT32 val ) const
{
	ostringstream resStr;
	resStr << val;
...
/*----------------------------------------------------------------------------*/
string CValueHelper::INT16ToString( LSA_INT16 val ) const
{
	ostringstream resStr;
	resStr << val;
...
/*----------------------------------------------------------------------------*/
string CValueHelper::INT8ToString( LSA_INT8 val ) const
{
...
/*----------------------------------------------------------------------------*/
string CValueHelper::BOOLToString( LSA_BOOL val ) const
{
...
string CValueHelper::MAC2String( const LSA_UINT8* pMac ) const
{
	LSA_CHAR tmpBuf[20];
...
/*----------------------------------------------------------------------------*/
string CValueHelper::Buffer2String( const LSA_UINT8* pBuffer, LSA_UINT16 bufferSize ) const
{
	string result = "";
...
/*----------------------------------------------------------------------------*/
string CValueHelper::IP2String( LSA_UINT32 ip ) const
{
	LSA_UINT16 ipVal[4];
	ipVal[0] = ((ip >> 24) & 0xFF);
	ipVal[1] = ((ip >> 16) & 0xFF);
...
/*----------------------------------------------------------------------------*/
string CValueHelper::IP2String( const LSA_UINT8* pIP ) const
{
	PND_ASSERT( pIP != 0 );
...
LSA_VOID CValueHelper::PndHandle2SysPtrPSI( PND_HANDLE_PTR_TYPE cpHandle, LSA_VOID* psiSysPtr ) const
{
...
LSA_BOOL CValueHelper::SplitObjectID( string objID, LSA_UINT16 *pInst, LSA_UINT16 *pDevId, LSA_UINT16 *pVenId ) const
{
...
/*----------------------------------------------------------------------------*/
string CValueHelper::GetLeftSubString( string srcStr, char seperator ) const
{
	string result = "";

	if ( !srcStr.empty())
	{
		LSA_UINT foundIdx = srcStr.find( seperator );
...
#endif //0 
/*----------------------------------------------------------------------------*/
string CValueHelper::GetJoinedString( stringVector& srcList, string seperator ) const
{
	LSA_UINT len = srcList.size();

	string result = (len > 0 ) ? srcList[0] : "";

	for ( LSA_UINT idx = 1; idx < len; idx++ )
	{
		result += seperator;
		result += srcList[idx];
	}
...
PNIO_UINT8* CValueHelper::HexArrToCharArr(const PNIO_UINT8* src, PNIO_UINT32 dataLength) const
{
...
LSA_VOID CValueHelper::CharArrToHexArr( PNIO_UINT8* dst, const PNIO_UINT8* src, PNIO_UINT32 dataLength) const
{
...
LSA_UINT32 CValueHelper::GetDigitCount(const LSA_UINT32 number) const
{
    LSA_UINT32 digitCount = 1;
	LSA_UINT32 tmp = number;
    while(true)
    {
		tmp = tmp /10;
        if(tmp == 0)
...
PNIO_UINT32 CValueHelper::AlignSize(PNIO_UINT32 size, PNIO_UINT32 alignment ) const
{

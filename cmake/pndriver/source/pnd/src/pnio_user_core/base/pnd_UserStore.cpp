...
CUserStore::CUserStore(): IUserStore()
{
...
/*----------------------------------------------------------------------------*/
CUserStore::CUserStore( PND_HANDLE_PTR_TYPE pHandle, PND_PNUC_LADDR_PTR_TYPE laddr_ptr, CUserNode *pParent ) : IUserStore( pHandle, pParent )
{
...
CUserNode* CUserStore::createUser( PND_PNIO_USER_TYPE userType, PND_PNUC_LADDR_PTR_TYPE laddr_ptr )
{
	...
	case OHA_USER: // OHA user
		{
			...
			COHAUser *pUserOHA = new COHAUser( &m_pndHandle, this, new CPndAdapter() );
			PND_ASSERT( pUserOHA != 0 );

			LSA_UINT32 res = pParam->GetParamOHA( &m_pndHandle, &ohaParam );
			PND_ASSERT( res == PNIO_OK );

			pUserOHA->Create( &ohaParam, new COHAEventHandler(pUserOHA->GetPndHandle(), pUserOHA) );
			pUser = pUserOHA;
		}
		break;
    ...
    case ISO_USER:
        {
            PND_PARAM_ISO_HD_IF_TYPE isoParam;
			pnd_memset( &isoParam, 0, sizeof ( PND_PARAM_ISO_HD_IF_TYPE ));

			LSA_UINT32 res = pParam->GetParamISO( &m_pndHandle, &isoParam );
			PND_ASSERT( res == PNIO_OK );
			
			if ( isoParam.isIsoConfiguration == LSA_TRUE )
			{
			    CISOUser *pUserISO = new CISOUser( &m_pndHandle, this );
			    PND_ASSERT( pUserISO != 0);
			                
	            pUserISO->Create(&isoParam);
	            pUser = pUserISO;
			}
			else
			{
			    PND_PNUC_TRACE_00( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE_HIGH,
			        "CUserStore::createUser(): CISOUser is not created since the config XML does not support IsoM");
			}
        }
        break;
    #endif

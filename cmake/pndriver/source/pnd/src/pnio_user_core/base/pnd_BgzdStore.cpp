...
/*---------------------------------------------------------------------------*/
void CBgzdInfoStore::UpdateBgzdInfo( CBgzdInfoStore& bgzdInfo ) const
{
...
#if 0 /* future requirement, see TIA 1376282 */ // TODO: Added for PnRun 06.01.00.00_00.01.00.26 integration 
	if ( ( submodState & CM_SUB_STATE_ADVICE_MASK) == CM_SUB_STATE_ADVICE_MASK)
	{
...
LSA_BOOL CBgzdInfoStore::IsNotPlugged() const
{
...
/*---------------------------------------------------------------------------*/
PND_BGZD_STATE_TYPE CBgzdInfoStore::GetBgzdState() const
{
...
/*---------------------------------------------------------------------------*/
string CBgzdInfoStore::GetBgzdSubslot() const
{
	stringVector tmp;
	CValueHelper val;
...
/*---------------------------------------------------------------------------*/
string CBgzdInfoStore::GetBgzdUser() const
{
	stringVector tmp;
	CValueHelper val;
...
LSA_BOOL CBgzdInfoStore::IsOkState() const
{
...
/*---------------------------------------------------------------------------*/
LSA_BOOL CBgzdInfoStore::IsDeactivatedState() const
{
...
/*---------------------------------------------------------------------------*/
LSA_BOOL CBgzdInfoStore::IsNotExistsState() const
{
	LSA_BOOL result   = LSA_FALSE;
...
/*---------------------------------------------------------------------------*/
LSA_BOOL CBgzdInfoStore::IsNotAvaliableState() const
{
...
/*---------------------------------------------------------------------------*/
LSA_BOOL CBgzdInfoStore::IsFaultyState() const
{
...
/*---------------------------------------------------------------------------*/
void CBgzdInfoStore::dumpBgzd() const
{
...
{
	PND_ASSERT( boardNr <= PSI_CFG_MAX_HD_CNT);
...
/*---------------------------------------------------------------------------*/
void CBgzdLedStore::SetLED( const PND_LED_STORE_TYPE& led, LSA_BOOL state )
{
...
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
/*lint -e{1746} Parameter 'user_id' in function could be made const reference */
...
PND_LED_STATE_TYPE CBgzdLedStore::getLedStateBF() const
{
...
/*---------------------------------------------------------------------------*/
PND_LED_STATE_TYPE CBgzdLedStore::getLedStateSF() const
{
	PND_LED_STATE_TYPE result = LED_STATE_OFF;
...
/*---------------------------------------------------------------------------*/
PND_LED_STATE_TYPE CBgzdLedStore::getLedStateMAINT() const
{
	PND_LED_STATE_TYPE result = LED_STATE_OFF;
...
/*---------------------------------------------------------------------------*/
LSA_BOOL CBgzdLedStore::isLedOn( const PND_LED_STATE_TYPE& ledState ) const
{
	LSA_BOOL result = LSA_FALSE;

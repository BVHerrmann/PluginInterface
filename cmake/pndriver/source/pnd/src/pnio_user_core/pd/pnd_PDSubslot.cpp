...
void CPDSubslot::Create( LSA_UINT16 slotNr, LSA_UINT16 subslotNr, LSA_UINT32 modIdent, LSA_UINT32 subIdent, LSA_UINT16 laddr )
{
	...
	m_laddr        = laddr;
    m_pdevProperties = CM_PDEV_PROP_TYPE_COMPACT | CM_PDEV_PROP_STATE_PLUGGED; // TODO: To be asked!
    m_im0Bits      = 0;  // TODO: To be asked!
}
...
/*----------------------------------------------------------------------------*/
LSA_UINT16 CPDSubslot::GetSlotNr() const
{
	return( m_slotNr );
}

/*----------------------------------------------------------------------------*/
LSA_UINT16 CPDSubslot::GetSubslotNr() const
{
	return( m_subslotNr );
}

/*----------------------------------------------------------------------------*/
LSA_UINT16 CPDSubslot::GetSubslotLogAddr() const
{
	return( m_laddr );
}

/*----------------------------------------------------------------------------*/
LSA_UINT32 CPDSubslot::GetSlotIdent() const
{
	return( m_slotIdent );
}

/*----------------------------------------------------------------------------*/
LSA_UINT32 CPDSubslot::GetSubslotIdent() const
{
	return( m_subslotIdent );
}

LSA_UINT8 CPDSubslot::GetPdevProperties() const
{
    return(m_pdevProperties);
}

LSA_UINT8 CPDSubslot::GetIm0Bits() const
{
    return(m_im0Bits);
}

/*----------------------------------------------------------------------------*/
PND_SYNC_STATE_TYPE CPDSubslot::GetSyncState() const
{
...
LSA_BOOL CPDSubslot::IsMapped() const
{
...
/*----------------------------------------------------------------------------*/
LSA_BOOL CPDSubslot::IsLinkUp() const
{
...
void CPDSubslot::WriteRecord( PND_RQB_PTR_TYPE p_rb )
{
    ...
    pRQB->args.pd.prm_rdwr->mk_remanent = LSA_FALSE; // z003nw8d: For MT with PNRunV07.00. Check it.

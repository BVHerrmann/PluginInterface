...
CPDSubslot* CPDUser::GetMappedSubslot( LSA_UINT16 subslotNr )
{
	...
	LSA_UINT subslotCount = getNodeList( m_subslotMap, subList );

	for ( LSA_UINT idx = 0; idx < subslotCount; idx++ )
	{
...
/*----------------------------------------------------------------------------*/
LSA_VOID CPDUser::SendPDSyncInfo(LSA_UINT16 portID)
{
    CPDSubslot *pSubslot;
    pSubslot = GetSubslot(portID);

    // create RQB to send sync info to ISOUser
    PND_RQB_PTR_TYPE pRQB = (PND_RQB_PTR_TYPE)pnd_mem_alloc(sizeof(PND_RQB_TYPE));
    PND_ASSERT(pRQB != 0);
	PND_ASSERT(pSubslot != PNIO_NULL);

    LSA_RQB_SET_OPCODE(pRQB, PND_OPC_SYNC_STATE_CHANGE);

    pRQB->args.sync_change.pnd_handle = m_pndHandle;
    pRQB->args.sync_change.syncState = pSubslot->GetSyncState();

    // Direct call to pnd_pnio_user_core_request function
    pnd_request_start(PND_MBX_ID_PNIO_USER_CORE, (PND_REQUEST_FCT)pnd_pnio_user_core_request, (LSA_VOID_PTR_TYPE)pRQB);

    pnd_mem_free(pRQB);
}

/*----------------------------------------------------------------------------*/
LSA_VOID CPDUser::SendRxLocalTransferEndNs(LSA_UINT32 rxLocalTransferEndNs) const
{
    // create RQB to send rxLocalTransferEnd to ISOUser
    PND_RQB_PTR_TYPE pRQB = (PND_RQB_PTR_TYPE)pnd_mem_alloc(sizeof(PND_RQB_TYPE));
    PND_ASSERT(pRQB != 0);

    LSA_RQB_SET_OPCODE(pRQB, PND_OPC_SET_RX_LOCAL_TRANSFER_END);

    pRQB->args.set_rx_local_transfer_end_ns.pnd_handle = m_pndHandle;
    pRQB->args.set_rx_local_transfer_end_ns.rxLocalTransferEndNs = rxLocalTransferEndNs;
...
LSA_VOID CPDUser::CreateCmPd()
{
	...
	LSA_UINT16 subslotCount = (LSA_UINT16)getNodeList( m_subslotMap, subList );
	LSA_UINT16 more_size = subslotCount * sizeof(CM_PD_MAP_TYPE);
	...
	pRQB->args.pd.create->map_count = (LSA_UINT16)subslotCount;

	for ( LSA_UINT16 portID = 0; portID < subslotCount; portID++ )
	{
		...
		pMap->pdev_properties = pSub->GetPdevProperties();
		pMap->im0_bits   = pSub->GetIm0Bits();
	}
...
/*----------------------------------------------------------------------------*/
/*lint -e{818} Pointer parameter 'pEntry' could be declared as pointing to const */
...
/*----------------------------------------------------------------------------*/
LSA_VOID CPDUser::InterfaceOpen( PND_RQB_PTR_TYPE p_rb ) const
{
...
/*----------------------------------------------------------------------------*/
LSA_VOID CPDUser::AlarmIndication( LSA_VOID * p_rb ) const
{
   PND_ASSERT(p_rb);
...
LSA_UINT16 CPDUser::GetVendorId() const
{
    return m_pdParam.vendor_id;
}

LSA_UINT16 CPDUser::GetDeviceId() const
{
    return m_pdParam.device_id;
}

LSA_UINT16 CPDUser::GetInstanceId() const
{

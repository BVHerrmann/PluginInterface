...
/*----------------------------------------------------------------------------*/
static PNIO_ALARM_TYPE PND_PD_ConvertAlarmTypeToHostLibType(LSA_UINT16 AlarmLeType)
{
  PNIO_ALARM_TYPE ret;
  switch(AlarmLeType)
  {
	...
	case CM_OK:
		{
            ...
            m_pUser->SendRxLocalTransferEndNs(pEvent->u.pd.appl_ready.rsv_interval_red.RxLocalTransferEndNs);
#endif
		}
		break;
...
#ifdef PND_CFG_ISO_SUPPORT
	// if the port is an interface port
	if(pRQB->args.pd.sync_led_info->edd_port_id == 0)
	{
	    m_pUser->SendPDSyncInfo(pRQB->args.pd.sync_led_info->edd_port_id);
	}    
#endif
...
LSA_VOID CPDEventHandler::StartListening( LSA_UINT16 nrOfEvents, LSA_UINT16 nrOfSub )
{
	...
	{
		CM_UPPER_RQB_PTR_TYPE pRQB = (CM_UPPER_RQB_PTR_TYPE)pnd_cm_alloc_rqb(CM_OPC_PD_PROVIDE_EVENT, handle, &m_pndHandle, 0 );
		PND_ASSERT(pRQB);
	...
	for ( LSA_UINT16 portID = 0; portID < nrOfSub; portID++ )
	{
		...
		// Alarms
		pRQB = (CM_UPPER_RQB_PTR_TYPE)pnd_cm_alloc_rqb(CM_OPC_PD_PROVIDE_ALARM, handle, &m_pndHandle, 0 );
		PND_ASSERT( pRQB );

		pRQB->args.pd.alarm->alarm_priority = CM_ALARM_PRIORITY_LOW;

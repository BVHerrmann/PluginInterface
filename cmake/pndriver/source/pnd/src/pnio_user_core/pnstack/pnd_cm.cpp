...
LSA_VOID_PTR_TYPE pnd_cm_alloc_rqb( LSA_OPCODE_TYPE opcode, LSA_HANDLE_TYPE handle, PND_HANDLE_PTR_TYPE pnd_sys, LSA_UINT16 more_size )
{
	...
	CM_ALLOC_UPPER_RQB((LSA_VOID_PTR_TYPE *)&pRQB, dummy_user_id, sizeof(*pRQB), &psi_sys );
	PND_ASSERT(pRQB);
	...
	case CM_OPC_CL_ALARM_SEND:
		{
			pRQB->args.pd.alarm->alarm_data_length = EDD_FRAME_BUFFER_LENGTH;
			CM_ACP_ALLOC_ALARM_MEM((CM_ACP_LOWER_MEM_PTR_TYPE*)&pRQB->args.pd.alarm->alarm_data, EDD_FRAME_BUFFER_LENGTH, &psi_sys);
			PND_ASSERT( pRQB->args.pd.alarm->alarm_data);
		}
		break;
	...
	case CM_OPC_PD_PROVIDE_ALARM:
		{
			pRQB->args.pd.alarm->alarm_data_length = 0;
			pRQB->args.pd.alarm->alarm_data        = LSA_NULL;
		}
		break;
	default:
		break;
...
LSA_VOID pnd_cm_free_rqb( LSA_VOID_PTR_TYPE p_rqb, PND_HANDLE_PTR_TYPE pnd_sys )
{
	...
	case CM_OPC_PD_PROVIDE_ALARM:
		{
			if ( pRQB->args.pd.alarm->alarm_data != LSA_NULL )
			{
				CM_ACP_FREE_ALARM_MEM( &rc, pRQB->args.pd.alarm->alarm_data, &psi_sys);

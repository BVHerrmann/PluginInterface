...
static CPnioUserCoreApp *pApp = NULL;
/*---------------------------------------------------------------------------*/
...
LSA_UINT32 pnd_pnio_user_core_init( LSA_VOID )
{
	...
	if ( pApp != NULL ) // init already done ?
	{
		// invalid state
	...
	else
	{
		...
		pApp = new CPnioUserCoreApp(new CUserStoreFactory(), new CPndAdapter());
...
LSA_VOID pnd_pnio_user_core_request( PND_RQB_PTR_TYPE  rqb_ptr )
{
    ...
    case PND_OPC_SYNC_STATE_CHANGE:
        {
            pApp->SyncStateChange( rqb_ptr );
        }
        break;
    case PND_OPC_ISO_SYNC:
        {
            pApp->StartSynchronization( rqb_ptr );
        }   
        break;
    case PND_OPC_SET_RX_LOCAL_TRANSFER_END:
        {
            pApp->SetRxLocalTransferEndNs( rqb_ptr );
        }
        break;
#endif
	default:

...
/////////////////////////////////////////////////////////////////////////////
/*----------------------------------------------------------------------------*/
COHAUser::COHAUser( PND_HANDLE_PTR_TYPE pHandle, CUserNode *pParent, IPndAdapter* _pPndAdapter ) : IOHAUser( pHandle, pParent ),
    m_pPndAdapter(_pPndAdapter)
{
	...
	m_isDcpRequestActive = LSA_FALSE;
...
/*----------------------------------------------------------------------------*/
void COHAUser::Create( LSA_VOID_PTR_TYPE pConfig, IOHAEventHandler* pOhaEventHandler )
{
    ...
    m_pEventHandler = pOhaEventHandler;
}
...
void COHAUser::Shutdown( LSA_VOID* p_rb )
{
	...
	{
		if (!m_isDcpRequestActive) // not Waiting on DCP response?
		{
...
void COHAUser::OpenChannel()
{
    ...
    OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb(OHA_OPC_OPEN_CHANNEL, m_pnioHandle, &m_pndHandle);
	PND_ASSERT(pRQB);
    ...
    m_pPndAdapter->epsUserOpenChannelLower(pRQB, this, COHAUser::OpenChannelDone);
}
...
void COHAUser::CloseChannel()
{
	...
	OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb( OHA_OPC_CLOSE_CHANNEL, m_pnioHandle, &m_pndHandle );
	PND_ASSERT(pRQB);
	...
	PND_ASSERT(m_pEventHandler != PNIO_NULL);
...
void COHAUser::RemaRead( LSA_UINT32 remaIdx, LSA_UINT32 dataSize )
{
	...
	OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb( OHA_OPC_REMA_READ, m_pnioHandle, &m_pndHandle );
	PND_ASSERT( pRQB != 0 );
	PND_ASSERT( dataSize <= 0xFFFF );

    if (dataSize > 0)
    {
        pRQB->args.RemaData.data = (LSA_UINT8*)m_pPndAdapter->ohaAllocUpperMem((LSA_UINT16)dataSize, &m_pndHandle);
    }

	pRQB->args.RemaData.index       = remaIdx;
	pRQB->args.RemaData.data_size = dataSize;

	PND_OHA_TRACE_03( m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
		"COHAUser::RemaRead(): OHA REMA Read, DataIdx(%#x) ReadSize(%u) rqb(%08x)",
		remaIdx, dataSize, pRQB );
    
	m_pPndAdapter->epsUserRequestLower( pRQB, this, COHAUser::RemaReadDone );
}
...
void COHAUser::DcpAccessInit()
{
    ...
    OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb(OHA_OPC_DCP_CONTROL, m_pnioHandle, &m_pndHandle);
	PND_ASSERT( pRQB != 0 );
    ...
    m_pPndAdapter->epsUserRequestLower(pRQB, this, COHAUser::DcpAccessInitDone);
}

/*----------------------------------------------------------------------------*/
LSA_VOID COHAUser::DcpRequest(LSA_UINT8 event, LSA_UINT8 reason)
{
	m_isDcpRequestActive = LSA_TRUE;

	OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb(OHA_OPC_DCP_RESPONSE, m_pnioHandle, &m_pndHandle);
	PND_ASSERT(pRQB != 0);

	// Prepare args
	pRQB->args.DcpInd.Event = event;
	pRQB->args.DcpInd.Reason = reason;
	if (IsShutdown())
	{
		// shutdown is ongoing, so don´t make a set ip
		pRQB->args.DcpInd.Event = OHA_MODE_NO_SET;
		pRQB->args.DcpInd.Reason = OHA_DCP_REASON_DS_NOT_SET;
	}

	PND_OHA_TRACE_03(m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
		"COHAUser::DcpRequest(); OHA DCP request, event(%#x) reason(%#x) rqb(0x%08x)",
		pRQB->args.DcpInd.Event,
		pRQB->args.DcpInd.Reason,
		pRQB);

	eps_user_request_lower(pRQB, this, COHAUser::DcpRequestDone);
}
...
void COHAUser::SetDcpRole()
{
	...
	OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb( OHA_OPC_DCP_WRITE_ELEMENT, m_pnioHandle, &m_pndHandle );
	PND_ASSERT( pRQB != 0 );
	...
	m_pPndAdapter->epsUserRequestLower( pRQB, this, COHAUser::SetDcpRoleDone );
}
...
void COHAUser::SetDcpStationType()
{
	...
	pRQB->args.DcpElem.Remanent     = LSA_FALSE;
    pnd_memcpy(pRQB->args.DcpElem.pVariable, m_ohaParam.station_type, PSI_STRLEN(m_ohaParam.station_type));
    pRQB->args.DcpElem.Length = (LSA_UINT16)PSI_STRLEN(m_ohaParam.station_type);
	pRQB->args.DcpElem.AccCtrl      = OHA_READ_ONLY;
	...
	m_pPndAdapter->epsUserRequestLower( pRQB, this, COHAUser::SetDcpStationTypeDone );
}
...
void COHAUser::LldpAccessInit( LSA_UINT16 portID )
{
	...
	OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb( OHA_OPC_LLDP_ACCESS_CONTROL, m_pnioHandle, &m_pndHandle );
	PND_ASSERT( pRQB != 0 );
	...
	m_pPndAdapter->epsUserRequestLower( pRQB, this, COHAUser::LldpAccessInitDone );
}
...
void COHAUser::SNMPWriteSysName()
{
	...
	OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb( OHA_OPC_LLDP_WRITE_OPTION, m_pnioHandle, &m_pndHandle );
	PND_ASSERT( pRQB != 0 );
	...
	m_pPndAdapter->epsUserRequestLower( pRQB, this, COHAUser::SNMPWriteSysNameDone );
}
/*----------------------------------------------------------------------------*/
void COHAUser::CreateSystemDescription(string & desc) const
{
    ostringstream prod_stream;
    prod_stream << PND_VENDOR << ", " << PND_PRODUCT_FAMILY << ", " << PND_IM_DEVICE_TYPE << ", " << PND_MLFB << ", ";

    ostringstream hw_version_stream;
    hw_version_stream << "HW: Version " << right << setw(2) << setfill('0') << PND_HW_REVISION << ", ";

    ostringstream fw_version_stream;
    fw_version_stream << "FW: Version " << PND_VERSION_REVISION_PREFIX \
        << right << setw(2) << setfill('0') << PND_VERSION_FUNCTIONAL_ENHANCEMENT << "." \
        << right << setw(2) << setfill('0') << PND_VERSION_BUG_FIX << "." \
        << right << setw(2) << setfill('0') << PND_VERSION_INTERNAL_CHANGE << ", ";

    ostringstream serial_no_stream;
    serial_no_stream << PND_PRODUCT_SERIAL_NUMBER;

    ostringstream desc_stream;
    desc_stream << prod_stream.str() << hw_version_stream.str() << fw_version_stream.str() << serial_no_stream.str();
...
void COHAUser::SNMPWriteSysDescr()
{
    ...
    string desc;
    CreateSystemDescription(desc);

    LSA_CHAR *temp_desc = new LSA_CHAR[desc.length() + 1];
    pnd_strcpy(temp_desc, desc.c_str());
    OHA_COMMON_MEM_U8_PTR_TYPE sys_descr = (OHA_COMMON_MEM_U8_PTR_TYPE)temp_desc;
    ...
    m_pPndAdapter->epsUserRequestLower( pRQB, this, COHAUser::SNMPWriteSysDescrDone );
}
...
void COHAUser::SNMPWriteIfDescr( LSA_UINT16 portID )
{
    ...
    if( portID == 0)
    {
        ...
        ostringstream if_no_stream;
        if_no_stream << "X" << m_pndHandle.pnio_if_nr;

        desc += if_no_stream.str();
    }
    else
    {
        ostringstream eth_info_stream;
        eth_info_stream << ", Ethernet Port X" << m_pndHandle.pnio_if_nr << " P" << portID;

        desc += eth_info_stream.str();
    }

    LSA_CHAR *temp_desc = new LSA_CHAR[desc.length() + 1];
    pnd_strcpy(temp_desc, desc.c_str());
    OHA_COMMON_MEM_U8_PTR_TYPE if_descr = (OHA_COMMON_MEM_U8_PTR_TYPE)temp_desc;
    ...
    m_pPndAdapter->epsUserRequestLower( pRQB, this, COHAUser::SNMPWriteIfDescrDone );
}
...
void COHAUser::SetAddress( LSA_VOID* p_rb )
{
	...
	if( m_ohaParam.IPAddrValAllowed != LSA_TRUE )
	{
		if( (Mode & PNIO_SET_IP_MODE) == PNIO_SET_IP_MODE || (Mode & PNIO_SET_NOS_MODE) == PNIO_SET_NOS_MODE )
		{
            ...
            m_pPndAdapter->pnioUserRequestIobcDone(pPndRQB);
			return;
		}
	}
    ...
    OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb( OHA_OPC_SET_ADDRESS, m_pnioHandle, &m_pndHandle );
	PND_ASSERT( pRQB != 0 );
    ...
    if((Mode & PNIO_SET_IP_MODE) == PNIO_SET_IP_MODE)
    {
		...
		if( IpParam.Remanent )
		{
			pAddressType->RemaOptions |= OHA_CTRL_IP_SUITE;
		}
    ...
    if((Mode & PNIO_SET_NOS_MODE) == PNIO_SET_NOS_MODE)
    {
		...
		{
			pAddressType->RemaOptions |= OHA_CTRL_STATION_NAME;
		}

		pAddressType->pStationData->StationNameLen = NoSParam.Length;
								
        pAddressType->pStationData->pStationName = (LSA_UINT8*)m_pPndAdapter->ohaAllocUpperMem(OHA_MAX_STATION_NAME_LEN, &m_pndHandle);
    ...
    m_pPndAdapter->epsUserRequestLower(pRQB, this, COHAUser::SetAddressDone);
}
/*---------------------------------------------------------------------------*/
void COHAUser::WriteParameter( LSA_VOID* p_rb )
{
    ...
    switch(pPndRQB->args.prm_write.record_index)
    {
        case 0xF8000002: /* MultipleSNMPRecord */ 
            ...
            {
                OHA_UPPER_RQB_PTR_TYPE pRQB = (OHA_UPPER_RQB_PTR_TYPE)m_pPndAdapter->ohaAllocRqb( OHA_OPC_PRM_WRITE, m_pnioHandle, &m_pndHandle );
	            PND_ASSERT( pRQB != 0 );
                ...
                m_pPndAdapter->epsUserRequestLower( pRQB, this, COHAUser::WriteParameterDone );
            }
            break;
        default:
...
void COHAUser::OpenChannelDone( void* p_inst, void* p_rb)
{
    ...
    pUser->m_pPndAdapter->ohaFreeRqb(pRQB, &pUser->m_pndHandle);
}
...
void COHAUser::RemaReadDone( void* p_inst, void* p_rb)
{
	...
	else
	{
		...
		else
		{
            ...
            {
                pPndRQB->args.rema_data.data = (PNIO_UINT8*)pUser->m_pPndAdapter->memAlloc(pPndRQB->args.rema_data.data_length);
                pnd_memcpy(pPndRQB->args.rema_data.data, pRQB->args.RemaData.data, pRQB->args.RemaData.data_length);
            }
            else
            {
                pPndRQB->args.rema_data.data = PNIO_NULL;
            }

			// direct call to pnd_pnio_user_core_request
		    pUser->m_pPndAdapter->requestStart(PND_MBX_ID_PNIO_USER_CORE, (PND_REQUEST_FCT)pnd_pnio_user_core_request, (LSA_VOID_PTR_TYPE)pPndRQB);

		    pUser->m_pPndAdapter->pnioUserRequestIobcDone( pPndRQB );
	    }
	}
...
/*----------------------------------------------------------------------------*/
LSA_VOID COHAUser::DcpRequestDone(void * p_inst, void * p_rb)
{
	COHAUser *pUser = static_cast<COHAUser*>(p_inst);
	OHA_UPPER_RQB_PTR_TYPE pRQB = static_cast<OHA_UPPER_RQB_PTR_TYPE>(p_rb);
	...
	PND_OHA_TRACE_07(pUser->m_pndHandle.pnio_if_nr, LSA_TRACE_LEVEL_NOTE,
		"COHAUser::DcpRequestDone(): OHA Query Response DCP.IND rsp(%#x) DCP-event(%u) Type(%u) SubType(%u) Options(%#x) RemaOptions(%#x) Count(%u)",
		OHA_RQB_GET_RESPONSE(pRQB),
		pDCP->Event,
		pDCP->Type,
		pDCP->SubType,
	...
	pUser->m_isDcpRequestActive = LSA_FALSE;

	if (pUser->IsShutdown())
	{
		// send close channel RQB
		pUser->CloseChannel();
	}
...
LSA_VOID COHAUser::RemaReleaseResources()
{
	...
	while ((pRemaRqb = GetRemaResource()) != 0)
	{
		...
		m_pPndAdapter->pnioUserRequestIobcDone( pRemaRqb );
	}
}

/*----------------------------------------------------------------------------*/

LSA_VOID COHAUser::GetStationType(LSA_UINT8* pBuffer) const
{
    PND_ASSERT(pBuffer);
    pnd_memcpy(pBuffer, m_ohaParam.station_type, sizeof(m_ohaParam.station_type));
}

/*----------------------------------------------------------------------------*/

LSA_BOOL COHAUser::GetIpAddrValidationState() const
{
...
LSA_BOOL COHAUser::GetNameOfStationValidationState() const
{
	return m_ohaParam.NameOfStationValAllowed;
}

/*----------------------------------------------------------------------------*/
LSA_VOID COHAUser::GetInterfaceData( LSA_UINT32* pIp, LSA_UINT32* pMask, LSA_UINT32* pGwIp, LSA_UINT8* pName ) const
{
	CValueHelper valObj;
	...
	{
		LSA_CHAR *cstr = new LSA_CHAR[strLength + 1];
		pnd_strcpy(cstr, m_nameOfStation.c_str());
		pnd_memcpy(pName, (LSA_VOID *)cstr, strLength );	
		delete[] cstr;
	}
...
void COHAUser::SetInterfaceData( const LSA_UINT8* pIp, const LSA_UINT8* pMask, const LSA_UINT8* pGwIp, LSA_BOOL ipDup, const LSA_UINT8* pName, LSA_UINT16 size, LSA_BOOL nosDup )
{
	CValueHelper valObj;

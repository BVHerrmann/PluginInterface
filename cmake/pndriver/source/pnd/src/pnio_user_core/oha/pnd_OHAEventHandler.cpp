...
void COHAEventHandler::DcpIndication( OHA_UPPER_RQB_PTR_TYPE pRQB )
{
	...
	if (( pDCP->Options & OHA_CTRL_IP_SUITE ) == OHA_CTRL_IP_SUITE )
	{
		if( pDCP->Event == OHA_EVENT_QUERY_SET ) // OHA IP suite user query
		{
			...
			{
				m_pUser->DcpRequest(OHA_MODE_SET, LSA_NULL);
			}
			else
			{
				...
				{
					m_pUser->DcpRequest(OHA_MODE_NO_SET, LSA_NULL);
				}
				else
				{
					m_pUser->DcpRequest(OHA_MODE_EMERGENCY_SET, LSA_NULL);
				}
			}
		}
	...
	if ((pDCP->Options & OHA_CTRL_RESET_TO_FACTORY) == OHA_CTRL_RESET_TO_FACTORY)
	{
		if (pDCP->Event == OHA_EVENT_QUERY_SET) 
		{
			if (m_pUser->GetIpAddrValidationState() /* IPAddressValidation == 0xFFFF */ ||
				m_pUser->GetNameOfStationValidationState() /* NameOfStationValidation == 0xFFFF */)
			{
				m_pUser->DcpRequest(OHA_MODE_SET, LSA_NULL);
			}
			else
			{
				m_pUser->DcpRequest(OHA_MODE_NO_SET, OHA_DCP_REASON_NOT_APPLICABLE);
			}
		}
	}
...
void COHAEventHandler::RemaIndication( OHA_UPPER_RQB_PTR_TYPE pRQB )
{
	...
	if ( OHA_RQB_GET_RESPONSE( pRQB ) == OHA_OK )
	{
		...
		case OHA_SNMP_DATA:     // SNMP settings
			{
				if(pRQB->args.RemaData.index == OHA_RECORD_INDEX_MULTIPLE_SNMP)
				{
					m_pUser->RemaRead( pRQB->args.RemaData.index, pRQB->args.RemaData.data_length );
                    m_responseRQB = pRQB;
				}
...
void COHAEventHandler::EventReceived(void* p_inst, void* p_rb)
{
	...
	{
		if (!eventObj->m_pUser->IsShutdown())  // OHA User not in shutdown ?
		{
		...
		{
			// late indication received, free only without handle it for reprovide
			pnd_oha_free_rqb(pRQB, &eventObj->m_pndHandle);
		}
	}
}
...
void COHAEventHandler::RemaIndicationDone()
{
	...
	// reprovide RQB
	PND_ASSERT(m_responseRQB != PNIO_NULL);

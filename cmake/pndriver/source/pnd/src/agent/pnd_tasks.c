...
{ 
    { "IO_BASECORET",  PND_MBX_ID_IO_BASE_CORE,   0, 0, LSA_FALSE, EPS_POSIX_THREAD_PRIORITY_NORMAL },
    { "PNIO_USRCORET", PND_MBX_ID_PNIO_USER_CORE, 0, 0, LSA_FALSE, EPS_POSIX_THREAD_PRIORITY_NORMAL }
};
...
{
    LSA_UINT16 idx;
	/*lint -e{550} symbol tp not accessed*/
    PND_THREAD *tp;
...
static LSA_VOID pnd_start_thread( PND_THREAD* tp )
{
    int ret_val;
	...
	ret_val = pthread_attr_setschedpolicy(&th_attr, /*SCHED_OTHER atw12r13*/ SCHED_FIFO);
    ...
    ret_val = pthread_create(&(tp->threadHandle), &th_attr,  /*(void *)*/pnd_thread_proc, tp);
    PND_ASSERT(ret_val == 0);
    ...
    // PND_SYSTEM_TRACE_01(0, LSA_TRACE_LEVEL_NOTE,"pnd_start_thread() - started thread %d",tp->threadHandle);
}
...
LSA_UINT32 pnd_threads_get_current_id( LSA_VOID )
{
    ...
    for ( idx = 0; idx < PND_MAX_TASK_ID - 1; idx++ )
    {
...
/*----------------------------------------------------------------------------*/
LSA_VOID pnd_threads_signal_ready( LSA_VOID_PTR_TYPE argument)
{
    LSA_UNUSED_ARG (argument);
...
/*----------------------------------------------------------------------------*/
LSA_VOID pnd_threads_signal_stopped( LSA_VOID_PTR_TYPE argument)
{

    PND_THREAD *tp_this = (PND_THREAD*)argument;

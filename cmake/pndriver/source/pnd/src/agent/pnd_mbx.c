...
struct pnd_mbx_pnio_header_tag {
    LSA_RQB_HEADER(struct pnd_pnio_header*)
};

typedef struct pnd_mbx_pnio_header_tag * PND_MBX_PNIO_HEADER_PTR_TYPE;
...
static LSA_UINT16  pnd_get_message(LSA_VOID (**fkt)(void *), struct pnd_pnio_header **rqb )
{
...
LSA_INT32 pnd_core_thread_proc(LSA_VOID_PTR_TYPE argument)
{
    ...
    pnd_threads_signal_ready(argument);
    ...
    pnd_threads_signal_stopped(argument);

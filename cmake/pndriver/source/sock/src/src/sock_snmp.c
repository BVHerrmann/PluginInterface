...
static sock_snmp_activate_int(
	...
	{
		soc = sock_socket_alloc(p_cha, lower_handle, SOCK_LOW_DGRAM);
	}
...
sock_snmp_dispatch_request(
	...
	else   /* pass data to internal agent */
	{
		 ...
		 */
		LSA_BOOL pending = (TCIP_PENDING_YES == tcip_mib2_statistics_update() ? LSA_TRUE : LSA_FALSE);
...
#else
	LSA_UNUSED_ARG(p_rem_addr);

	SOCK_SNMP_AGT_PARSE((LSA_UINT8*)p_snmp_data
						, (LSA_UINT)p_snmp_data_length
						, (LSA_UINT8*)sock_data.snmp_int_agent_return_buf
						, SOCK_SNMP_BUFFER_SIZE, &ret_val
						);
#endif

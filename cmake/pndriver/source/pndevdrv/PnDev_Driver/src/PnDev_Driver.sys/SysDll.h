...
//	#define FPGA1_ERTEC200P_ARM_NO_TOPLEVEL_ICU									// Host-IRQ created by PNIP-ICU2 IRQ0
	...
	//------------------------------------------------------------------------
	#define	PNDEV_DIM_ARRAY_EVENT_TIMER	3

	typedef enum _ePNDEV_EVENT_TIMER
	{
		ePNDEV_EVENT_TIMER_INVALID			= 0,							// don't change this value, see fnWaitForEventTimer()
		ePNDEV_EVENT_TIMER_STOP_WITH_CON	= 1,
		ePNDEV_EVENT_TIMER_KTICK			= 2,
		ePNDEV_EVENT_TIMER_SYNC_KTICK		= 3
	}
	ePNDEV_EVENT_TIMER;
	...
	typedef enum _ePNDEV_FATAL_ASYNC
	{
		...
		ePNDEV_FATAL_ASYNC__ACCESS_ERROR				= 21,

		ePNDEV_FATAL_ASYNC__SIMULATED_FATAL_SYS			= 22,
		ePNDEV_FATAL_ASYNC__SIMULATED_FATAL_THREAD_SYS	= 23
	}
	ePNDEV_FATAL_ASYNC;
	...
	#define	PNDEV_FATAL_ASYNC__ACCESS_ERROR					_TEXT("Access error detected!")
...
//	#pragma pack(show)
	...
	typedef struct _uPNDEV_START_USER_FW_LINUX_IN
	{
		uPNDEV64_HANDLE			uAppSys;
		UINT32					lOffsetFwBuf;
		UINT32					lIndexFw;
	...
	uPNDEV_START_USER_FW_LINUX_IN;

	//------------------------------------------------------------------------
	typedef struct _uPNDEV_START_USER_FW_LINUX_OUT
	{
		_TCHAR					sError[PNDEV_SIZE_STRING_BUF];
		uPNDEV_HW_DESC_SDRAM	uAsicSdram_Updated;
	}
	uPNDEV_START_USER_FW_LINUX_OUT;
	...
	typedef struct _uPNDEV_GET_INT_STATE_IN
	{
		UINT32 lDummy;
	}
	uPNDEV_GET_INT_STATE_IN;

	//------------------------------------------------------------------------
	typedef struct _uPNDEV_GET_INT_STATE_OUT
	{
		_TCHAR			sError[PNDEV_SIZE_STRING_BUF];
		BOOLEAN			bIntOccured[PNDEV_MSIX_VECTOR_COUNT_MAX];
	}
	uPNDEV_GET_INT_STATE_OUT;

...
UINT32 lPnIpReg_IcuControl = 0;
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_FPGA1_HERA:	// FPGA1-Hera board
		{
			...
			if	(pDevExtIn->eIcu == ePNDEV_ICU_HERA)
				// Hera 
			{
				if (pDevExtIn->lBitStreamId == 0x09)
					// Inc9 - TODO Remove
				{
					// ToDo: Problem mit EddpTest: IsrDPC feuert Event: UISR not queued, no MasterAPP 
					// Init priorities at PCIe UserLogic - low prio
					// MSI-X 0-7
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + 0xCA0),
						0x0000FF00);

					// Init priorities at PCIe UserLogic - high prio
					// MSI-X 8-15
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + 0xCB0),
						0x000000ff);

					// Init priorities at PCIe UserLogic - super high prio
					// MSI-X 16-31
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + 0xCC0),
						0xFFFF0000);
				}
					...
					// Inc 12
				{
					...
					// MSI - X interrupts : UL - internal priority assignment.
					// 0x0 : none(disabled)
					// 0x1 : low priority
					// 0x2 : medium priority
					// 0x3 : high priority
					...
					// MSI-X 0-7 - all set to low prio
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + HERA_PCIE_UL_REG__MSIX_PRIO_0),
						0x11111111);

					// MSI-X 8-15 - all set to high prio
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + HERA_PCIE_UL_REG__MSIX_PRIO_1),
						0x33333333);

					// MSI-X 16-23 - all set to super high prio
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + HERA_PCIE_UL_REG__MSIX_PRIO_2),
						0x44444444);

					// MSI-X 24-31- all set to super high prio
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + HERA_PCIE_UL_REG__MSIX_PRIO_3),
						0x44444444);
				}
			}
	...
	BOOLEAN			bEnableClockSdram2	= FALSE;
				...
				// BoardType
		{
			...
			case ePNDEV_BOARD_EB200P:	// EB200P board
			{
				...
				if 	(pDevExtIn->lBoardRevision < 0x00020001)
					// not Eb200P-2
				{
					// see above
                ...
                UINT32  lScrbId                 = 0;
                    ...
                    char    sTraceDetail[PNDEV_SIZE_STRING_TRACE] = {0};
						...
						if 	(pDevExtIn->lBoardRevision >= 0x00020001)
							// Eb200P-2
						{
							// set values for Page mode
							lAsyncWaitCycle			= 0x40000080;
							lSdramConfig			= 0x00002521;
							lSdramRefresh			= 0x000003d0;
							lAsyncBank0 			= 0x0C302506;
						...
						{						
							...
							lSdramRefresh			= 0x000003d0;
							lAsyncBank0 			= 0x0C302506;	//alt 0x3ffe2502;
							lAsyncBank1 			= 0x3FFFFFF2;	//alt 0x0421fff2;
							lAsyncBank2 			= 0x3ffffff2;
							lAsyncBank3 			= 0x3ffffff2;
							lLowPowerExtendedMode	= 0x00000000;
							lBurstMode				= 0x00000000;
...
UINT32 i = 0;
	...
	if	(pDevExtIn->eAsic == ePNDEV_ASIC_ERTEC200P)
	{
		// reset StatisticRam
		...
		{
			WRITE_REG_UINT32(	(pDevExtIn->pPnIp + PNIP_RAM__OFFSET_STATISTIC32 + i),
								0x00000000);
		}
	...
	*/
	...
	else if (pDevExtIn->eAsic == ePNDEV_ASIC_HERA)
	{
		if (pDevExtIn->lBitStreamId == 0x09)
			// Inc09
		{
			// reset StatisticRam
			for (i = 0; i < PNIP_RAM_HERA__SIZE_STATISTIC; i += 4)
			{
				WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_STATISTIC32 + i),
					0x00000000);
			}
			...
			// reset SyncRam
			for (i = 0; i < PNIP_RAM_HERA__SIZE_SYNC; i += 4)
			{
				WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_SYNC + i),
					0x00000000);
			}
			...
			// reset CmdRam
			for (i = 0; i < PNIP_RAM_HERA__SIZE_CMD; i += 4)
			{
				WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_CMD + i),
					0x00000000);
			}
			...
			// reset ApiCtrlRam
			for (i = 0; i < PNIP_RAM_HERA__SIZE_API_CTRL; i += 4)
			{
				WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_API_CTRL + i),
					0x00000000);
			}
			...
			// Inc12
		{
			if (pDevExtIn->lBitStreamPatchLabel == 0x0)
				// IFA
			{
				// reset StatisticRam32
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_STATISTIC32_IFA; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_STATISTIC32 + i),
						0x00000000);
				}

				// reset StatisticRam64
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_STATISTIC64_IFA; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_STATISTIC64 + i),
						0x00000000);
				}

				// reset SyncRam
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_SYNC_IFA; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_SYNC + i),
						0x00000000);
				}
				...
				// reset CmdRam
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_CMD_IFA; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_CMD + i),
						0x00000000);
				}
				...
				// reset ApiCtrlRam
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_API_CTRL_IFA; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_API_CTRL + i),
						0x00000000);
				}
			...
			{
				// reset StatisticRam32
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_STATISTIC32_IFB; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_STATISTIC32 + i),
						0x00000000);
				}
				...
				// reset StatisticRam64
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_STATISTIC64_IFB; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_STATISTIC64 + i),
						0x00000000);
				}
				...
				// reset SyncRam
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_SYNC_IFB; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_SYNC + i),
						0x00000000);
				}
				...
				// reset CmdRam
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_CMD_IFB; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_CMD + i),
						0x00000000);
				}
				...
				// reset ApiCtrlRam
				for (i = 0; i < PNIP_RAM_HERA__USED_SIZE_API_CTRL_IFB; i += 4)
				{
					WRITE_REG_UINT32((pDevExtIn->pPnIp + PNIP_RAM__OFFSET_API_CTRL + i),
						0x00000000);
				}

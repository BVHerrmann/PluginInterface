...
#define __Private_h__
	...
	typedef struct _uPNDEV_HW_RES_BOARD
	{
		union
		{
			...
			struct
			{
				...
				uPNDEV_HW_DESC_KM	uPnIp;
				uPNDEV_HW_DESC_KM	uXgdma;
				uPNDEV_HW_DESC_KM	uKrisc32_CORE_A_Btcm;
				uPNDEV_HW_DESC_KM	uKrisc32_CORE_A_TimerReg;
				uPNDEV_HW_DESC_KM	uKrisc32_CORE_A_VIC;
				uPNDEV_HW_DESC_KM	uKrisc32_CORE_A_Atcm;
				uPNDEV_HW_DESC_KM	uKrisc32_CORE_B_Btcm;
				uPNDEV_HW_DESC_KM	uKrisc32_CORE_B_TimerReg;
				uPNDEV_HW_DESC_KM	uKrisc32_CORE_B_VIC;
				uPNDEV_HW_DESC_KM	uKrisc32_CORE_B_Atcm;
				uPNDEV_HW_DESC_KM	uApbPer;
			}
			uHera;
			...
			struct
			{
				...
				uPNDEV_HW_DESC_KM	uPruss2Config;
				uPNDEV_HW_DESC_KM	uPruss2;
                uPNDEV_HW_DESC_KM	uQspiReg;
            ...
            #if defined(PNDEV_SUPPORT_I210) || defined(PNDEV_SUPPORT_I210SFP)
            struct
            {
                uPNDEV_HW_DESC_KM	uBar0;
...
#if defined (PNDEV_OS_LINUX) && defined (PNDEV_DRIVER_SYS)
    ...
    //------------------------------------------------------------------------
	#define PNDEV_KTIMER_STATISTICS_COUNT 100000 // 100 seconds
	typedef struct _uPNDEV_VAR_SYS
	{
		struct
		{
			KTIMER			uTimer;
			KDPC			uDpc;
			uLIST_HEADER	uListReceiveTick;
			WDFSPINLOCK		hReceiveTickLock;

			struct
			{
				LARGE_INTEGER	lFreq;
				LARGE_INTEGER	lStart;
				LARGE_INTEGER	lTimeStamps[PNDEV_KTIMER_STATISTICS_COUNT];
				UINT32			lTimeStampIndex;
				UINT32			lTimeStampCount;
			}
			uStatistics;
		}
		uKTimer;
	...
	typedef struct _uPNDEV_VAR_SYS
	{
        ...
        BOOLEAN				        bPnDevMemInUse[4];         // indicates if static g_pndev_mem pool with same index is used (see os.c)
        BOOLEAN                     bPnDevSharedMemInUse[1];   // indicates if static g_pndev_shared_mem pool with same index is used (see os.c)
	}
	uPNDEV_VAR_SYS;
	...
	{
	#if defined (PNDEV_OS_WIN)
		WDFINTERRUPT	hInterrupt[PNDEV_MSIX_VECTOR_COUNT_MAX];												// handle to InterruptObject, returned by InterruptCreate
		...
		pthread_spinlock_t	    uSpinlockIrq[PNDEV_MSIX_VECTOR_COUNT_MAX];

	    UINT32					lInterrupt[PNDEV_MSIX_VECTOR_COUNT_MAX];
	    adn_pci_msi_t			uMsiEntry;
	...
	//	- otherwise os.h needs private.h (e.g. DEVICE_EXTENSION) and reverse (e.g. PHYSICAL_ADDRESS)
		...
		KDEFERRED_ROUTINE					fnTickEvtTimer;

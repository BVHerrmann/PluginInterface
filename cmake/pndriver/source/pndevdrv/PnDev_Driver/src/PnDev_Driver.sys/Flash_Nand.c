...
UINT16	lColumn	= 0;
	...
	lPage = (UINT16)(lOffsetDestIn / (pFlashParIn->uNand.lSizePage * pFlashParIn->lDataWidth));
	...
	// calculate number of column in page
	{
		...
		else
		{
			...
			lColumn = (UINT16)(lOffsetDestIn - (lPage * (pFlashParIn->uNand.lSizePage *  pFlashParIn->lDataWidth)));
							...
							_TCHAR*				sErrorOut)
{
    ...
    (void)READ_REG_UINT32(pFlashParIn->uNand.pPollReg); 
			...
			// Length equal or less than block size
		{
				...
				// if ReedSolomonCode was added and read operation successful
			{
				...
				{	
					...
					//lint --e(613) pArrayRscData possible use of null pointer - cannot be reached if allocation fails
					//lint --e(613) pArrayTmpEnc  possible use of null pointer - cannot be reached if allocation fails
...
//  D e s c r i p t i o n :
//
// helper functions and variables for ECC calculation
//************************************************************************
...
//************************************************************************
static const UINT8 g_lArrayEccColParity[256] = {
 ...
 //  D e s c r i p t i o n :
 //
 // fnFlashNand_EccHelper_CalculateEcc - calculate 3-byte ECC for 256-byte block
 //************************************************************************
static UINT32 fnFlashNand_EccHelper_CalculateEcc(	const UINT8*	pDataIn,
													UINT8*			pEccCalcOut)
{	
UINT32	i		= 0;
UINT8	lIdx	= 0;
...
UINT8	lTemp2	= 0;
	...
	for(i = 0; i < 256; i++) 
	{
		lIdx = g_lArrayEccColParity[*pDataIn++];
		lReg1 ^= (lIdx & 0x3f);
	...
	lTemp1  = (lReg3 & 0x80) >> 0; /* B7 -> B7 */
	lTemp1 |= (lReg2 & 0x80) >> 1; /* B7 -> B6 */
	lTemp1 |= (lReg3 & 0x40) >> 1; /* B6 -> B5 */
	lTemp1 |= (lReg2 & 0x40) >> 2; /* B6 -> B4 */
	lTemp1 |= (lReg3 & 0x20) >> 2; /* B5 -> B3 */
	lTemp1 |= (lReg2 & 0x20) >> 3; /* B5 -> B2 */
	lTemp1 |= (lReg3 & 0x10) >> 3; /* B4 -> B1 */
	lTemp1 |= (lReg2 & 0x10) >> 4; /* B4 -> B0 */

	lTemp2  = (lReg3 & 0x08) << 4; /* B3 -> B7 */
	lTemp2 |= (lReg2 & 0x08) << 3; /* B3 -> B6 */
	lTemp2 |= (lReg3 & 0x04) << 3; /* B2 -> B5 */
	lTemp2 |= (lReg2 & 0x04) << 2; /* B2 -> B4 */
	lTemp2 |= (lReg3 & 0x02) << 2; /* B1 -> B3 */
	lTemp2 |= (lReg2 & 0x02) << 1; /* B1 -> B2 */
	lTemp2 |= (lReg3 & 0x01) << 1; /* B0 -> B1 */
	lTemp2 |= (lReg2 & 0x01) << 0; /* B7 -> B0 */

	pEccCalcOut[0] = ~lTemp1;
	pEccCalcOut[1] = ~lTemp2;
	pEccCalcOut[2] = ((~lReg1) << 2) | 0x03;
...
//  D e s c r i p t i o n :
//
// fnFlashNand_EccHelper_GenerateEcc - just calls the algorithm
//************************************************************************
static UINT32 fnFlashNand_EccHelper_GenerateEcc(uPNDEV_FLASH_NAND_ECC_PAR*	pEccParIn,
												const UINT8*				pDataIn, 									
												UINT8*						pEccCalcOut)
{
UINT32 i			= 0;
UINT32 lEccSize		= pEccParIn->lEccSize;
UINT32 lEccBytes	= pEccParIn->lEccBytes;
UINT32 lEccSteps	= pEccParIn->lEccSteps;		

	// calling the ecc calculation for single calculation blocks of input data
	for (i = 0; lEccSteps; lEccSteps--, i += lEccBytes, pDataIn += lEccSize)
	{
		fnFlashNand_EccHelper_CalculateEcc(pDataIn, &pEccCalcOut[i]);
	}
...
//  D e s c r i p t i o n :
//
// adds ECC in spare part of propgrammed blocks
//************************************************************************

BOOLEAN	fnFlashNand_CalculateEccHamming(DEVICE_EXTENSION*			pDevExtIn,
										uPNDEV_FLASH_PAR*			pFlashParIn,										
										uPNDEV_FLASH_NAND_ECC_PAR*	pEccParIn,
										const UINT32				lDimErrorIn,										
										_TCHAR*						sErrorOut)
{
BOOLEAN	bResult		= FALSE;
...
UINT32  i, j		= 0;
	...
	// allocate memory for the data we read from the nand flash
	pData = (UINT8*)fnAllocMemNonPaged((pFlashParIn->uNand.lSizePage * pFlashParIn->lDataWidth) * sizeof(UINT8), TAG('D', 'a', 't', 'a'));
		...
		// no memory
	{
		// set ErrorString
		fnBuildString(	_TEXT(__FUNCTION__),
						_TEXT("(): could not allocate memory for ecc calculation (flash data)!"),
						NULL,
						lDimErrorIn,
						sErrorOut);
	}
	...
	{
		// allocate memory for the ecc data
		pEccCalc	= (UINT8*)fnAllocMemNonPaged(pEccParIn->lNandSpareSize * sizeof(UINT8), TAG('D', 'a', 't', 'a'));
			...
			// no memory
		{
			// set ErrorString
			fnBuildString(	_TEXT(__FUNCTION__),
							_TEXT("(): could not allocate memory for ecc calculation (ecc data)!"),
							NULL,
							lDimErrorIn,
							sErrorOut);
		}
		...
		{
			// we got all the needed memory, do the ecc calculation

			for (i = 0; i < pEccParIn->lEccCalcSize / (pFlashParIn->uNand.lSizePage * pFlashParIn->lDataWidth); i++)
				// all the given pages (from param) within the nand flash, stepping through loop in pagesize junks
			{
				RtlZeroMemory(pData, (pFlashParIn->uNand.lSizePage * pFlashParIn->lDataWidth) * sizeof(UINT8));
				RtlZeroMemory(pEccCalc, pEccParIn->lNandSpareSize * sizeof(UINT8));

				// preset the spare array with 0xFF
				for (j = 0; j < pEccParIn->lNandSpareSize; j++)
				{
					pEccCalc[j] = 0xFF;
				}

				// read Page
				bResult = fnFlashNand_ReadPage(	pDevExtIn,
												pFlashParIn,
												pEccParIn->lEccCalcAddrStart + ((pFlashParIn->uNand.lSizePage * pFlashParIn->lDataWidth) * i),
												(pFlashParIn->uNand.lSizePage * pFlashParIn->lDataWidth) * sizeof(UINT8),
												lDimErrorIn,
												(UINT8*)pData,
												sErrorOut);
					...
					// error at reading
				{
					// set ErrorString
					fnBuildString(	_TEXT(__FUNCTION__),
									_TEXT("(): ecc calculation - error at reading from nand!"),
									NULL,
									lDimErrorIn,
									sErrorOut);
				...
				{
					// generate the ecc array for the read page
					fnFlashNand_EccHelper_GenerateEcc(	pEccParIn,
														pData,
														pEccCalc + pEccParIn->lEccPos);

					// program the ecc to the spare part of the page
					bResult = fnFlashNand_ProgramPage(	pDevExtIn,
														pFlashParIn, 
														pEccParIn->lEccCalcAddrStart + ((pFlashParIn->uNand.lSizePage * pFlashParIn->lDataWidth) * i),
														pEccCalc,
														pEccParIn->lNandSpareSize * sizeof(UINT8),
														TRUE,
														lDimErrorIn, 
														sErrorOut);
						...
						// error at writing
					{
						// set ErrorString
						fnBuildString(	_TEXT(__FUNCTION__),
										_TEXT("(): ecc calculation - error at writing to nand!"),
										NULL,
										lDimErrorIn,
										sErrorOut);
			...
			// free ecc block
			fnFreeMemNonPaged(pEccCalc, TAG('D', 'a', 't', 'a'));
		}

		// free data block
		fnFreeMemNonPaged(pData, TAG('D', 'a', 't', 'a'));
	}

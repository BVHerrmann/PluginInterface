...
BOOLEAN	bErrorTimeLoop		= FALSE;
			...
			// BoardType
	{
		...
		UINT32	lArrayUserLogic[((HERA_PCIE_UL_REG__ADDR13_HIGH - HERA_PCIE_UL_REG__ADDR0_LOW) / sizeof(UINT32) + 1)];
	
			if	(pDevExtIn->bFlrSupport)
				// FLR is supported
			{
				if	(fnBoard_SaveRegisterHwReset(	pDevExtIn,
													lArrayUserLogic,
													lDimErrorIn,
													sErrorOut))
					// saving user logic state ok
				{
							...
							// no TransactionPending
						{
								...
								// setting of FLR successful
							{
								...
								UINT16 lDeviceId = 0;
									...
									if	(fnGetVendorDeviceIdAtConfigSpace(	pDevExtIn,
																			(UINT16*) &lVendorId,
																			(UINT16*) &lDeviceId,
																			lDimErrorIn,
																			sErrorOut))
										// read Vendor/-DeviceId correct after reset
									{
											...
											// error at check Ids
										{
											// set ErrorString
											fnBuildString(	_TEXT(__FUNCTION__),
															_TEXT("(): Error at check VendorId and DeviceId!"),
															NULL,
															lDimErrorIn,
															sErrorOut);
										}
										else
										{
											...
											{
												if	(fnBoard_RestoreRegisterHwReset(	pDevExtIn,
																						lArrayUserLogic,
																						lDimErrorIn,
																						sErrorOut))
													// restore user logic state
												{
													// wait 100msec
													if	(fnDelayThread(	pDevExtIn,
																		100,
														...
														// starting WaitTime of current thread ok
													{
														...
														{
															// successful
														UINT16 lTimeOutCounter	= 0;
														UINT32 lServiceReg0Tmp	= 0;
														char	sTraceDetail[PNDEV_SIZE_STRING_TRACE] = { 0 };															

															for (lTimeOutCounter = 0; lTimeOutCounter < 30; lTimeOutCounter++)																
															{
																lServiceReg0Tmp = READ_REG_UINT32(pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_SCRB + 0x34);

																// set TraceDetail
																RtlStringCchPrintfA(sTraceDetail,
																					_countof(sTraceDetail),
																					"PnDev[%s]: HERA wait for primary, TimeOutCount(%d) lServiceReg0Tmp(0x%08x)",
																					pDevExtIn->sPciLocShortAscii,
																					lTimeOutCounter,
																					lServiceReg0Tmp);
															...
															if (!bResult)
															{
																// set ErrorString
																fnBuildString(	_TEXT(__FUNCTION__),
																				_TEXT("(): Error at waiting for HERA primary bootloader!"),
																				NULL,
																				lDimErrorIn,
																				sErrorOut);
															}
						...
						// board with Ertec200P/PCIe-XHIF
					{
								...
								// BoardType
						{
							...
							case ePNDEV_BOARD_EB200P:	// EB200P board
							{
								...
								WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_STATUS_ACK),
													MAXUINT32);
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_FPGA1_HERA:		// FPGA1-Hera board
		{
				...
				// version of board is ok
			{
				...
				// set PCIe_To_AMBA_Read_Error_Disable - Transmit_AXI_Errors_to_Host
				// this is to disable Completer Aborts if any Reads to non-mapped memory are performed
				WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + HERA_PCIE_UL_REG__UL_CONTROL_2),
				        		 (READ_REG_UINT32(pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + HERA_PCIE_UL_REG__UL_CONTROL_2) | PNDEV_UINT32_SET_BIT_16));
			
				// set minimum TIMEOUT_CNT, otherwise Bluescreens occures at access to ctc memory
				WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_CTC + 0x38),
									0x0);
			}
			...
			// BoardType
	{
		...
		UINT32 lBoardRevisionRequired = 0x00010001;		// V1.1
			...
			UINT32 lTmp = 0;
				...
				lTmp = READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__XHIF_CONFIG);
				...
				{
					lTmp = READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__STATUS);

					if	(lTmp & BOARD_ADMIN_STATUS_MSI)
		...
		{
			//Inc9 - TODO Remove
			lBitStreamIdRequired3		= 0x9;
			lBitStreamVersionRequired3	= 0x5;

			// INC12
			lBitStreamIdRequired2		= 0xC;
			lBitStreamVersionRequired2	= 0x7;
		...
		// success
	{
			...
			// invalid BitStream1
		{
			if	(	(pDevExtIn->lBitStreamId		!=	lBitStreamIdRequired2)
				...
				// invalid BitStream2
			{
				if	(	(pDevExtIn->lBitStreamId		!=	lBitStreamIdRequired3)
					||	(pDevExtIn->lBitStreamVersion	<	lBitStreamVersionRequired3))
					// invalid BitStream3
				{
					// error
					bResult = FALSE;
					...
					_RtlStringCchPrintf(sErrorDetail,
										_countof(sErrorDetail),
										_TEXT("At least BitStream version 0x%x.0x%x or 0x%x.0x%x or 0x%x.0x%x required (current: 0x%x.0x%x)"), 
										lBitStreamIdRequired1,
										lBitStreamVersionRequired1,
										lBitStreamIdRequired2,
										lBitStreamVersionRequired2,
										lBitStreamIdRequired3,
										lBitStreamVersionRequired3,
										pDevExtIn->lBitStreamId,
										pDevExtIn->lBitStreamVersion);
...
char	sTraceDetail[PNDEV_SIZE_STRING_TRACE] = {0};
	...
	UINT32 lPnIpReg_IcuControl = 0;
		...
		else if	(pDevExtIn->eBoard == ePNDEV_BOARD_FPGA1_HERA)	// ToDo: muss wieder zurückgebaut werden, da eigneltich Pnip zutrifft
				// FPGA1-Hera board
		{
			bResult = TRUE;	
		}
...
//  D e s c r i p t i o n :
//
//  check if any Access errors occured
//************************************************************************

BOOLEAN fnBoardIrtePnip_CheckAccessErrors(DEVICE_EXTENSION* pDevExtIn)
{
	BOOLEAN	bResult = FALSE;
	char	sTraceDetail[PNDEV_SIZE_STRING_TRACE] = { 0 };
			...
			// BoardType
	{
		case ePNDEV_BOARD_FPGA1_HERA:	// FPGA1-HERA board
		{
				...
				// Inc 12
			{
				...
				// read if an access error occured
				lMaster = (READ_REG_UINT32(pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_SCRB + HERA_SCRB_REG__ACC_ERR_MASTER));
				lSlave	= (READ_REG_UINT32(pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_SCRB + HERA_SCRB_REG__ACC_ERR_SLAVE));
				lBridge = (READ_REG_UINT32(pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_SCRB + HERA_SCRB_REG__ACC_ERR_AHB_APB_BRIDGE));
				lScrb	= (READ_REG_UINT32(pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_SCRB + HERA_SCRB_REG__ACC_ERR_SCRB));

				if (   ((lMaster>>31) == 0) // Bit31 Master_Error
					&& (lSlave	== HERA_SCRB_REG__ACC_ERR_SLAVE_RESET_VAL)
					&& (lBridge == HERA_SCRB_REG__ACC_ERR_AHB_APB_BRIDGE_RESET_VAL)
					&& (lScrb	== HERA_SCRB_REG__ACC_ERR_SCRB_RESET_VAL))
					// no access error detected
				{
					...
					// ACCESS ERROR
				{
					UINT32 lPcieUlErrorStatus = 0;

					// to gather some additional informations from PCIe UL
					lPcieUlErrorStatus = READ_REG_UINT32(pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + HERA_PCIE_UL_REG__ACCESS_ERROR);

					// set TraceDetail
					RtlStringCchPrintfA(sTraceDetail,
										_countof(sTraceDetail),
										"PnDev[%s]: ########## Access error detected (ACC_ERR_MASTER: 0x%x) (ACC_ERR_SLAVE: 0x%x) (ACC_ERR_AHB_APB_BRIDGE: 0x%x) (ACC_ERR_SCRB: 0x%x) (PCIE_UL_ACCESS_ERROR: 0x%x)!",
										pDevExtIn->sPciLocShortAscii,
										lMaster,
										lSlave,
										lBridge,
					...
					// reset the error registers because we already detected the error and fatal will be transmitted
					// otherwise every new cycle will detect the same error again
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_SCRB + HERA_SCRB_REG__ACC_ERR_MASTER),			0x0);
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_SCRB + HERA_SCRB_REG__ACC_ERR_SLAVE),				0x0);					
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_SCRB + HERA_SCRB_REG__ACC_ERR_SCRB),				0x0);
					WRITE_REG_UINT32((pDevExtIn->uHwResKm.uBoard.as.uHera.uApbPer.pPtr + HERA_APB_PER__OFFSET_PCIE_UL + HERA_PCIE_UL_REG__ACCESS_ERROR),		0x0);
				}
			}
			else
			{
...
UINT32 lResult = 0;
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_EB200P:			// EB200P board
		case ePNDEV_BOARD_DB_EB200_PCIE:	// DB-EB200P-PCIe board
		{
			lResult = READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__ID);
...
UINT32 lResult = 0;
			...
			// BoardType
	{
		...
		{
			lResult = READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__REVISION);
									...
									const BOOLEAN		bStartIn)
{
		...
		// start reset
	{
				...
				// BoardType
		{
			...
			{
				// start reset
				WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__RESET),
									0x55aa55aa);
...
UINT32	lTmp	= 0;
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_EB200P:	// EB200P board
		{
			...
			(lTmp & BOARD_ADMIN_STATUS_RESET) ? (bResult = TRUE) : (bResult = FALSE);
...
UINT32	lTmp	= 0;
			...
			// BoardType
	{
			...
			
		{
			...
			(lTmp & BOARD_ADMIN_STATUS_PLL_LOC) ? (bResult = TRUE) : (bResult = FALSE);
...
UINT32	lTmp	= 0;
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_DB_EB200_PCIE:	// DB-Eb200-PCIe board
		{
			...
			if	(lTmp & BOARD_ADMIN_STATUS_INT)
				// any interrupt exist
			{
			UINT32 lIrr	= 0;
			UINT32 lAck	= 0;
			UINT32 lImr	= 0;
				...
				// get IRR
				lIrr = READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_STATUS_ACK);

				// get IntMask
				lImr = READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_MASK);

				if	(	(lIrr & BOARD_ADMIN_INT_STATUS_ACK_ACCESS_TIMEOUT)
					&&	(lImr & BOARD_ADMIN_INT_STATUS_ACK_ACCESS_TIMEOUT))
					// AccessTimeoutInt set
				{
					pIntDetailOut->uCommon.bAccessTimeoutInt = TRUE;

					lAck |= BOARD_ADMIN_INT_STATUS_ACK_ACCESS_TIMEOUT;
				}

				if	(	(lIrr & BOARD_ADMIN_INT_STATUS_ACK_GPIO)
					&&	(lImr & BOARD_ADMIN_INT_STATUS_ACK_GPIO))
					// GpioInt set
				{
					pIntDetailOut->uCommon.bGpioInt = TRUE;

					lAck |= BOARD_ADMIN_INT_STATUS_ACK_GPIO;
				}

				if	(	(lIrr & BOARD_ADMIN_INT_STATUS_ACK_DMA)
					&&	(lImr & BOARD_ADMIN_INT_STATUS_ACK_DMA))
					// DmaInt set
				{
					pIntDetailOut->uCommon.bDmaInt = TRUE;

					lAck |= BOARD_ADMIN_INT_STATUS_ACK_DMA;
				}

				if	(	(lIrr & BOARD_ADMIN_INT_STATUS_ACK_DMA_ERROR)
					&&	(lImr & BOARD_ADMIN_INT_STATUS_ACK_DMA_ERROR))
					// DmaErrorInt set
				{
					pIntDetailOut->uCommon.bDmaErrorInt = TRUE;

					lAck |= BOARD_ADMIN_INT_STATUS_ACK_DMA_ERROR;
				}

				if	(	(lIrr & BOARD_ADMIN_INT_STATUS_ACK_DEMO_INT)
					&&	(lImr & BOARD_ADMIN_INT_STATUS_ACK_DEMO_INT))
					// BoardDemoInt set
				{
					// Note:
					...
					lAck |= BOARD_ADMIN_INT_STATUS_ACK_DEMO_INT;
				}

				if	(	(lIrr & BOARD_ADMIN_INT_STATUS_ACK_XHIF_HOST)
					&&	(lImr & BOARD_ADMIN_INT_STATUS_ACK_XHIF_HOST))
					// AsicInt set
				{
					// any AsicInt occurred
					pIntDetailOut->uCommon.bAsicInt = TRUE;

					lAck |= BOARD_ADMIN_INT_STATUS_ACK_XHIF_HOST;
				}
				...
				WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL),
									(READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL) | BOARD_ADMIN_CONTROL_EOI));
...
UINT32	lTmp	= 0;
	...
	*pWarningOut = FALSE;
			...
			// BoardType
	{
		...
		{
			lTmp = READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_STATUS_ACK);

			if	(lTmp & BOARD_ADMIN_INT_STATUS_ACK_ACCESS_TIMEOUT)
				// PCIe-AccessTimeout occurred
			{
				// Note:
				//	- if a PCIe-AccessTimeout occurred then there must be a BoardReset and AccessTimeoutInt must be acked
				...
				WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_STATUS_ACK),
									BOARD_ADMIN_INT_STATUS_ACK_ACCESS_TIMEOUT);
			}
			else
			{
				// success
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_DB_EB200_PCIE:	// DB-EB200-PCIe board
		{
				...
				// enable interrupt
			{
				// global
				WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL),
									(READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL) | BOARD_ADMIN_CONTROL_INT_ENABLE));

				// specific
				//	- Ertec200P XHIF interrupt
				WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_MASK),
									(READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_MASK) | BOARD_ADMIN_INT_MASK_ENABLE_XHIF_HOST));
										...
										const BOOLEAN		bSetIn)
{
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_DB_EB200_PCIE:	// DB-EB200-PCIe board
		{
			...
			{
				WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_MASK),
									(READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_MASK) | BOARD_ADMIN_INT_MASK_SET_DEMO_INT));
			}
			else	// clear BoardDemoInt
			{
				WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__INT_MASK),
										...
										BOOLEAN				bSetA20In)
{
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_EB200P:	// EB200P board
		{
			...
			{
				WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL),
									(READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL) & (~BOARD_ADMIN_CONTROL_PAGE_CS_R)));
			}
			else	// set signal XHIF_CS_R_A20 by external multiplexer to CS-R (PageCfgReg)
			{
				WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL),
...
UINT32	lTmp	= 0;
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_EB200P:	// EB200P board
		{
			lTmp = READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL);

			((lTmp & BOARD_ADMIN_CONTROL_PAGE_CS_R) == 0) ? (bResult = TRUE) : (bResult = FALSE);

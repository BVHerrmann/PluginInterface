...
//	#pragma alloc_text (PAGE, fnBoardIrte_SetStaticPages)
//	#pragma alloc_text (PAGE, fnBoardIrte_InitDriverPage)
//	#pragma alloc_text (PAGE, fnBoardIrte_StartDriverPage)
//	#pragma alloc_text (PAGE, fnBoardIrte_StopDriverPage)
...
BOOLEAN bResult = FALSE;
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_DB_EB200_PCIE:	// DB-EB200-Pcie board
		{
			...
			{
				// after reset:
				//	- 4*2 MB, regarding LBU Paging for Ertec200
				//	- PageCfgReg only accessible by XHIF_XCS_R 
				//		-> BAR1 (XHIF_XCS_R_A20 = XCS_R)
				{
					pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg = pDevExtIn->uHwResKm.uBar[1].pVirtualAdr;
				}				
			}
			
			// set static pages
			fnBoardIrte_SetStaticPages(pDevExtIn);
				...
				// XHIF
			{
				...
				else
				{
					...
					{			
						...
						if	(pDevExtIn->eBoardDetail == ePNDEV_BOARD_DETAIL_ERTEC200_XHIF)
						...
						{	
							...
							//	- EB200P/EB200-PCIe board:	hardwired
...
UINT32  lOffsetExpressCapability = 0;
			...
			// BoardType
	{
		...
		case ePNDEV_BOARD_EB200:			// EB200 board
		case ePNDEV_BOARD_DB_EB200_PCIE:	// DB-EB200-PCI board
		{
			// start/stop reset
			{
				// SystemCtrlReg: SoftReset=ON
				...
				UINT32	lEmifVersion	= 0;
						...
						// invalid SCRB-ID
					{
						...
						fnBuildString(	_TEXT(__FUNCTION__),
							...
							&&	(pDevExtIn->eBoard == ePNDEV_BOARD_EB200))
							// invalid BootModePin
					{
						// set ErrorDetail
						_RtlStringCchPrintf(sErrorDetail,
			...
			// BoardType
	{
		...
		uPNDEV_FLASH_DATA_CP1616    *puFlashData;		
			...
			else
			{
				...
				{
					RtlZeroMemory(puFlashPar, sizeof(uPNDEV_FLASH_PAR));
						...
						// success
					{
						// check version
						{
							...
							{
								lArrayVersion[0] = puFlashData->uBootVersion.uByte[9];
								lArrayVersion[1] = puFlashData->uBootVersion.uByte[11];
								lArrayVersion[2] = puFlashData->uBootVersion.uByte[13];
								lArrayVersion[3] = puFlashData->uBootVersion.uByte[15];
		...
		case ePNDEV_BOARD_DB_EB200_PCIE:	// DB-EB200-PCIe board
		{
			if	(pDevExtIn->lBoardRevision < 0x00020020)
				// invalid version
			{
				// set ErrorDetail
				_RtlStringCchPrintf(sErrorDetail,
									_countof(sErrorDetail),
									_TEXT("(): Invalid version of PCI-Bridge-FPGA (real: 0x%x, required: >= 0x00020020)!"),
...
void fnBoardIrte_SetStaticPages(DEVICE_EXTENSION* pDevExtIn)
{
	// set page0 to IRTE
	{
		if	(pDevExtIn->eBoard == ePNDEV_BOARD_EB200)
		...
		{
			// 2MB aligned
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE0_OFFSET),
								ERTEC200_AHB_ADR__IRTE);
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE0_RANGE),
								BOARD_EB200_XHIF__PAGE_SIZE_2MB);
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE0_CFG),
								ERTEC200_LBU_PX_CFG__32_BIT);
			
			// ptr=BAR2.Page0
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uIrte.pPtr				= (pDevExtIn->uHwResKm.uBar[2].pVirtualAdr + BOARD_EB200_XHIF__BAR2_OFFSET_PAGE0);
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uIrte.lIndexBar			= 2;
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uIrte.lOffsetBar		= BOARD_EB200_XHIF__BAR2_OFFSET_PAGE0;
		}
	...
	// set page1 to ApbPer
	{
		...
		{
			// 2MB aligned
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE1_OFFSET),
								ERTEC200_AHB_ADR__APB_PER);
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE1_RANGE),
								BOARD_EB200_XHIF__PAGE_SIZE_2MB);		
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE1_CFG),
								ERTEC200_LBU_PX_CFG__32_BIT);
			
			// ptr=BAR2.Page1
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uApbPer.pPtr			= (pDevExtIn->uHwResKm.uBar[2].pVirtualAdr + BOARD_EB200_XHIF__BAR2_OFFSET_PAGE1);
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uApbPer.lIndexBar		= 2;
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uApbPer.lOffsetBar		= BOARD_EB200_XHIF__BAR2_OFFSET_PAGE1;
		}
	...
	UINT32 lAhbAdrSdram = 0;
		...
		{
			// 2MB aligned
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE2_OFFSET),
								lAhbAdrSdram);
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE2_RANGE),
								BOARD_EB200_XHIF__PAGE_SIZE_2MB);	
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE2_CFG),
								ERTEC200_LBU_PX_CFG__32_BIT);
			
			// ptr=BAR2.Page2
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uEmifSdram.pPtr				= (pDevExtIn->uHwResKm.uBar[2].pVirtualAdr + BOARD_EB200_XHIF__BAR2_OFFSET_PAGE2);
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uEmifSdram.lIndexBar		= 2;
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uEmifSdram.lOffsetBar		= BOARD_EB200_XHIF__BAR2_OFFSET_PAGE2;
			pDevExtIn->uHwResKm.uBoard.as.uErtec200.uEmifSdram.lSize			= BOARD_EB200_XHIF__PAGE_SIZE_2MB;
		}
	...
	// set page3 (dynamic DriverPage) to ...
	{
		...
		fnBoardIrte_InitDriverPage(pDevExtIn);
	}
	
	if	(pDevExtIn->eBoard == ePNDEV_BOARD_DB_EB200_PCIE)
		// DB-EB200-PCIe board
	{
		// set AUTO_ALIGNMENT for Automatically split misaligned BAR2 transactions
		WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL),
							(READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL) | PNDEV_UINT32_SET_BIT_4));
	}
...
void fnBoardIrte_InitDriverPage(DEVICE_EXTENSION* pDevExtIn)
{
...
void fnBoardIrte_StartDriverPage(	DEVICE_EXTENSION*					pDevExtIn,
									const eBOARD_EB200__LBU_DYN_PAGE	ePageIn,
									const UINT32						lSegmentAhbIn,
									const BOOLEAN 						bNoAutoalignmentIn,
									const char*							sTraceIn)
{
char sTraceDetail[PNDEV_SIZE_STRING_TRACE] = {0};
	...
	uPNDEV_HW_DESC_KM*	pHwDescKm		= NULL;
				...
				// dynamic DriverPage
		{
			...
			case eBOARD_EB200__LBU_DYN_PAGE_EMIF_ASYNC_BANK0:
			{
				...
				// 16bit access, 0x0 for non-buffered mode
				lPageCfg = 0;

				pHwDescKm		= &pDevExtIn->uHwResKm.uBoard.as.uErtec200.uEmifAsyncBank0;
			
				if	((pDevExtIn->pAdminReg != NULL) && (bNoAutoalignmentIn))
					// disable autoaligment
				{
					// clear AUTO_ALIGNMENT to make 16bit access for flash programing possible
					WRITE_REG_UINT32(	(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL),
										(READ_REG_UINT32(pDevExtIn->pAdminReg + BOARD_ADMIN_REG__CONTROL) & PNDEV_UINT32_CLR_BIT_4));
				}
		...
		else
		{
				...
				// success
			{
					...
					// LBU
				{
					...
					(void)READ_REG_UINT16(pDevExtIn->uHwResKm.uBoard.as.uErtec200.uLbuReg.pPtr + ERTEC200_LBU_REG__P3_CFG);
					...
					//lint --e(550) lCfgTmp not accessed - yes, but it is needed, // ensure that new page is active before any access is done to it
				}
				else
				{
				UINT32 lAdrAhbTmp = 0;
	
					WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE3_OFFSET),
										lAdrAhb);
					WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE3_RANGE),
										BOARD_EB200_XHIF__PAGE_SIZE_2MB);
					WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE3_CFG),
										lPageCfg);
	
					// ptr=BAR2.Page3
					//	- dynamic DriverPage not accessible at UserMode -> set only pPtr
					// ptr = BAR3
					pHwDescKm->pPtr				= (pDevExtIn->uHwResKm.uBar[2].pVirtualAdr + BOARD_EB200_XHIF__BAR2_OFFSET_PAGE3);
					pHwDescKm->lIndexBar		= 2;
					pHwDescKm->lOffsetBar		= BOARD_EB200_XHIF__BAR2_OFFSET_PAGE3;
					...
					//			(read access is done to a different IP -> IP2-read can be executed before IP1-write)
					//		->	PageReg (IP1) must be read back (-> IP1-write finished) before there is a read access to IP2
					lAdrAhbTmp = READ_REG_UINT32(pDevExtIn->uHwResKm.uBoard.as.uErtec200.pXhifPageCfgReg + ERTEC200_XHIF_REG_CSR__PAGE3_OFFSET);
...
void fnBoardIrte_StopDriverPage(DEVICE_EXTENSION* pDevExtIn)
{
char sTraceDetail[PNDEV_SIZE_STRING_TRACE] = {0};
...
*/
		...
		// set AltFct0 (= function 01) for GPIO160
		//	- AltFct0: Bit1:0=01
		WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uSoc1.uApbPer.pPtr + SOC1_APB_PER__OFFSET_GPIO + SOC1_GPIO_REG__PORT_MODE5_L),
							(READ_REG_UINT32(pDevExtIn->uHwResKm.uBoard.as.uSoc1.uApbPer.pPtr + SOC1_APB_PER__OFFSET_GPIO + SOC1_GPIO_REG__PORT_MODE5_L) | PNDEV_UINT32_SET_BIT_0));

		// connect IntSrc=IRTE
		//	- use PCI-ICU-MuxA-Input13
		{
			// Note:
			//	- IRQ0_HP of IRTE is connected to IR_INTA of Logic-IRTE
			//	- IR_INTA of Logic-IRTE is connected to Pin160 of PCI-ICU

			// connect correct Pin
			WRITE_REG_UINT32(	(pDevExtIn->uHwResKm.uBoard.as.uSoc1.uPciIcu.pPtr + SOC1_PCI_ICU_REG__IRQ_SEL_A13),
								SOC1_PCI_ICU_PIN__IRTE_IRQ0);
		}

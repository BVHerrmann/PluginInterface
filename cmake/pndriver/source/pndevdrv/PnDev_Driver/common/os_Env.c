...
uPNDEV_FCT_PTR*	pFctPtrTmp	= NULL;
	...
	else
	{
		...
		pFctPtrTmp->pFnOpenTimer		= (PNDEV_FN_OPEN_TIMER)			GetProcAddress(	hDllTmp, "fnPnDev_OpenTimer");
		pFctPtrTmp->pFnCloseTimer		= (PNDEV_FN_CLOSE_TIMER)		GetProcAddress(	hDllTmp, "fnPnDev_CloseTimer");
...
uPNDEV_FCT_PTR*	pFctPtrTmp	= NULL;
	...
	pFctPtrTmp->pFnGetSharedHostMemAddr   = (PNDEV_FN_GET_SHARED_HOST_MEM_ADDR)   &fnPnDev_GetSharedHostMemAddr;
	...
	if	(	(pFctPtrTmp->pFnTest			== NULL)
		...
		||	(pFctPtrTmp->pFnRcveNrt			== NULL)
		||	(pFctPtrTmp->pFnChangeNrt		== NULL)
		||  (pFctPtrTmp->pFnGetSharedHostMemAddr       == NULL))
		// error
	{
		// set ErrorString
		_tcscpy_s(	sErrorOut, lDimErrorIn, _TEXT(__FUNCTION__));

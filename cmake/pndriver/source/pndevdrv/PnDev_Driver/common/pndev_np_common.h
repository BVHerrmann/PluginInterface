...
#define kNP_MAX_NUMBER_OF_EVENTS 3                          // maximum number of events supoorted by netPROFI
#define kNP_MAX_NUMBER_OF_EVENT_DATA 20                     // maximum number of event data which can be sent to high level
#define kNP_PREAMBLE                 0x55                   // The preamble value
#define kNP_PREAMBLE_PLUS_FLAG_BYTE_COUNT 8                 // The preamble is 7 bytes and Flag for filter event mechanism is 1 byte
                                                            // Total is 8 bytes.
#define kNP_OVERFLOW_EVENT_ID 0xFFFF
#define kNP_ALARM_FRAME_PDU_TYPE 0x11
#define kNP_ALARM_PAYLOAD_LENGTH 1432
    ...
    typedef enum _eNP_EVENT_TYPE
    {
        eNP_INVALID_EVENT = 0,
        eNP_ALARM_EVENT = 1,
        eNP_APDU_STATUS_EVENT = 2,
    }
    eNP_EVENT_TYPE;
...
#define kNP_MAX_ENTRIES_PROTOCOL 55									// maximal number of entries of ProtocolArray (MaxIndex+1)
    ...
    typedef enum _eNP_CODE_SERVICE
	{
		...
		eNP_SERVICE_SETUP_FILTER_EVENTS = 6,
		eNP_SERVICE_GET_FILTER_EVENTS = 7
	}
	eNP_CODE_SERVICE;
    ...
    typedef struct _uNP_FRAME_INFO
	{
		...
		UINT16			lOffsetData;                                /**< */
		UINT16			lSizeData;                                  /**< */
		eNP_EVENT_TYPE	lFlagForEvent;                              /**< Flag for event mechanism*/
...
//------------------------------------------------------------------------
	...
	uNP_FILE_INFO_V14;
//------------------------------------------------------------------------
	typedef struct _uNP_FILE_INFO_V15
	{
		// Attention: compiler may create FillBytes if size!=n*32Byte and alignment not ok

		// first block of 32Byte
		UINT32	lLenStruct;													// fixed position, sizeof(uNP_FILE_INFO_V15)
		UINT32	lVersion;													// fixed position, 0x15 = V1.5
		UINT64	lIndexFileAbsolute;											// absolute file index (incremented since creation of files at this directory)
		UINT32	lCtrByteValid;
		UINT32	lRevisionAsic;												// important for getting tick of ERTEC timer (10nsec, 1nsec)
		UINT32	lSizeFile;													// fixed size of a DataFile
    ...
    typedef struct _uNP_ALARM_BLOCK_HEADER
    {
        UINT16 lBlockType;
        UINT16 lBlockLength;
        UINT8 lBlockVersionHigh;
        UINT8 lBlockVersionLow;
    }uNP_ALARM_BLOCK_HEADER;


    typedef struct _uNP_ALARM_NOTIFICATION_LOW
    {
        uNP_ALARM_BLOCK_HEADER uBlockHeader;
        UINT16 lAlarmType;
        UINT32 lApi;
        UINT16 lSlotNumber;
        UINT16 lSubSlotNumber;
        UINT32 lModuleIdentNumber;
        UINT32 lSubModuleIdentNumber;
        UINT16 lAlarmSpecifier;
        UINT8  lRawPayload[kNP_ALARM_PAYLOAD_LENGTH];
    }uNP_ALARM_NOTIFICATION_LOW;
    
    /** Alarm Frame Structure.
     *   Alarm Frame Structure is used to define an alarm frame.
     */
    typedef struct _uNP_AlARM_FRAME
    {
        UINT16 lAlarmDstEndpoint;	/**< Destination ServiceAccess Point */
        UINT16 lAlarmSrcEndpoint;	/**< Source ServiceAccess Point */
        UINT8  lPduType;			/**< Protocol Data Unit (Pdu) Type*/
        UINT8  lAddFlags;			/**< Add flags */
        UINT16 lSendSeqNum;			/**< Send Sequence Number */
        UINT16 lAckSeqNum;			/**< Ack Sequence Number */
        UINT16 VarPartLen;			/**< Variable Part Length */
        uNP_ALARM_NOTIFICATION_LOW uAlarmNotificationLow; /**< AlarmNotification - PDU */

    }uNP_ALARM_FRAME;

    typedef struct _uNP_ALARM_EVENT
    {
        UINT16 lAlarmType;
        eNP_BOOL eAlarmTypeSpecified;

        UINT32 lApi;
        eNP_BOOL eApiSpecified;

        UINT16 lSlotNumber;
        eNP_BOOL eSlotSpecified;

        UINT16 lSubSlotNumber;
        eNP_BOOL eSubSlotSpecified;

        UINT32 lModuleIdentNumber;
        eNP_BOOL eModuleIdentNumberSpecified;

        UINT32 lSubModuleIdentNumber;
        eNP_BOOL eSubModuleIdentNumberSpecified;

        UINT16 lAlarmSpecifier;
        eNP_BOOL eAlarmSpecifierSpecified;

        UINT8 lAlarmPayload[kNP_ALARM_PAYLOAD_LENGTH];
        eNP_BOOL eAlarmPayloadSpecified;
    }
    uNP_ALARM_EVENT;

    typedef struct _uNP_APDU_STATUS_EVENT
    {
        UINT16 lFrameId;
        eNP_BOOL eFrameIdSpecified;
        UINT8 lDataStatus;
        eNP_BOOL eDataStatusSpecified;
        UINT8 lTransferStatus;
        eNP_BOOL eTransferStatusSpecified;
    }
    uNP_APDU_STATUS_EVENT;

    typedef struct _uNP_EVENT_CONFIG
    {
        UINT32 lEventId;
        UINT64 lSrcMacAdr;
        eNP_BOOL eSrcMacAdrSpecified;
        UINT64 lDstMacAdr;
        eNP_BOOL eDstMacAdrSpecified;

        eNP_EVENT_TYPE eEventType;

        uNP_ALARM_EVENT uAlarmEvent;
        uNP_APDU_STATUS_EVENT uApduStatusEvent;
    }
    uNP_EVENT_CONFIG;

    typedef struct _uNP_FILTER_EVENT_DATA
    {
        UINT32 lEventId;

        UINT32 lTimeHighPart;        /**< Counter overflow of AsicTimer */
        UINT32 lTimeLowPart;         /**< Value of AsicTimer */

        UINT32 lTimeCaptureStartAsic_10nsec;
        UINT64 lTimeCaptureStartPc_100nsec1601;

        UINT16 lPortRcve;  /**< Receive Port*/
    }uNP_FILTER_EVENT_DATA;

    typedef struct _uNP_FILTER_EVENTS
    {
        UINT16 lNumberOfEvents;
        uNP_FILTER_EVENT_DATA   uVarFilterEventData[kNP_MAX_NUMBER_OF_EVENT_DATA];
    }uNP_FILTER_EVENTS;
	...
	{
		UINT8	lLenHeaderAndVersionIpDiv4;									// real HeaderLength = lLenHeaderIpDiv4 x 4Byte (StandardValue=5)
		UINT8	lTos;
